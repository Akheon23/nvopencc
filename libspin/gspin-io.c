...
#ifdef __MINGW32__
  map_hd = CreateFileMapping((HANDLE)_get_osfhandle(fd), NULL,
                        PAGE_READWRITE, 0, (DWORD)mem_seg_length, NULL);
  GS_ASSERT(map_hd != NULL, "CreateFileMapping failure.\n");
  mem_seg = (char*)MapViewOfFileEx(map_hd, FILE_MAP_WRITE, 0, 0, 
                                   (DWORD)mem_seg_length, NULL);
  GS_ASSERT(mem_seg != NULL, "MapViewOfFileEx failed.");
#else /* __MINGW32__ */
...
#ifdef __MINGW32__
  map_hd = CreateFileMapping((HANDLE)_get_osfhandle(fd), NULL, PAGE_READONLY,
                             0, (DWORD)statbuf.st_size, NULL);
  GS_ASSERT(map_hd != NULL, "CreateFileMapping failure.\n");
  mem_seg = MapViewOfFileEx(map_hd, FILE_MAP_COPY, 0, 0, (DWORD)statbuf.st_size,
                            NULL);                              
  GS_ASSERT(mem_seg != NULL, "MapViewOfFileEx failed.");
#else /* __MINGW32__ */
