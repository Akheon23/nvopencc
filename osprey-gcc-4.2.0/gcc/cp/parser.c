...
#ifdef TARG_NVISA
extern bool loop_end_with_brace;
#endif
...
#ifdef TARG_NVISA
/*
 * gcc currently handles omp pragmas on loops and attribute-like pragmas on 
 * types and variables.  Most of the omp pragmas are handled by creating a 
 * omp tree that encloses the loop.  We don't want to modify the spin trees 
 * that much, and some directives like flush generate a builtin for the
 * information.  So we do a similar hack and generate an unroll builtin
 * that wgen will translate back to a pragma.
 */
static void
cp_parser_unroll (cp_parser *parser, cp_token *pragma_tok)
{
  tree amount = NULL_TREE;
  tree t;
  tree args;
  if (cp_lexer_next_token_is (parser->lexer, CPP_NUMBER)) {
    cp_token *token;
    /* Look for the number */
    token = cp_parser_require (parser, CPP_NUMBER, "number");
    /* Return the value.  */
    amount = token->u.value;
    if (TREE_CODE (amount) != INTEGER_CST)
      error ("invalid constant in %<#pragma unroll%>");
  }
  else {
    /* create max amount */
    amount = build_int_cstu (NULL_TREE, UINT_MAX);
  }
  cp_parser_require_pragma_eol (parser, pragma_tok);

  args = tree_cons (NULL, amount, NULL);
  t = built_in_decls[BUILT_IN_PRAGMA_UNROLL];
...
#ifdef TARG_NVISA
    case PRAGMA_UNROLL:
      cp_parser_unroll (parser, pragma_tok);
      return false;
#endif /* NVISA */
