...
#ifdef TARG_NVISA
tree
do_poplevel_end_loop (tree stmt_list)
{
   tree block = NULL;
   ...
   {
     tree end_loop_label_decl;
     tree end_loop_label_stmt;

     end_loop_label_decl = create_artificial_label ();
     end_loop_label_stmt = build1 (LABEL_EXPR, void_type_node, end_loop_label_decl);
     SET_EXPR_LOCATION (end_loop_label_stmt, input_location);

     if (TREE_CODE(stmt_list) == STATEMENT_LIST)
        append_to_statement_list(end_loop_label_stmt, &stmt_list);
     else
        if (TREE_CODE(stmt_list) == BIND_EXPR)
        {
           tree list;

           list = BIND_EXPR_BODY(stmt_list);
           if (TREE_CODE(list) == BIND_EXPR) 
            {
              if (loop_end_with_brace) {
                 tree list_1;

                 list_1 = make_node(STATEMENT_LIST);
                 TREE_TYPE (list_1) = void_type_node;
                 append_to_statement_list(list, &list_1);
                 append_to_statement_list(end_loop_label_stmt, &list_1);
                 BIND_EXPR_BODY(stmt_list)=list_1;
               }
             }
           else
              append_to_statement_list(end_loop_label_stmt, &list);
         }
         else
         if (loop_end_with_brace)
         {
           ...
           append_to_statement_list(stmt_list, &list);
           append_to_statement_list(end_loop_label_stmt, &list);
           return list;
         }
   }
...
{
#ifdef TARG_NVISA
  WHILE_BODY (while_stmt) = do_poplevel_end_loop (WHILE_BODY (while_stmt));
#else
...
{
#ifdef TARG_NVISA
  FOR_BODY (for_stmt) = do_poplevel_end_loop (FOR_BODY (for_stmt));
#else
