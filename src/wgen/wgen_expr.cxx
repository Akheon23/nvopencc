	...
	// OSP_382, do not store MTYPE_M into temp
	    ...
	    (volt ||
	     ...
	     assign_code == GS_POSTDECREMENT_EXPR)) { // save result in a preg
          ...
          // NVISA: use StidPreg/LdidPreg so preg ty matches
          // (avoids case of preg with "long int" type).
          // Should be fine for other targets too.
          wn = WN_StidPreg(rtype, result_preg, result_wn);
          WGEN_Stmt_Append (wn, Get_Srcpos());
          result_wn = WN_LdidPreg(rtype, result_preg);
        }
      ...
      if (need_result) {
        if (! result_in_temp)
          wn = WN_CreateLdid(OPR_LDID, rtype, desc, 
			     ...
			     field_id);
        else wn = WN_LdidPreg(rtype, result_preg);
        if (is_realpart)
...
#ifdef TARG_NVISA
static void
emit_builtin_fence (INTRINSIC id, gs_t exp, INT32 k)
{
  WN *wn;
  gs_t list = gs_tree_operand (exp, 1);
  emit_barrier (TRUE,  list, k);
  wn = WN_Create_Intrinsic (OPC_VINTRINSIC_CALL, id, 0, NULL);
  WGEN_Stmt_Append (wn, Get_Srcpos());
  emit_barrier (FALSE, list, k);
} /* emit_builtin_fence */
...
static WN*
emit_builtin_clock64 (void)
{
  WN *wn = WN_Create_Intrinsic (OPC_I8INTRINSIC_CALL, INTRN_CLOCK64, 0, NULL);
  WGEN_Stmt_Append (wn, Get_Srcpos());

  ST       *preg_st = MTYPE_To_PREG(MTYPE_I8);
  TY_IDX    preg_ty_idx = Be_Type_Tbl(MTYPE_I8);
  PREG_NUM  preg = Create_Preg (MTYPE_I8, NULL);

  wn = WN_Ldid (MTYPE_I8, -1, Return_Val_Preg, preg_ty_idx);
  wn = WN_Stid (MTYPE_I8, preg, preg_st, preg_ty_idx, wn),
  WGEN_Stmt_Append (wn, Get_Srcpos());

  wn = WN_Ldid (MTYPE_I8, preg, preg_st, preg_ty_idx);
  return wn;
}


static WN*
emit_builtin_atomic (gs_t exp, UINT nkids, TYPE_ID mtype, INTRINSIC iopc)
{
  WN        *wn;
  WN        *arg_wn;
  WN        *ikids [3];
...
static WN*
emit_builtin_vote (gs_t exp, INTRINSIC iopc)
{
  // NB: We are using MTYPE_I4 instead of MTYPE_B since
  // intrinsics do not allow the predicate type.
  TYPE_ID Return_Val_Ty;
  if (iopc == INTRN_VOTEBALLOT) {
      Return_Val_Ty = MTYPE_U4;
  } else {
      Return_Val_Ty = MTYPE_I4;
  }   
  OPCODE     opc = OPCODE_make_op (OPR_INTRINSIC_CALL, Return_Val_Ty, MTYPE_V);
  gs_t arg = gs_tree_value (gs_tree_operand (exp, 1));
  TY_IDX arg_ty_idx = Get_TY(gs_tree_type (arg));
  TYPE_ID arg_mtype = TY_mtype (arg_ty_idx);
  WN *wn, *arg_wn, *ikids [1];

  arg_wn = WGEN_Expand_Expr (arg);
  arg_wn = WN_CreateParm (arg_mtype, arg_wn, arg_ty_idx, WN_PARM_BY_VALUE);
  ikids [0] = arg_wn;
  ...
  ST       *preg_st = MTYPE_To_PREG(Return_Val_Ty);
  TY_IDX    preg_ty_idx = ST_type(preg_st);
  PREG_NUM  preg = Create_Preg (Return_Val_Ty, NULL);

  wn = WN_Ldid (Return_Val_Ty, -1, Return_Val_Preg, preg_ty_idx);
  wn = WN_Stid (Return_Val_Ty, preg, preg_st, preg_ty_idx, wn);
  WGEN_Stmt_Append (wn, Get_Srcpos());

  wn = WN_Ldid (Return_Val_Ty, preg, preg_st, preg_ty_idx);

  return wn;
}

static void
emit_builtin_pmevent (gs_t exp)
{
  gs_t list = gs_tree_operand (exp, 1);
  TY_IDX arg_ty_idx = Get_TY (gs_tree_type (gs_tree_value (list)));
  TYPE_ID arg_mtype = TY_mtype (arg_ty_idx);
  WN *wn, *arg_wn = WGEN_Expand_Expr (gs_tree_value (list));

  arg_wn = WN_CreateParm (arg_mtype, arg_wn, arg_ty_idx, WN_PARM_BY_VALUE);
  emit_barrier (TRUE,  NULL, 0);
  wn = WN_Create_Intrinsic (OPC_VINTRINSIC_CALL, INTRN_PMEVENT, 1, &arg_wn);
  WGEN_Stmt_Append (wn, Get_Srcpos());
  emit_barrier (FALSE, NULL, 0);
  FmtAssert (gs_tree_code (gs_tree_value (list)) == GS_INTEGER_CST,
             ("__prof_trigger() requires constant int argument"));
} /* emit_builtin_pmevent */

#ifdef FUTURE_SUPPORT  /* FERMI */
static WN *
emit_builtin_bitcount (gs_t exp, INTRINSIC iopc, TYPE_ID formal_type)
{
  gs_t arg = gs_tree_value (gs_tree_operand (exp, 1));
  TY_IDX arg_ty_idx = Get_TY (gs_tree_type (arg));
  WN *arg_wn  = WN_CreateParm (formal_type, WGEN_Expand_Expr (arg), arg_ty_idx, WN_PARM_BY_VALUE);
  WN *wn = WN_Create (OPR_INTRINSIC_CALL, MTYPE_I4, MTYPE_V, 1);

  WN_intrinsic (wn) = iopc;
  WN_kid0 (wn) = arg_wn;
  WGEN_Stmt_Append (wn, Get_Srcpos ());
...
static char
get_asm_mtype_constraint (TYPE_ID mtype)
{
        switch (mtype) {
        case MTYPE_I1:
        case MTYPE_U1:
...
create_asm_output_constraints (INT num_outputs, TYPE_ID mtype)
{
  ...
  for (opnd_num = 0; opnd_num < num_outputs; ++opnd_num) {
        ...
        buf[0] = '=';
        buf[1] = get_asm_mtype_constraint(mtype);
        buf[2] = '\0';
        ST_Init(constraint_st, Save_Str(buf),
...
static WN*
emit_builtin_texfetch (gs_t exp, const char *name, TYPE_ID rtype, TYPE_ID ctype)
{
  ...
  gs_t list = gs_tree_operand (exp, 1);
  WN *texture_arg = WGEN_Expand_Expr (gs_tree_value (list));
  list       = gs_tree_chain (list);
  WN *coord_arg = WGEN_Expand_Expr (gs_tree_value (list));
...
static const char*
get_clamp_value (WN *clamp_tree)
{
  FmtAssert (WN_operator(clamp_tree) == OPR_INTCONST, ("clamp not constant?"));
  switch (WN_const_val(clamp_tree)) {
  case 0:
    return ".zero";
  case 1:
    return ".clamp";
  case 2:
    return ".trap";
  default:
    FmtAssert(FALSE, ("unexpected clamp value"));
    return NULL;
  }
}

// parameterized routine for surface reads:
// pass in ptx inst name, element type of result and coord
static WN*
emit_builtin_surfread (gs_t exp, INT cdim, INT ddim, TYPE_ID dtype, const char *name)
{
  gs_t list = gs_tree_operand (exp, 1);
  WN *surface_arg = WGEN_Expand_Expr (gs_tree_value (list));
  list       = gs_tree_chain (list);
  WN *coord_arg1 = WGEN_Expand_Expr (gs_tree_value (list));
  WN *coord_arg2;
  if (cdim == 2) {
    list       = gs_tree_chain (list);
    coord_arg2 = WGEN_Expand_Expr (gs_tree_value (list));
  }
  list       = gs_tree_chain (list);
  WN *clamp_arg = WGEN_Expand_Expr (gs_tree_value (list));

  char buf[80];
  strcpy(buf, name);

  strcat (buf, get_clamp_value(clamp_arg));

  FmtAssert(cdim == 1 || cdim == 2, ("unexpected cdim value"));
  if (ddim == 1) {
    strcat(buf, " {%0},");
    if (cdim == 1)
      strcat(buf, "[%1,{%2}];");
    ...
    FmtAssert(FALSE, ("unexpected ddim value"));

  WN *asm_wn = WN_CreateAsm_Stmt (3 + cdim /* 2 + #inputs */, buf);
  ...
  UINT opnd_num = ddim;
  WN_kid1(asm_wn) = create_asm_output_constraints (opnd_num, dtype);

  // surface param
  WN_kid (asm_wn, 2) = WN_CreateAsm_Input ("m", opnd_num, surface_arg);
  ++opnd_num;
  // coord param
  WN_kid (asm_wn, 3) = WN_CreateAsm_Input ("r", opnd_num, coord_arg1);
  ++opnd_num;
  if (cdim == 2) {
    WN_kid (asm_wn, 4) = WN_CreateAsm_Input ("r", opnd_num, coord_arg2);
  }
  return asm_wn;
}

// parameterized routine for surface writes:
// pass in ptx inst name, element type of result and coord
static void
emit_builtin_surfwrite (gs_t exp, INT cdim, INT adim, TYPE_ID atype, const char *name)
{
  gs_t list = gs_tree_operand (exp, 1);
  WN *addr_arg = WGEN_Expand_Expr (gs_tree_value(list));
  list       = gs_tree_chain (list);
  WN *surface_arg = WGEN_Expand_Expr (gs_tree_value (list));
  list       = gs_tree_chain (list);
  WN *coord_arg1 = WGEN_Expand_Expr (gs_tree_value (list));
  ...
  strcat(buf, " [%0,");
  FmtAssert(cdim == 1 || cdim == 2, ("unexpected cdim value"));
  FmtAssert(adim == 1 || adim == 2 || adim == 4, ("unexpected adim value"));
  if (cdim == 1) {
    strcat(buf, "{%1}],");
    if (adim == 1)
      strcat(buf, "{%2};");
  ...
  WN *asm_wn = WN_CreateAsm_Stmt (3 + cdim + adim /* 2 + #inputs */, buf);
  ...
  UINT opnd_num = 0;
  UINT kidnum = 2;
  WN_kid1(asm_wn) = create_asm_output_constraints (0, MTYPE_V);
  ...
  ++kidnum;
  ++opnd_num;
  WN_kid (asm_wn, kidnum) = WN_CreateAsm_Input ("r", opnd_num, coord_arg1);
  if (cdim == 2) {
    ++kidnum;
    ++opnd_num;
    WN_kid (asm_wn, kidnum) = WN_CreateAsm_Input ("r", opnd_num, coord_arg2);
  }

  // addr param (split into fields)
  INT offset = 0;
  INT field_id = 1;
  char cbuf[4];
  cbuf[0] = get_asm_mtype_constraint(atype);
  cbuf[1] = '\0';
  for (; field_id <= adim; ++field_id) {
    ++kidnum;
    ++opnd_num;
    WN *addrf = WN_Ldid (atype, offset, WN_st(addr_arg), WN_ty(addr_arg),
      field_id);
    WN_kid (asm_wn, kidnum) = WN_CreateAsm_Input (cbuf, opnd_num, addrf);
    offset += MTYPE_byte_size(atype);
  }
...
/* mov.b64 {r1,r2},fd1 */
static WN*
emit_builtin_double_to_int2 (gs_t exp)
{
  gs_t list = gs_tree_operand (exp, 1);
  WN *double_arg = WGEN_Expand_Expr (gs_tree_value (list));
  ...
  // double param
  WN_kid (asm_wn, 2) =
        WN_CreateAsm_Input ((char*)"f", opnd_num, double_arg);
  ++opnd_num;
...
/* mov.b64 fd1,{r1,r2} */
static WN*
emit_builtin_int2_to_double (gs_t exp)
{
  gs_t list = gs_tree_operand (exp, 1);
  WN *int2_arg = WGEN_Expand_Expr (gs_tree_value (list));
...
#ifdef FUTURE_SUPPORT  /* FERMI */ 
static WN*
emit_builtin_bar_red (gs_t exp, INTRINSIC iopc)
{
  // NB: We are using MTYPE_I4 instead of MTYPE_B since
  // intrinsics do not allow the predicate type.
...
#ifdef TARG_NVISA
                // This can happen due to textures and surfaces since the 
                // pattern COMMA with kid0 being a BLOCK containing an ASM 
                // statement and kid1 being the load or iload
                else if ((WN_operator(result_wn) == OPR_COMMA) &&
                         (WN_operator(WN_kid1(result_wn)) == OPR_ILOAD ||
                          WN_operator(WN_kid1(result_wn)) == OPR_LDID
                         )
                        ) {
          	  WGEN_Stmt_Append(WN_Stid (mtype, ST_ofst(st), st, ty,
                                  result_wn), Get_Srcpos());
...
#ifdef __MINGW32__
        gs_realign_t val;
        val.data[0] = gs_ull(gs_tree_int_cst_low(exp));
        val.data[1] = gs_ll(gs_tree_int_cst_high(exp));
        wn = WN_Intconst(mtyp, val.ll);
#else //__MINGW32__
...
#else
        // NOTE: This should be more correct than the code above for other
        // targets as well, leaving as TARG_NVISA since we do not test other
        // targets
        {
          if (mtyp != WN_rtype(wn0)) {
	    wn = WN_CreateCvtl(OPR_CVTL, Widen_Mtype(WN_rtype(wn0)), MTYPE_V,
                               MTYPE_size_min(mtyp), wn0);
            if (WN_rtype(wn0) != Widen_Mtype(mtyp)) {
              wn = WN_Cvt(WN_rtype(wn0), Widen_Mtype(mtyp), wn);
            }
          } else {
            wn = WN_CreateCvtl(OPR_CVTL, Widen_Mtype(mtyp), MTYPE_V,
                               MTYPE_size_min(mtyp), wn0);
...
#else
#ifdef TARG_NVISA
       // want result to be boolean (cause only have boolean result in ptx)
...
#ifdef TARG_NVISA
  		call_wn = WN_Create_Intrinsic (OPC_VINTRINSIC_CALL, INTRN_TRAP,
                                               0, NULL);
#else
...
*/
              ...
              case GSBI_BUILT_IN_FMAX:
                ...
                whirl_generated = TRUE;
                break;
              case GSBI_BUILT_IN_MULHI:
              case GSBI_BUILT_IN_UMULHI:
              case GSBI_BUILT_IN_MUL64HI:
              case GSBI_BUILT_IN_UMUL64HI:
                arg1 = gs_tree_value (arglist);
                arg2 = gs_tree_value (gs_tree_chain (arglist));
                ...
                whirl_generated = TRUE;
                break;
              case GSBI_BUILT_IN_INT2FLOAT:
              case GSBI_BUILT_IN_FLOAT2INT:
              case GSBI_BUILT_IN_LONGLONG2DOUBLE:
              case GSBI_BUILT_IN_DOUBLE2LONGLONG:
                arg_wn = WGEN_Expand_Expr (gs_tree_value (gs_tree_operand (exp, 1)));
                wn = WN_Tas (ret_mtype, MTYPE_To_TY(ret_mtype), arg_wn);
                whirl_generated = TRUE;
                break;
              case GSBI_BUILT_IN_RSQRT:
              case GSBI_BUILT_IN_RSQRTF:
                arg_wn = WGEN_Expand_Expr (gs_tree_value(gs_tree_operand(exp, 1)));
                wn = WN_CreateExp1 (OPR_RSQRT, ret_mtype, MTYPE_V, arg_wn);
                whirl_generated = TRUE;
                break;

              case GSBI_BUILT_IN_GETSHAREDMEM:
                iopc =   INTRN_GETSHAREDMEM;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_GETBLOCKIDPTR:
                iopc =   INTRN_GETBLOCKIDPTR;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_EXP2F:
                iopc = INTRN_F4EXP2;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_EXP2:
                iopc = INTRN_F8EXP2;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_LOG2F:
                iopc = INTRN_F4LOG2;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_LOG2:
                iopc = INTRN_F8LOG2;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_SATURATE:
                iopc = INTRN_F8SATURATE;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_SATURATEF:
                iopc = INTRN_F4SATURATE;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_MUL24:
                iopc = INTRN_MUL24;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_UMUL24:
                iopc = INTRN_UMUL24;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_F4ADD_ROUND:
                iopc = INTRN_F4ADD_ROUND;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_F4ADD_TRUNC:
                iopc = INTRN_F4ADD_TRUNC;
                intrinsic_op = TRUE;
                break;
#ifdef FUTURE_SUPPORT  /* FERMI */
              case GSBI_BUILT_IN_F4ADD_FLOOR:
                iopc = INTRN_F4ADD_FLOOR;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_F4ADD_CEIL:
                iopc = INTRN_F4ADD_CEIL;
                intrinsic_op = TRUE;
                break;
#endif  /* FUTURE_SUPPORT */
              case GSBI_BUILT_IN_F8ADD_ROUND:
                iopc = INTRN_F8ADD_ROUND;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_F8ADD_TRUNC:
                iopc = INTRN_F8ADD_TRUNC;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_F8ADD_FLOOR:
                iopc = INTRN_F8ADD_FLOOR;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_F8ADD_CEIL:
                iopc = INTRN_F8ADD_CEIL;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_F4MUL_ROUND:
                iopc = INTRN_F4MUL_ROUND;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_F4MUL_TRUNC:
                iopc = INTRN_F4MUL_TRUNC;
                intrinsic_op = TRUE;
                break;
#ifdef FUTURE_SUPPORT  /* FERMI */
              case GSBI_BUILT_IN_F4MUL_FLOOR:
                iopc = INTRN_F4MUL_FLOOR;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_F4MUL_CEIL:
                iopc = INTRN_F4MUL_CEIL;
                intrinsic_op = TRUE;
                break;
#endif  /* FUTURE_SUPPORT */
              case GSBI_BUILT_IN_F8MUL_ROUND:
                iopc = INTRN_F8MUL_ROUND;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_F8MUL_TRUNC:
                iopc = INTRN_F8MUL_TRUNC;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_F8MUL_FLOOR:
                iopc = INTRN_F8MUL_FLOOR;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_F8MUL_CEIL:
                iopc = INTRN_F8MUL_CEIL;
                intrinsic_op = TRUE;
                break;
#ifdef FUTURE_SUPPORT  /* FERMI */
              case GSBI_BUILT_IN_FDIV_ROUND:
                iopc = INTRN_F4DIV_ROUND;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_FDIV_TRUNC:
                iopc = INTRN_F4DIV_TRUNC;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_FDIV_FLOOR:
                iopc = INTRN_F4DIV_FLOOR;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_FDIV_CEIL:
                iopc = INTRN_F4DIV_CEIL;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_FRCP_ROUND:
                iopc = INTRN_F4RCP_ROUND;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_FRCP_TRUNC:
                iopc = INTRN_F4RCP_TRUNC;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_FRCP_FLOOR:
                iopc = INTRN_F4RCP_FLOOR;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_FRCP_CEIL:
                iopc = INTRN_F4RCP_CEIL;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_FSQRT_ROUND:
                iopc = INTRN_F4SQRT_ROUND;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_FSQRT_TRUNC:
                iopc = INTRN_F4SQRT_TRUNC;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_FSQRT_FLOOR:
                iopc = INTRN_F4SQRT_FLOOR;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_FSQRT_CEIL:
                iopc = INTRN_F4SQRT_CEIL;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_DDIV_ROUND:
                iopc = INTRN_F8DIV_ROUND;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_DDIV_TRUNC:
                iopc = INTRN_F8DIV_TRUNC;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_DDIV_FLOOR:
                iopc = INTRN_F8DIV_FLOOR;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_DDIV_CEIL:
                iopc = INTRN_F8DIV_CEIL;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_DRCP_ROUND:
                iopc = INTRN_F8RCP_ROUND;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_DRCP_TRUNC:
                iopc = INTRN_F8RCP_TRUNC;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_DRCP_FLOOR:
                iopc = INTRN_F8RCP_FLOOR;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_DRCP_CEIL:
                iopc = INTRN_F8RCP_CEIL;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_DSQRT_ROUND:
                iopc = INTRN_F8SQRT_ROUND;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_DSQRT_TRUNC:
                iopc = INTRN_F8SQRT_TRUNC;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_DSQRT_FLOOR:
                iopc = INTRN_F8SQRT_FLOOR;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_DSQRT_CEIL:
                iopc = INTRN_F8SQRT_CEIL;
                intrinsic_op = TRUE;
                break;
#endif  /* FUTURE_SUPPORT */
              ...
              case GSBI_BUILT_IN_FMA_ROUND:
                iopc = INTRN_F8MA_ROUND;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_FMA_TRUNC:
                iopc = INTRN_F8MA_TRUNC;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_FMA_FLOOR:
                iopc = INTRN_F8MA_FLOOR;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_FMA_CEIL:
                iopc = INTRN_F8MA_CEIL;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_FMAF:
                iopc = INTRN_F4MA;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_FDIV_APPROX:
                iopc = INTRN_F4DIV_APPROX;
                intrinsic_op = TRUE;
                break;
#ifdef FUTURE_SUPPORT  /* FERMI */
              case GSBI_BUILT_IN_FMAF_ROUND:
                iopc = INTRN_F4MA_ROUND;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_FMAF_TRUNC:
                iopc = INTRN_F4MA_TRUNC;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_FMAF_FLOOR:
                iopc = INTRN_F4MA_FLOOR;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_FMAF_CEIL:
                iopc = INTRN_F4MA_CEIL;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_FMAF_IEEE_ROUND:
                iopc = INTRN_F4MA_IEEE_ROUND;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_FMAF_IEEE_TRUNC:
                iopc = INTRN_F4MA_IEEE_TRUNC;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_FMAF_IEEE_FLOOR:
                iopc = INTRN_F4MA_IEEE_FLOOR;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_FMAF_IEEE_CEIL:
                iopc = INTRN_F4MA_IEEE_CEIL;
                intrinsic_op = TRUE;
                break;
#endif  /* FUTURE_SUPPORT */
              case GSBI_BUILT_IN_SAD:
                iopc = INTRN_I4SAD;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_USAD:
                iopc = INTRN_U4SAD;
                intrinsic_op = TRUE;
                break;

              case GSBI_BUILT_IN_HILOINT_TO_DOUBLE:
                iopc = INTRN_F8HLI2D;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_DOUBLE_TO_LOINT:
                iopc = INTRN_I4D2LI;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_DOUBLE_TO_HIINT:
                iopc = INTRN_I4D2HI;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_DOUBLE_TO_FLOAT_ROUND:
                iopc = INTRN_F4F8CVT_ROUND;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_DOUBLE_TO_FLOAT_TRUNC:
                iopc = INTRN_F4F8CVT_TRUNC;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_DOUBLE_TO_FLOAT_CEIL:
                iopc = INTRN_F4F8CVT_CEIL;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_DOUBLE_TO_FLOAT_FLOOR:
                iopc = INTRN_F4F8CVT_FLOOR;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_DOUBLE_TO_INT_ROUND:
                iopc = INTRN_I4F8CVT_ROUND;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_DOUBLE_TO_INT_TRUNC:
                iopc = INTRN_I4F8CVT_TRUNC;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_DOUBLE_TO_INT_CEIL:
                iopc = INTRN_I4F8CVT_CEIL;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_DOUBLE_TO_INT_FLOOR:
                iopc = INTRN_I4F8CVT_FLOOR;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_DOUBLE_TO_UINT_ROUND:
                iopc = INTRN_U4F8CVT_ROUND;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_DOUBLE_TO_UINT_TRUNC:
                iopc = INTRN_U4F8CVT_TRUNC;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_DOUBLE_TO_UINT_CEIL:
                iopc = INTRN_U4F8CVT_CEIL;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_DOUBLE_TO_UINT_FLOOR:
                iopc = INTRN_U4F8CVT_FLOOR;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_INT_TO_DOUBLE_ROUND:
                iopc = INTRN_F8I4CVT_ROUND;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_INT_TO_DOUBLE_TRUNC:
                iopc = INTRN_F8I4CVT_TRUNC;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_INT_TO_DOUBLE_FLOOR:
                iopc = INTRN_F8I4CVT_FLOOR;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_INT_TO_DOUBLE_CEIL:
                iopc = INTRN_F8I4CVT_CEIL;
                intrinsic_op = TRUE;
                break;
             case GSBI_BUILT_IN_UINT_TO_DOUBLE_ROUND:
               iopc = INTRN_F8U4CVT_ROUND;
               intrinsic_op = TRUE;
               break;
             case GSBI_BUILT_IN_UINT_TO_DOUBLE_TRUNC:
               iopc = INTRN_F8U4CVT_TRUNC;
               intrinsic_op = TRUE;
               break;
             case GSBI_BUILT_IN_UINT_TO_DOUBLE_FLOOR:
               iopc = INTRN_F8U4CVT_FLOOR;
               intrinsic_op = TRUE;
               break;
             case GSBI_BUILT_IN_UINT_TO_DOUBLE_CEIL:
               iopc = INTRN_F8U4CVT_CEIL;
               intrinsic_op = TRUE;
               break;
             case GSBI_BUILT_IN_INT_TO_FLOAT_ROUND:
               iopc = INTRN_F4I4CVT_ROUND;
               intrinsic_op = TRUE;
               break;
             case GSBI_BUILT_IN_INT_TO_FLOAT_TRUNC:
               iopc = INTRN_F4I4CVT_TRUNC;
               intrinsic_op = TRUE;
               break;
             case GSBI_BUILT_IN_INT_TO_FLOAT_FLOOR:
               iopc = INTRN_F4I4CVT_FLOOR;
               intrinsic_op = TRUE;
               break;
             case GSBI_BUILT_IN_INT_TO_FLOAT_CEIL:
               iopc = INTRN_F4I4CVT_CEIL;
               intrinsic_op = TRUE;
               break;
             case GSBI_BUILT_IN_UINT_TO_FLOAT_ROUND:
               iopc = INTRN_F4U4CVT_ROUND;
               intrinsic_op = TRUE;
               break;
             case GSBI_BUILT_IN_UINT_TO_FLOAT_TRUNC:
               iopc = INTRN_F4U4CVT_TRUNC;
               intrinsic_op = TRUE;
               break;
             case GSBI_BUILT_IN_UINT_TO_FLOAT_FLOOR:
               iopc = INTRN_F4U4CVT_FLOOR;
               intrinsic_op = TRUE;
               break;
              case GSBI_BUILT_IN_UINT_TO_FLOAT_CEIL:
                iopc = INTRN_F4U4CVT_CEIL;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_FLOAT_TO_INT_ROUND:
                iopc = INTRN_I4F4CVT_ROUND;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_FLOAT_TO_INT_TRUNC:
                iopc = INTRN_I4F4CVT_TRUNC;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_FLOAT_TO_INT_FLOOR:
                iopc = INTRN_I4F4CVT_FLOOR;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_FLOAT_TO_INT_CEIL:
                iopc = INTRN_I4F4CVT_CEIL;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_FLOAT_TO_UINT_ROUND:
                iopc = INTRN_U4F4CVT_ROUND;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_FLOAT_TO_UINT_TRUNC:
                iopc = INTRN_U4F4CVT_TRUNC;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_FLOAT_TO_UINT_FLOOR:
                iopc = INTRN_U4F4CVT_FLOOR;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_FLOAT_TO_UINT_CEIL:
                iopc = INTRN_U4F4CVT_CEIL;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_FLOAT_TO_LONGLONG_ROUND:
                iopc = INTRN_I8F4CVT_ROUND;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_FLOAT_TO_LONGLONG_TRUNC:
                iopc = INTRN_I8F4CVT_TRUNC;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_FLOAT_TO_LONGLONG_FLOOR:
                iopc = INTRN_I8F4CVT_FLOOR;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_FLOAT_TO_LONGLONG_CEIL:
                iopc = INTRN_I8F4CVT_CEIL;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_FLOAT_TO_ULONGLONG_ROUND:
                iopc = INTRN_U8F4CVT_ROUND;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_FLOAT_TO_ULONGLONG_TRUNC:
                iopc = INTRN_U8F4CVT_TRUNC;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_FLOAT_TO_ULONGLONG_FLOOR:
                iopc = INTRN_U8F4CVT_FLOOR;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_FLOAT_TO_ULONGLONG_CEIL:
                iopc = INTRN_U8F4CVT_CEIL;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_DOUBLE_TO_LONGLONG_ROUND:
                iopc = INTRN_I8F8CVT_ROUND;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_DOUBLE_TO_LONGLONG_TRUNC:
                iopc = INTRN_I8F8CVT_TRUNC;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_DOUBLE_TO_LONGLONG_FLOOR:
                iopc = INTRN_I8F8CVT_FLOOR;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_DOUBLE_TO_LONGLONG_CEIL:
                iopc = INTRN_I8F8CVT_CEIL;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_DOUBLE_TO_ULONGLONG_ROUND:
                iopc = INTRN_U8F8CVT_ROUND;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_DOUBLE_TO_ULONGLONG_TRUNC:
                iopc = INTRN_U8F8CVT_TRUNC;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_DOUBLE_TO_ULONGLONG_FLOOR:
                iopc = INTRN_U8F8CVT_FLOOR;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_DOUBLE_TO_ULONGLONG_CEIL:
                iopc = INTRN_U8F8CVT_CEIL;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_LONGLONG_TO_FLOAT_ROUND:
                iopc = INTRN_F4I8CVT_ROUND;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_LONGLONG_TO_FLOAT_TRUNC:
                iopc = INTRN_F4I8CVT_TRUNC;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_LONGLONG_TO_FLOAT_FLOOR:
                iopc = INTRN_F4I8CVT_FLOOR;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_LONGLONG_TO_FLOAT_CEIL:
                iopc = INTRN_F4I8CVT_CEIL;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_ULONGLONG_TO_FLOAT_ROUND:
                iopc = INTRN_F4U8CVT_ROUND;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_ULONGLONG_TO_FLOAT_TRUNC:
                iopc = INTRN_F4U8CVT_TRUNC;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_ULONGLONG_TO_FLOAT_FLOOR:
                iopc = INTRN_F4U8CVT_FLOOR;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_ULONGLONG_TO_FLOAT_CEIL:
                iopc = INTRN_F4U8CVT_CEIL;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_LONGLONG_TO_DOUBLE_ROUND:
                iopc = INTRN_F8I8CVT_ROUND;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_LONGLONG_TO_DOUBLE_TRUNC:
                iopc = INTRN_F8I8CVT_TRUNC;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_LONGLONG_TO_DOUBLE_FLOOR:
                iopc = INTRN_F8I8CVT_FLOOR;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_LONGLONG_TO_DOUBLE_CEIL:
                iopc = INTRN_F8I8CVT_CEIL;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_ULONGLONG_TO_DOUBLE_ROUND:
                iopc = INTRN_F8U8CVT_ROUND;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_ULONGLONG_TO_DOUBLE_TRUNC:
                iopc = INTRN_F8U8CVT_TRUNC;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_ULONGLONG_TO_DOUBLE_FLOOR:
                iopc = INTRN_F8U8CVT_FLOOR;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_ULONGLONG_TO_DOUBLE_CEIL:
                iopc = INTRN_F8U8CVT_CEIL;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_FLOAT_TO_HALF_ROUND:
                iopc = INTRN_F2F4CVT_ROUND;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_HALF_TO_FLOAT:
                iopc = INTRN_F4F2CVT;
                intrinsic_op = TRUE;
                break;

#ifdef FUTURE_SUPPORT  /* FERMI */
              case GSBI_BUILT_IN_CLZ_NVISA:
                iopc = INTRN_I4I4CLZ;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_CLZLL_NVISA:
                iopc = INTRN_I4I8CLZ;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_POPC:
                iopc = INTRN_I4U4POPC;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_POPCLL:
                iopc = INTRN_I4U8POPC;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_BREV:
                iopc = INTRN_BREV;
                intrinsic_op = TRUE;
                break;
              case GSBI_BUILT_IN_BREVLL:
                iopc = INTRN_BREVLL;
                intrinsic_op = TRUE;
                break;
#endif  /* FUTURE_SUPPORT */

              case GSBI_BUILT_IN_SYNCTHREADS:
                emit_builtin_fence (INTRN_SYNCTHREADS, exp, num_args);
                whirl_generated = TRUE;
                break;
              case GSBI_BUILT_IN_THREADFENCE:
                emit_builtin_fence (INTRN_THREADFENCE, exp, num_args);
                whirl_generated = TRUE;
                break;
              case GSBI_BUILT_IN_THREADFENCE_BLOCK:
                emit_builtin_fence (INTRN_THREADFENCE_BLOCK, exp, num_args);
                whirl_generated = TRUE;
                break;
#ifdef FUTURE_SUPPORT  /* FERMI */
              case GSBI_BUILT_IN_THREADFENCE_SYS:
                emit_builtin_fence (INTRN_THREADFENCE_SYS, exp, num_args);
                whirl_generated = TRUE;
                break;
#endif  /* FUTURE_SUPPORT */

              case GSBI_BUILT_IN_BRKPT:
                emit_builtin_brkpt ();
                whirl_generated = TRUE;
                break;
              case GSBI_BUILT_IN_CLOCK:
                // make it a call not an op cause not pure
                wn = emit_builtin_clock ();
                whirl_generated = TRUE;
                break;
#ifdef FUTURE_SUPPORT  /* FERMI */
              case GSBI_BUILT_IN_CLOCK64:
                wn = emit_builtin_clock64 ();
                whirl_generated = TRUE;
                break;
#endif
              case GSBI_BUILT_IN_IATOMICADD:
                wn = emit_builtin_atomic (exp, 2, MTYPE_I4, INTRN_I4ATOMICADD);
                whirl_generated = TRUE;
                break;
              case GSBI_BUILT_IN_UATOMICADD:
                wn = emit_builtin_atomic (exp, 2, MTYPE_U4, INTRN_U4ATOMICADD);
                whirl_generated = TRUE;
                break;
              case GSBI_BUILT_IN_ULLATOMICADD:
                wn = emit_builtin_atomic (exp, 2, MTYPE_U8, INTRN_U8ATOMICADD);
                whirl_generated = TRUE;
                break;
              case GSBI_BUILT_IN_FATOMICADD:
                wn = emit_builtin_atomic (exp, 2, MTYPE_F4, INTRN_F4ATOMICADD);
                whirl_generated = TRUE;
                break;
              case GSBI_BUILT_IN_DATOMICADD:
                wn = emit_builtin_atomic (exp, 2, MTYPE_F8, INTRN_F8ATOMICADD);
                whirl_generated = TRUE;
                break;
              case GSBI_BUILT_IN_IATOMICMIN:
                wn = emit_builtin_atomic (exp, 2, MTYPE_I4, INTRN_I4ATOMICMIN);
                whirl_generated = TRUE;
                break;
              case GSBI_BUILT_IN_UATOMICMIN:
                wn = emit_builtin_atomic (exp, 2, MTYPE_U4, INTRN_U4ATOMICMIN);
                whirl_generated = TRUE;
                break;
              case GSBI_BUILT_IN_FATOMICMIN:
                wn = emit_builtin_atomic (exp, 2, MTYPE_F4, INTRN_F4ATOMICMIN);
                whirl_generated = TRUE;
                break;
              case GSBI_BUILT_IN_IATOMICMAX:
                wn = emit_builtin_atomic (exp, 2, MTYPE_I4, INTRN_I4ATOMICMAX);
                whirl_generated = TRUE;
                break;
              case GSBI_BUILT_IN_UATOMICMAX:
                wn = emit_builtin_atomic (exp, 2, MTYPE_U4, INTRN_U4ATOMICMAX);
                whirl_generated = TRUE;
                break;
              case GSBI_BUILT_IN_FATOMICMAX:
                wn = emit_builtin_atomic (exp, 2, MTYPE_F4, INTRN_F4ATOMICMAX);
                whirl_generated = TRUE;
                break;
              case GSBI_BUILT_IN_IATOMICEXCH:
                wn = emit_builtin_atomic (exp, 2, MTYPE_I4, INTRN_I4ATOMICEXCH);
                whirl_generated = TRUE;
                break;
              case GSBI_BUILT_IN_UATOMICEXCH:
                wn = emit_builtin_atomic (exp, 2, MTYPE_U4, INTRN_U4ATOMICEXCH);
                whirl_generated = TRUE;
                break;
              case GSBI_BUILT_IN_ULLATOMICEXCH:
                wn = emit_builtin_atomic (exp, 2, MTYPE_U8, INTRN_U8ATOMICEXCH);
                whirl_generated = TRUE;
                break;
              case GSBI_BUILT_IN_FATOMICEXCH:
                wn = emit_builtin_atomic (exp, 2, MTYPE_F4, INTRN_F4ATOMICEXCH);
                whirl_generated = TRUE;
                break;
              case GSBI_BUILT_IN_DATOMICEXCH:
                wn = emit_builtin_atomic (exp, 2, MTYPE_F8, INTRN_F8ATOMICEXCH);
                whirl_generated = TRUE;
                break;
              case GSBI_BUILT_IN_IATOMICAND:
              case GSBI_BUILT_IN_UATOMICAND:
                wn = emit_builtin_atomic (exp, 2, MTYPE_U4, INTRN_U4ATOMICAND);
                whirl_generated = TRUE;
                break;
              case GSBI_BUILT_IN_IATOMICOR:
              case GSBI_BUILT_IN_UATOMICOR:
                wn = emit_builtin_atomic (exp, 2, MTYPE_U4, INTRN_U4ATOMICOR);
                whirl_generated = TRUE;
                break;
              case GSBI_BUILT_IN_IATOMICXOR:
              case GSBI_BUILT_IN_UATOMICXOR:
                wn = emit_builtin_atomic (exp, 2, MTYPE_U4, INTRN_U4ATOMICXOR);
                whirl_generated = TRUE;
                break;
              case GSBI_BUILT_IN_UATOMICINC:
                wn = emit_builtin_atomic (exp, 2, MTYPE_U4, INTRN_U4ATOMICINC);
                whirl_generated = TRUE;
                break;
              case GSBI_BUILT_IN_UATOMICDEC:
                wn = emit_builtin_atomic (exp, 2, MTYPE_U4, INTRN_U4ATOMICDEC);
                whirl_generated = TRUE;
                break;
              case GSBI_BUILT_IN_IATOMICCAS:
                wn = emit_builtin_atomic (exp, 3, MTYPE_I4, INTRN_I4ATOMICCAS);
                whirl_generated = TRUE;
                break;
              case GSBI_BUILT_IN_UATOMICCAS:
                wn = emit_builtin_atomic (exp, 3, MTYPE_U4, INTRN_U4ATOMICCAS);
                whirl_generated = TRUE;
                break;
              case GSBI_BUILT_IN_ULLATOMICCAS:
                wn = emit_builtin_atomic (exp, 3, MTYPE_U8, INTRN_U8ATOMICCAS);
                whirl_generated = TRUE;
                break;
              case GSBI_BUILT_IN_FATOMICCAS:
                wn = emit_builtin_atomic (exp, 3, MTYPE_F4, INTRN_F4ATOMICCAS);
                whirl_generated = TRUE;
                break;
              case GSBI_BUILT_IN_DATOMICCAS:
                wn = emit_builtin_atomic (exp, 3, MTYPE_F8, INTRN_F8ATOMICCAS);
                whirl_generated = TRUE;
                break;

              case GSBI_BUILT_IN_ALL:
                wn = emit_builtin_vote (exp, INTRN_VOTEALL);
                whirl_generated = TRUE;
                break;
              case GSBI_BUILT_IN_ANY:
                wn = emit_builtin_vote (exp, INTRN_VOTEANY);
                whirl_generated = TRUE;
                break;
#ifdef FUTURE_SUPPORT  /* FERMI */
              case GSBI_BUILT_IN_BALLOT:
                wn = emit_builtin_vote (exp, INTRN_VOTEBALLOT);
                whirl_generated = TRUE;
                break;              
              case GSBI_BUILT_IN_SYNCTHREADS_COUNT:
                wn = emit_builtin_bar_red (exp, INTRN_BARREDPOPC);
                whirl_generated = TRUE;
                break; 
              case GSBI_BUILT_IN_SYNCTHREADS_AND:
                wn = emit_builtin_bar_red (exp, INTRN_BARREDAND);
                whirl_generated = TRUE;
                break; 
              case GSBI_BUILT_IN_SYNCTHREADS_OR:
                wn = emit_builtin_bar_red (exp, INTRN_BARREDOR);
                whirl_generated = TRUE;
                break; 
                
#endif
              case GSBI_BUILT_IN_PROF_TRIGGER:
                emit_builtin_pmevent (exp);
                whirl_generated = TRUE;
                break;

              case GSBI_BUILT_IN_PRAGMA_UNROLL:
                /* unroll is passed as a builtin that needs to get changed
                 * back to a pragma */
                arg1 = gs_tree_value (arglist);
                WGEN_Stmt_Append (
                  WN_CreatePragma (WN_PRAGMA_UNROLL, (ST*)NULL, 
                    gs_get_integer_value(arg1), 0),
                  Get_Srcpos());
                whirl_generated = TRUE;
                break;
#endif /* NVISA */
	  ...
	  else
	    {
	      ...
	      Fail_FmtAssertion ("Target-specific builtins NYI");
                ...
                char *name = gs_identifier_pointer(gs_decl_name(func));
...
#ifdef FUTURE_SUPPORT
                else if (strcmp(name, "__surf1Dreadc1") == 0) {
                        wn = emit_builtin_surfread (exp, 1, 1, MTYPE_U1,
                          "suld.b.1d.b8");
                        asm_generated = TRUE;
                }
                else if (strcmp(name, "__surf1Dreadc2") == 0) {
                        wn = emit_builtin_surfread (exp, 1, 2, MTYPE_U1,
                          "suld.b.1d.v2.b8");
                        asm_generated = TRUE;
                }
                else if (strcmp(name, "__surf1Dreadc4") == 0) {
                        wn = emit_builtin_surfread (exp, 1, 4, MTYPE_U1,
                          "suld.b.1d.v4.b8");
                        asm_generated = TRUE;
                }
                else if (strcmp(name, "__surf1Dreads1") == 0) {
                        wn = emit_builtin_surfread (exp, 1, 1, MTYPE_U2,
                          "suld.b.1d.b16");
                        asm_generated = TRUE;
                }
                else if (strcmp(name, "__surf1Dreads2") == 0) {
                        wn = emit_builtin_surfread (exp, 1, 2, MTYPE_U2,
                          "suld.b.1d.v2.b16");
                        asm_generated = TRUE;
                }
                else if (strcmp(name, "__surf1Dreads4") == 0) {
                        wn = emit_builtin_surfread (exp, 1, 4, MTYPE_U2,
                          "suld.b.1d.v4.b16");
                        asm_generated = TRUE;
                }
                else if (strcmp(name, "__surf1Dreadu1") == 0) {
                        wn = emit_builtin_surfread (exp, 1, 1, MTYPE_U4,
                          "suld.b.1d.b32");
                        asm_generated = TRUE;
                }
                else if (strcmp(name, "__surf1Dreadu2") == 0) {
                        wn = emit_builtin_surfread (exp, 1, 2, MTYPE_U4,
                          "suld.b.1d.v2.b32");
                        asm_generated = TRUE;
                }
                else if (strcmp(name, "__surf1Dreadu4") == 0) {
                        wn = emit_builtin_surfread (exp, 1, 4, MTYPE_U4,
                          "suld.b.1d.v4.b32");
                        asm_generated = TRUE;
                }
                else if (strcmp(name, "__surf1Dreadl1") == 0) {
                        wn = emit_builtin_surfread (exp, 1, 1, MTYPE_U8,
                          "suld.b.1d.b64");
                        asm_generated = TRUE;
                }
                else if (strcmp(name, "__surf1Dreadl2") == 0) {
                        wn = emit_builtin_surfread (exp, 1, 2, MTYPE_U8,
                          "suld.b.1d.v2.b64");
                        asm_generated = TRUE;
                }
                else if (strcmp(name, "__surf2Dreadc1") == 0) {
                        wn = emit_builtin_surfread (exp, 2, 1, MTYPE_U1,
                          "suld.b.2d.b8");
                        asm_generated = TRUE;
                }
                else if (strcmp(name, "__surf2Dreadc2") == 0) {
                        wn = emit_builtin_surfread (exp, 2, 2, MTYPE_U1,
                          "suld.b.2d.v2.b8");
                        asm_generated = TRUE;
                }
                else if (strcmp(name, "__surf2Dreadc4") == 0) {
                        wn = emit_builtin_surfread (exp, 2, 4, MTYPE_U1,
                          "suld.b.2d.v4.b8");
                        asm_generated = TRUE;
                }
                else if (strcmp(name, "__surf2Dreads1") == 0) {
                        wn = emit_builtin_surfread (exp, 2, 1, MTYPE_U2,
                          "suld.b.2d.b16");
                        asm_generated = TRUE;
                }
                else if (strcmp(name, "__surf2Dreads2") == 0) {
                        wn = emit_builtin_surfread (exp, 2, 2, MTYPE_U2,
                          "suld.b.2d.v2.b16");
                        asm_generated = TRUE;
                }
                else if (strcmp(name, "__surf2Dreads4") == 0) {
                        wn = emit_builtin_surfread (exp, 2, 4, MTYPE_U2,
                          "suld.b.2d.v4.b16");
                        asm_generated = TRUE;
                }
                else if (strcmp(name, "__surf2Dreadu1") == 0) {
                        wn = emit_builtin_surfread (exp, 2, 1, MTYPE_U4,
                          "suld.b.2d.b32");
                        asm_generated = TRUE;
                }
                else if (strcmp(name, "__surf2Dreadu2") == 0) {
                        wn = emit_builtin_surfread (exp, 2, 2, MTYPE_U4,
                          "suld.b.2d.v2.b32");
                        asm_generated = TRUE;
                }
                else if (strcmp(name, "__surf2Dreadu4") == 0) {
                        wn = emit_builtin_surfread (exp, 2, 4, MTYPE_U4,
                          "suld.b.2d.v4.b32");
                        asm_generated = TRUE;
                }
                else if (strcmp(name, "__surf2Dreadl1") == 0) {
                        wn = emit_builtin_surfread (exp, 2, 1, MTYPE_U8,
                          "suld.b.2d.b64");
                        asm_generated = TRUE;
                }
                else if (strcmp(name, "__surf2Dreadl2") == 0) {
                        wn = emit_builtin_surfread (exp, 2, 2, MTYPE_U8,
                          "suld.b.2d.v2.b64");
                        asm_generated = TRUE;
                }
                else if (strcmp(name, "__surf1Dwritec1") == 0) {
                        emit_builtin_surfwrite (exp, 1, 1, MTYPE_U1,
                          "sust.b.1d.b8");
                        asm_generated = TRUE;
                }
                else if (strcmp(name, "__surf1Dwritec2") == 0) {
                        emit_builtin_surfwrite (exp, 1, 2, MTYPE_U1,
                          "sust.b.1d.v2.b8");
                        asm_generated = TRUE;
                }
                else if (strcmp(name, "__surf1Dwritec4") == 0) {
                        emit_builtin_surfwrite (exp, 1, 4, MTYPE_U1,
                          "sust.b.1d.v4.b8");
                        asm_generated = TRUE;
                }
                else if (strcmp(name, "__surf1Dwrites1") == 0) {
                        emit_builtin_surfwrite (exp, 1, 1, MTYPE_U2,
                          "sust.b.1d.b16");
                        asm_generated = TRUE;
                }
                else if (strcmp(name, "__surf1Dwrites2") == 0) {
                        emit_builtin_surfwrite (exp, 1, 2, MTYPE_U2,
                          "sust.b.1d.v2.b16");
                        asm_generated = TRUE;
                }
                else if (strcmp(name, "__surf1Dwrites4") == 0) {
                        emit_builtin_surfwrite (exp, 1, 4, MTYPE_U2,
                          "sust.b.1d.v4.b16");
                        asm_generated = TRUE;
                }
                else if (strcmp(name, "__surf1Dwriteu1") == 0) {
                        emit_builtin_surfwrite (exp, 1, 1, MTYPE_U4,
                          "sust.b.1d.b32");
                        asm_generated = TRUE;
                }
                else if (strcmp(name, "__surf1Dwriteu2") == 0) {
                        emit_builtin_surfwrite (exp, 1, 2, MTYPE_U4,
                          "sust.b.1d.v2.b32");
                        asm_generated = TRUE;
                }
                else if (strcmp(name, "__surf1Dwriteu4") == 0) {
                        emit_builtin_surfwrite (exp, 1, 4, MTYPE_U4,
                          "sust.b.1d.v4.b32");
                        asm_generated = TRUE;
                }
               else if (strcmp(name, "__surf1Dwritel1") == 0) {
                       emit_builtin_surfwrite (exp, 1, 1, MTYPE_U8,
                         "sust.b.1d.b64");
                       asm_generated = TRUE;
               }
               else if (strcmp(name, "__surf1Dwritel2") == 0) {
                       emit_builtin_surfwrite (exp, 1, 2, MTYPE_U8,
                         "sust.b.1d.v2.b64");
                       asm_generated = TRUE;
               }
               else if (strcmp(name, "__surf2Dwritec1") == 0) {
                       emit_builtin_surfwrite (exp, 2, 1, MTYPE_U1,
                         "sust.b.2d.b8");
                       asm_generated = TRUE;
               }
               else if (strcmp(name, "__surf2Dwritec2") == 0) {
                       emit_builtin_surfwrite (exp, 2, 2, MTYPE_U1,
                         "sust.b.2d.v2.b8");
                       asm_generated = TRUE;
               }
               else if (strcmp(name, "__surf2Dwritec4") == 0) {
                       emit_builtin_surfwrite (exp, 2, 4, MTYPE_U1,
                         "sust.b.2d.v4.b8");
                       asm_generated = TRUE;
               }
               else if (strcmp(name, "__surf2Dwrites1") == 0) {
                       emit_builtin_surfwrite (exp, 2, 1, MTYPE_U2,
                         "sust.b.2d.b16");
                       asm_generated = TRUE;
               }
               else if (strcmp(name, "__surf2Dwrites2") == 0) {
                       emit_builtin_surfwrite (exp, 2, 2, MTYPE_U2,
                         "sust.b.2d.v2.b16");
                       asm_generated = TRUE;
               }
               else if (strcmp(name, "__surf2Dwrites4") == 0) {
                       emit_builtin_surfwrite (exp, 2, 4, MTYPE_U2,
                         "sust.b.2d.v4.b16");
                       asm_generated = TRUE;
               }
               else if (strcmp(name, "__surf2Dwriteu1") == 0) {
                       emit_builtin_surfwrite (exp, 2, 1, MTYPE_U4,
                         "sust.b.2d.b32");
                       asm_generated = TRUE;
               }
               else if (strcmp(name, "__surf2Dwriteu2") == 0) {
                       emit_builtin_surfwrite (exp, 2, 2, MTYPE_U4,
                         "sust.b.2d.v2.b32");
                       asm_generated = TRUE;
               }
                else if (strcmp(name, "__surf2Dwriteu4") == 0) {
                        emit_builtin_surfwrite (exp, 2, 4, MTYPE_U4,
                          "sust.b.2d.v4.b32");
                        asm_generated = TRUE;
                }
                else if (strcmp(name, "__surf2Dwritel1") == 0) {
                        emit_builtin_surfwrite (exp, 2, 1, MTYPE_U8,
                          "sust.b.2d.b64");
                        asm_generated = TRUE;
                }
                else if (strcmp(name, "__surf2Dwritel2") == 0) {
                        emit_builtin_surfwrite (exp, 2, 2, MTYPE_U8,
                          "sust.b.2d.v2.b64");
                        asm_generated = TRUE;
                }
#endif /* FUTURE_SUPPORT */
...
// individual structures
	    ...
	    ST *ret_st = Gen_Temp_Symbol(ret_ty_idx, 
		  st ? Index_To_Str(Save_Str2("__Mreturn__",
