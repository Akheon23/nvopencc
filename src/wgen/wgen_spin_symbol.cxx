...
Get_Name (gs_t node)
{
	...
	if (node == NULL) {
		++anon_num;
#ifdef TARG_NVISA
		sprintf(buf, "__anonymous__%d", anon_num);
#else
...
Is_Name_Vector_Name(char *name) {
  ...
  if (strcmp(name, "char1") == 0)
      ...
      return true;
  else if (strcmp(name, "longlong1") == 0)
      return true;
  else if (strcmp(name, "ulonglong1") == 0)
      return true;
  else if (strcmp(name, "longlong2") == 0)
      return true;
  else if (strcmp(name, "ulonglong2") == 0)
      return true;
  else if (strcmp(name, "double1") == 0)
      return true;
  else if (strcmp(name, "double2") == 0)
      return true;
  return false;
}
#endif //TARG_NVISA
...
Create_TY_For_Tree (gs_t type_tree, TY_IDX idx)
{
	...
	case GS_OFFSET_TYPE:
...
#ifdef KEY
		if (lookup_attribute((char*)"may_alias",gs_type_attributes(type_tree)))
		{
...
#endif
#ifdef TARG_NVISA
                // if typedef, look at basetype to find char/long
                if (gs_type_name(type_tree)
                  && gs_tree_code(gs_type_name(type_tree)) == GS_TYPE_DECL 
                  && gs_type_main_variant(type_tree) != type_tree) 
                {
		  idx = Get_TY (gs_type_main_variant(type_tree));
                ...
                // Gnu keeps the name "char" as the name for this case.
		else if (gs_type_name(type_tree)
                  && gs_decl_name(gs_type_name(type_tree)) 
                  && strcmp(Get_Name(gs_decl_name(gs_type_name(type_tree))), 
                            "char") == 0)
                {
                  // create a new type with this flag
                  TY &ty = New_TY (idx);
                  TY_Init (ty, tsize, KIND_SCALAR, mtype, Save_Str("char"));
                  Set_TY_is_character(ty);
                }
                // perform similar handling to distinguish "long"
                // from "int"
		else if (gs_type_name(type_tree)
                  && gs_decl_name(gs_type_name(type_tree)) 
                  && (strcmp(Get_Name(gs_decl_name(gs_type_name(type_tree))), 
                             ...
                             "long unsigned int") == 0))
                {
                  TY &ty = New_TY (idx);
                  TY_Init (ty, tsize, KIND_SCALAR, mtype,
                    Save_Str(Get_Name(gs_decl_name(gs_type_name(type_tree)))));
                  // Don't waste ty flag on this, will just use the name
		...
		*/
		...
		break;
	case GS_CHAR_TYPE:
                // why does "char" not go through here?
		...
		if (gs_tree_code(type_tree) == GS_UNION_TYPE) {
			Set_TY_is_union(idx);
                ...
                if (gs_tree_code(type_tree) == GS_RECORD_TYPE) {
                   char *name = Get_Name(gs_type_name(type_tree));
                   if (Is_Name_Vector_Name(name)) {
...
#ifdef TARG_NVISA
                  if (!TY_can_be_vector(idx)) // vectors returned in regs
#endif //TARG_NVISA
...
Create_ST_For_Tree (gs_t decl_node)
{
  ...
  if (gs_tree_code(decl_node) == GS_RESULT_DECL) {
    sprintf(tempname, "__result_decl_%d", gs_decl_uid(decl_node));
    name = tempname;
    anon_st = TRUE;
  }
#else
  if (gs_tree_code(decl_node) == GS_RESULT_DECL) {
...
#endif
#ifdef TARG_NVISA
  else if (gs_tree_code(decl_node) == GS_PARM_DECL) {
    name = (char*) alloca( // strlen ("__cudaparm_%s_%s")
    			strlen(ST_name(Get_Current_PU_ST())) +
			strlen (gs_identifier_pointer(gs_decl_name(decl_node))) 
			+ 12); 
    ...
    sprintf(name, "__cudaparm_%s_%s", ST_name(Get_Current_PU_ST()),
                 gs_identifier_pointer(gs_decl_name(decl_node)));
  }
#endif /* NVISA */
...
#ifdef TARG_NVISA
  // want to use user names of local variables
  else if (gs_tree_code(decl_node) == GS_VAR_DECL && gs_decl_name(decl_node)
    && gs_identifier_pointer(gs_decl_name(decl_node)))
  {
    name = (char *) gs_identifier_pointer (gs_decl_name (decl_node));
  }
...
#ifdef TARG_NVISA
            // handle either old input (static) or new input (extern)
            if (((sclass == SCLASS_EXTERN && eclass == EXPORT_PREEMPTIBLE)
              || (sclass == SCLASS_FSTATIC && eclass == EXPORT_LOCAL))
                && gs_tree_code(gs_tree_type(decl_node)) == GS_ARRAY_TYPE
                  && gs_decl_shared(decl_node)
                  && gs_type_size(gs_tree_type(decl_node)) == NULL)
            {
                  // HACK WARNING:
...
// individual structures
...
#ifdef TARG_NVISA
  if (gs_decl_global(decl_node)) {
    Set_ST_in_global_mem (st);
  }
  if (gs_decl_local(decl_node)) {
    Set_ST_in_local_mem (st);
  }
  if (gs_decl_shared(decl_node)) {
  ...
  if (gs_decl_constant(decl_node)) {
    Set_ST_in_constant_mem (st);
    Set_ST_is_const_var(st);
  }
  if (gs_decl_surface(decl_node)) {
    Set_ST_in_surface_mem (st);
  }
  if (gs_decl_texture(decl_node)) {
    Set_ST_in_texture_mem (st);
  }
  if (gs_tree_code(decl_node) == GS_FUNCTION_DECL &&
      gs_decl_thread_limit(decl_node) != 0 &&
      gs_decl_block_limit(decl_node) != 0 ) 
  {
    Set_PU_thread_limit (Pu_Table [ST_pu(st)], gs_decl_thread_limit(decl_node));
    Set_PU_block_limit (Pu_Table [ST_pu(st)], gs_decl_block_limit(decl_node));
  }
#endif /* NVISA */
