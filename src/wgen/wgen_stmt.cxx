...
#ifdef TARG_NVISA
        if (Debug_Level >=2 && WGEN_Block_has_locals (stmt))
        {


           if (lexical_block_level < 0 && Current_scope == 2) 
             lexical_block_level = 2;
           else
             lexical_block_level++;

           if (lexical_block_level>=3) {
             New_LABEL (CURRENT_SYMTAB, start_label_idx);
             New_LABEL (CURRENT_SYMTAB, end_label_idx);
             low_pc = WN_CreateLabel ( start_label_idx, 0, NULL);
             high_pc = WN_CreateLabel (end_label_idx, 0, NULL);

             WGEN_Stmt_Append (low_pc, Get_Srcpos());
             WGEN_Create_lexical_block(start_label_idx, end_label_idx);
           }
         }
#endif
	...
	Pop_Scope_And_Do_Cleanups ();
        ...
        if (Debug_Level >= 2 && WGEN_Block_has_locals (stmt))
        {
          if (lexical_block_level>=3) {
            ...
            WGEN_Stmt_Append (high_pc, Get_Srcpos());
            lexical_block_level--;
          }
        }
#endif
...
BOOL WGEN_Block_has_locals (gs_t stmt) {
    if (gs_tree_code(stmt) == GS_BIND_EXPR) {
	gs_t body = gs_bind_expr_body(stmt);
...
#ifdef FE_GNU_4_2_0
               for (int i=0; i < gs_length(stmt_list); i++)
                   if (gs_tree_code(gs_index(stmt_list, i)) == GS_DECL_EXPR)
                      return TRUE;
#else
	       gs_t list;
	       for (list = stmt_list; gs_code(list)!=EMPTY; list = gs_operand(list,1))
	          if (gs_tree_code(gs_operand(list,0)) == GS_DECL_EXPR)
                    return TRUE;
#endif
