...
#define MAX_NESTED_SCOPES    30 

static DST_INFO_IDX DST_get_context(gs_t intree);
static DST_INFO_IDX DST_get_context_special(gs_t intree);

DST_INFO_IDX dst_lexical_stack[MAX_NESTED_SCOPES+1];     
int lexical_block_level=-1;

void WGEN_Create_lexical_block(LABEL_IDX low_pc, LABEL_IDX high_pc ) 
{
   DST_INFO_IDX block_idx;
   DST_INFO_IDX parent_idx;
   ST_IDX  low, high;

   low = make_ST_IDX(low_pc, CURRENT_SYMTAB); 
   high = make_ST_IDX(high_pc, CURRENT_SYMTAB); 

   block_idx = DST_mk_lexical_block(NULL, low, high, DST_INVALID_IDX);

   if (lexical_block_level > MAX_NESTED_SCOPES) 
      Fail_FmtAssertion ("lexical_block_level should be smaller than MAX_NESTED_SCOPES");

   if (lexical_block_level >= 3) 
        parent_idx = dst_lexical_stack[lexical_block_level-1];  
   else
         Fail_FmtAssertion ("lexical_block_level should be >= 3");
   ...
   {        
         return;
          ...
          dst_lexical_stack[lexical_block_level] = block_idx; 
   }  
}
#endif
...
static DST_INFO_IDX 
DST_get_context_special (gs_t intree)
{
    if (lexical_block_level >= 3) 
       return dst_lexical_stack[lexical_block_level];
    else
       return DST_get_context  (intree);
}
...
#ifdef TARG_NVISA
      // for extern __shared__ variables, debugger needs to know the type info,
      // although such variable has no memory space allocated, 
      // and has no location info.
...
DST_Create_var(ST *var_st, gs_t decl)
{
	...
	gs_tree_code(gs_type_fields(gs_tree_type(decl))) != GS_TEMPLATE_DECL) {
      ...
      DST_INFO_IDX current_scope_idx =
#ifdef TARG_NVISA
	DST_get_context_special(gs_decl_context(decl));
#else
	DST_get_context(gs_decl_context(decl));
#endif
	  ...
	  char *copts[]) /* The array of option strings passed to fec(c) */
{
...
#ifdef __MINGW32__ /* no MAXHOSTNAMELEN in mingw */
                      (char *) malloc (strlen(cur_dir) + MAXPATHLEN + 10);
#else
   ...
   /* TARG_NVISA ignores the host.domain: part of the string so there is no point in generating it */
#ifndef TARG_NVISA
...
WGEN_Set_Line_And_File (UINT line, const char* f, bool check)
{
	...
	if (file_name == file) {
		// no path
#ifdef TARG_NVISA
                // If no path specified, we do not want an explicit cwd 
		dir = (char*)".";
#else
