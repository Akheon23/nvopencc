...
#ifdef TODO // requires passing multicore flag, may not be necessary in future
            // we inline all the functions for multicore right now
            if (flag_multicore) {
              Clear_PU_no_inline(ST_pu(func_st));
              Set_PU_must_inline(ST_pu(func_st));
              warning ("Noinline attribute is overridden under multicore mode for function '%s'", ST_name(func_st));
            }
#endif
...
#ifndef TARG_NVISA	// NVISA doesn't have zero-initialized section
...
WGEN_Generate_Temp_For_Initialized_Aggregate (gs_t init, const char * name)
{
  ...
  ST_Init (temp,
	Save_Str2 (name, Label_Name_Separator "init"),
...
#ifdef TARG_NVISA
  // don't do optimization if ret type is vector
  if (TY_can_be_vector(ret_ty_idx))
    return;
#endif
