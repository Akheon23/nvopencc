...
extern FILE *IR_open(char *filename)
{
   ...
   if (0 == fread(prefix_check,6,1,ir_file)) {}
   /* used to fool gcc 4.3.2 which does not accept the cast
    * (void)fread which is a bug in my opinion
    */
...
print_source (SRCPOS srcpos)
{
  ...
  if (USRCPOS_filenum(usrcpos) != cur_file_index) {
    ...
    if (cur_file->fileptr == NULL) {
      // show user's file number before invalidating it
  ...
  if (cur_file->max_line_printed < newmax) {
    for (i = cur_file->max_line_printed; i < newmax; i++) {
      if (NULL == fgets (text, sizeof(text), cur_file->fileptr)) {}
      /* used to please gcc 4.3.2 which does not accept the cast
       * (void)fgets which in my opinion is a bug
       */
    }
...
#ifdef TARG_NVISA
    if (OPCODE_is_load(opcode) && WN_Load_Uniform(wn)) {
	fprintf(ir_ofile, " # (uniform)");
    }
#endif // TARG_NVISA
...
/* Take the code to print pragma, to be used in some other places for printing
 * C-style code.
 */
void
print_pragma (WN *wn, FILE *fp)
{
  fprintf(fp, " # %s", WN_pragmas[WN_pragma(wn)].name);

#ifdef TARG_NVISA
  // Dump the routine name being inlined
  if (WN_pragma(wn) != 0
      && (strcmp(WN_pragmas[WN_pragma(wn)].name, "INLINE_BODY_START")==0
          || strcmp(WN_pragmas[WN_pragma(wn)].name, "INLINE_BODY_END") == 0)) {
    if (WN_st_idx(wn) != 0) {
      fprintf(fp, ", %s", ST_name(WN_st_idx(wn)));
    }
  }
#endif
  ...
  case WN_PRAGMA_DISTRIBUTE_RESHAPE:
    fprintf(fp, ", %d",WN_pragma_index(wn));
    ...
    case DISTRIBUTE_CYCLIC_CONST:
      fprintf(fp, ", CYCLIC(%d)", WN_pragma_preg(wn));
      break;
    case DISTRIBUTE_CYCLIC_EXPR:
      fprintf(fp, ", CYCLIC(expr)");
      break;
    }
    break;
  case WN_PRAGMA_ASM_CONSTRAINT:
    fprintf(fp, ", \"%s\", opnd:%d preg:%d",
  ...
  default:
    if (WN_pragma_arg2(wn) != 0 )
      fprintf(fp, ", %d, %d", 
            WN_pragma_arg1(wn), WN_pragma_arg2(wn));
    else if (WN_pragma_arg1(wn) != 0 ) {
      fprintf(fp, ", %d", WN_pragma_arg1(wn));
  }
  break;
  } /* switch */
  fprintf(fp, "\n");
...
// A call-back routine for print details when with_st
// is FALSE
//
static void (*print_ssa_var_info)(FILE *, WN *);

void
set_print_ssa_var_info (void (*new_print_ssa_var_info)(FILE *, WN *)) 
{
  print_ssa_var_info = new_print_ssa_var_info;
}

/* The following dump routines provided for dump 
 * a WN tree in C-style.
 */

/* Dump a ST in c-style. */
static void
ir_put_st_src (WN *wn, BOOL with_st, int offset)
{
  char *name;
  char *p;
  ...
  if (st_idx == (ST_IDX) 0) {
    /* labels may have empty st */
    fprintf(ir_ofile, "<NO_NAME>");

  } else if (!with_st) {
    /* Do not follow ST *, so that it can dump useful information
       even when ST * is not valid */
    if (print_ssa_var_info) {
      (*print_ssa_var_info)(ir_ofile, wn);
    } else {
      fprintf(ir_ofile, "aux_%d", (INT32) st_idx);
    }
  } else {
    const ST* st = &St_Table[st_idx];
    if (ST_class(st) == CLASS_CONST) {
      ...
      name = ST_name(st);
    fprintf (ir_ofile, "%s", name);
    if (offset != 0) {
      fprintf(ir_ofile, "_%d", offset);
    }
 ...
 */ 
static void 
ir_put_wn_src (WN * wn, INT indent)
{
...
// Check if extra braces are needed to honor the original 
// semantics during C-stype printing.
//
BOOL
Need_extra_brace (OPERATOR opr, OPERATOR opr2)
{
  if (opr2 == OPR_INTCONST || opr2 == OPR_IDNAME || opr2 == OPR_LDID
      || opr2 == OPR_ILOAD || opr2 == OPR_ILOADX || opr2 == OPR_MLOAD
      || opr2 == OPR_LDBITS || opr2 == OPR_ILDBITS || opr2 == OPR_CONST
      || opr2 == OPR_CVT || opr2 == OPR_TAS || opr2 == OPR_CVTL
      || opr2 == OPR_ABS || opr2 == OPR_SQRT || opr2 == OPR_RSQRT
      || opr2 == OPR_RECIP || opr2 == OPR_FIRSTPART || opr2 == OPR_SECONDPART
      || opr2 == OPR_PAREN || opr2 == OPR_RND || opr2 == OPR_TRUNC
      || opr2 == OPR_CEIL || opr2 == OPR_FLOOR || opr2 == OPR_LOWPART
      || opr2 == OPR_HIGHPART || opr2 == OPR_MINPART || opr2 == OPR_MAXPART
      || opr2 == OPR_EXTRACT_BITS || opr2 == OPR_PAIR || opr2 == OPR_DIVREM
      || opr2 == OPR_MAX || opr2 == OPR_MIN || opr2 == OPR_MINMAX
      || opr2 == OPR_ARRAY || opr2 == OPR_INTRINSIC_OP)
  {
    return FALSE;
  }
...
/* Collect some information about certain Operator */
const char * 
get_operator_info (OPERATOR opr, int *opnds, BOOL *is_prefix)
{
...
/*
 *  Write an expression and its children in source order
 */
void 
ir_put_expr_src(WN * wn, BOOL with_st)
{
  INT i;
  WN * wn2;
  ...
  op_name = get_operator_info (opr, &kid_count, &is_pre_fix_op); 
  if (op_name != NULL) {
    if (kid_count == 2) {
      if (is_pre_fix_op) {
        fprintf(ir_ofile, "%s", op_name);
        wn2 = WN_kid(wn, 0);
        ir_put_expr_src(wn2, with_st);
        fprintf(ir_ofile, ", ");
        wn2 = WN_kid(wn, 1);
        ir_put_expr_src(wn2, with_st);
        fprintf(ir_ofile, ")");
        ...
        wn2 = WN_kid(wn, 0);
        BOOL need_extra = Need_extra_brace (opr, WN_operator(wn2));
	...
	if (need_extra) {
	  fprintf(ir_ofile, "(");
        ...
        wn2 = WN_kid(wn, 1);
        need_extra = Need_extra_brace (opr, WN_operator(wn2));
        if (need_extra) {
          fprintf(ir_ofile, "(");
        }
        ir_put_expr_src(wn2, with_st);
  ...
  case OPR_IDNAME:
    ir_put_st_src (wn, with_st, WN_offset(wn));
    return;

  case OPR_SELECT:
  case OPR_CSELECT:
    fprintf(ir_ofile, "(");
    ir_put_expr_src(WN_kid(wn, 0), with_st);
    fprintf(ir_ofile, ") ? ");
    ir_put_expr_src(WN_kid(wn,1), with_st);
    fprintf(ir_ofile, " : ");
    ir_put_expr_src(WN_kid(wn,2), with_st);
    return;
  ...
  case OPR_CONST:
    fprintf (ir_ofile, "const_%d", WN_st_idx(wn));
    return;
  ...
  case OPR_ARRAY:
  {
    int ndim = WN_kid_count(wn)/2;
    int elem_size = WN_element_size(wn);
    WN *base = WN_kid(wn, 0);
    ir_put_expr_src(base, with_st);
    int i;
    for (i=1; i <= ndim; i++) {
      wn2 = WN_kid(wn, ndim+i);
      fprintf(ir_ofile, "[");
      ir_put_expr_src(wn2, with_st);
      fprintf(ir_ofile, "]");
    }
  ...
  case OPR_INTRINSIC_OP:
    fprintf(ir_ofile, "%s(", 
	  INTRINSIC_name((INTRINSIC) WN_intrinsic(wn)));
    for (i=0; i < WN_kid_count(wn); i++) {
      ir_put_expr_src(WN_kid(wn, i), with_st);
    }
    fprintf(ir_ofile, ")");
    return;
  ...
  case OPR_LDBITS:
    fprintf(ir_ofile, "ldbits(");
    ir_put_expr_src(WN_kid0(wn), with_st);
    fprintf(ir_ofile, ", %d, %d)", WN_bit_offset(wn), WN_bit_size(wn));
    return;

  case OPR_EXTRACT_BITS:
    fprintf(ir_ofile, "extract_bits(");
    ir_put_expr_src(WN_kid0(wn), with_st);
    fprintf(ir_ofile, ", %d, %d)", WN_bit_offset(wn), WN_bit_size(wn));
    return;
  ...
  case OPR_IO:
    break; // FIXME to be supported

  default:
    break;
  }

  fprintf (ir_ofile, "un-supported operator %s\n", 
           OPERATOR_name(WN_operator(wn)));
  ir_put_wn (wn, 0);
}

inline void
print_indent(int indent, FILE *fp)
{
 ...
 */
static void 
ir_put_stmt_src (WN * wn, INT indent, BOOL with_st)
{
  INT i, i2;
  WN * wn2;
  USRCPOS srcpos;
  ...
  if (wn) {
    ...
    if (USRCPOS_srcpos(srcpos) != 0 && 
        USRCPOS_srcpos(srcpos) != USRCPOS_srcpos(last_srcpos)) {
      last_srcpos = srcpos;
      print_indent(indent, ir_ofile);
      fprintf(ir_ofile, "# Line %d (file %d)\n", 
              USRCPOS_linenum(srcpos),
	      USRCPOS_filenum(srcpos));
    }
  }
  ...
  case OPC_BLOCK:
    wn2 = WN_first(wn);
    while (wn2) {
      if (OPCODE_is_stmt(WN_opcode(wn2)) || OPCODE_is_scf(WN_opcode(wn2))) {
        ir_put_stmt_src(wn2, indent, with_st);
      }
      wn2 = WN_next(wn2);
    }
    return;
  ...
  case OPC_WHILE_DO:
    print_indent(indent, ir_ofile);
    fprintf(ir_ofile, "while (");
    ir_put_expr_src (WN_kid(wn, 0), with_st);
    fprintf(ir_ofile, ")\n");
    print_indent(indent, ir_ofile);
    fprintf(ir_ofile, "{\n");
    ir_put_stmt_src(WN_kid(wn, 1), indent+1, with_st);
    print_indent(indent, ir_ofile);
    fprintf(ir_ofile, "}\n");
    fflush(ir_ofile);
    return;
  ...
  {
    int kid_count = WN_kid_count(wn);
    ST_IDX proc_name_idx = WN_st_idx(wn);
    fprintf (ir_ofile, "Procedure ");
    ir_put_st_src (wn, with_st, 0);
    fprintf (ir_ofile, "\n");

    for (i = 0; i < kid_count-3; i++) {
      WN *param_wn = WN_kid(wn, i);
      fprintf (ir_ofile, "  param: ");
      ST_IDX name_idx = WN_st_idx(param_wn);
      ir_put_st_src (param_wn, with_st, 0);
      fprintf (ir_ofile, "\n");
    }
    fflush(ir_ofile);
    // pragma for the routine
    ir_put_stmt_src (WN_kid(wn, kid_count-3), 1, with_st);
    // pragmas for call-site
    ir_put_stmt_src (WN_kid(wn, kid_count-2), 1, with_st);
    // function body 
    fprintf(ir_ofile, "{\n");
    ir_put_stmt_src (WN_kid(wn, kid_count-1), 1, with_st);
    fprintf(ir_ofile, "}\n");
    return;
  }
  ...
  case OPC_REGION:
    ir_put_marker("REGION EXITS", indent);
    ir_put_stmt_src(WN_region_exits(wn), indent+1, with_st);
    ir_put_marker("REGION PRAGMAS", indent);
    ir_put_stmt_src (WN_region_pragmas(wn), indent+1, with_st);

    ir_put_marker("REGION BODY", indent);
    ir_put_stmt_src(WN_region_body(wn), indent+1, with_st);
    return;
      
  case OPC_LABEL:
    fprintf(ir_ofile, "\nL%d:\n", WN_label_number(wn));
    if ( WN_label_loop_info(wn) != NULL ) {
      ir_put_stmt_src(WN_label_loop_info(wn), indent+1, with_st);
    }
    return;
  ...
  case OPC_IF:
    print_indent(indent, ir_ofile);
    fprintf(ir_ofile, "if (");
    ir_put_expr_src (WN_if_test(wn), with_st);
    fprintf(ir_ofile, ")\n");
    print_indent(indent, ir_ofile);
    fprintf(ir_ofile, "{\n");
    if (WN_then(wn)) {
     ir_put_stmt_src(WN_then(wn), indent+1, with_st);
    }
    print_indent(indent, ir_ofile);
    fprintf(ir_ofile, "}\n");

    if (WN_block_nonempty(WN_else(wn))) {
      print_indent(indent, ir_ofile);
      fprintf(ir_ofile, "else\n");
      print_indent(indent, ir_ofile);
      fprintf(ir_ofile, "{\n");
      ir_put_stmt_src(WN_else(wn), indent+1, with_st);
      print_indent(indent, ir_ofile);
      fprintf(ir_ofile, "}\n");
    }
    return;
  ...
  case OPC_DO_LOOP:
    print_indent(indent, ir_ofile);
    fprintf(ir_ofile, "for (");
    ir_put_stmt_src(WN_start(wn), 0, with_st);
    print_indent(indent, ir_ofile);
    ir_put_expr_src(WN_end(wn), with_st);
    fprintf(ir_ofile, "; ");
    ir_put_stmt_src(WN_step(wn), 0, with_st);
    print_indent(indent+1, ir_ofile);
    fprintf(ir_ofile, ")\n");
    print_indent(indent, ir_ofile);
    fprintf(ir_ofile, "{\n");
    /* optional loop_info */
    if ( WN_do_loop_info(wn) != NULL ) {
      ir_put_stmt_src(WN_do_loop_info(wn), indent+1, with_st);
    }
    ir_put_stmt_src(WN_do_body(wn), indent+1, with_st);
    print_indent(indent, ir_ofile);
    fprintf(ir_ofile, "}\n");
    return;
  ...
  case OPC_LOOP_INFO:
    ...
    if ( WN_loop_induction(wn) != NULL ) {
      fprintf(ir_ofile, "IV = ");
      ir_put_expr_src(WN_loop_induction(wn), with_st);
    }
    if ( WN_loop_trip(wn) != NULL ) {
      fprintf(ir_ofile, " TRIP = ");
      ir_put_expr_src(WN_loop_trip(wn), with_st);
    }
    fprintf(ir_ofile, "\n");
    fflush(ir_ofile);
    return;
  ...
  case OPC_COMPGOTO:
    ir_put_wn_src (wn, indent);
    ir_put_expr_src(WN_kid(wn,0), with_st);
    ir_put_stmt_src(WN_kid(wn,1), indent+1, with_st);
    if (WN_kid_count(wn) > 2) {
      ir_put_stmt_src(WN_kid(wn,2), indent+1, with_st);
    }
    ir_put_marker("END_COMPGOTO", indent);
    return;
  ...
  { 
    for (i = 0; i < WN_kid_count(wn); i++)
      ir_put_stmt_src(WN_kid(wn, i), indent+1, with_st);
    return;
  }
  ...
  case OPC_ASM_STMT:
    print_indent(indent, ir_ofile);
    ir_put_wn_src(wn, indent);
    ir_put_stmt_src(WN_kid(wn,0), indent+1, with_st);
    ir_put_stmt_src(WN_kid(wn,1), indent+1, with_st);

    for (i2 = 2; i2 < WN_kid_count(wn); i2++) {
      print_indent(indent+1, ir_ofile);
      ir_put_expr_src(WN_kid(wn,i2), with_st);
      fprintf(ir_ofile, "\n");
    }
    return;
  ...
  case OPC_BACKWARD_BARRIER:
    print_indent(indent, ir_ofile);
    if (opc == OPC_BACKWARD_BARRIER) {
      fprintf (ir_ofile, "BACKWARD-BARRIER\n");
    } else {
      fprintf (ir_ofile, "FORWARD-BARRIER\n");
    }
    for (i2=0; i2 < WN_kid_count(wn); i2++) {
      print_indent(indent+1, ir_ofile);
      ir_put_expr_src(WN_kid(wn, i2), with_st);
      fprintf(ir_ofile, "\n");
    }
  ...
  case OPR_FALSEBR:
    ...
    printf ("if (IS_FALSE(");
    ir_put_expr_src (WN_kid(wn, 0), with_st);
    fprintf (ir_ofile, ")) goto L%d\n\n", WN_label_number(wn));
    fflush(stdout);
    return;

  case OPR_TRUEBR:
    print_indent(indent, ir_ofile);
    fprintf (ir_ofile, "if (IS_TRUE(");
    ir_put_expr_src (WN_kid(wn, 0), with_st);
    fprintf (ir_ofile, ")) goto L%d\n\n", WN_label_number(wn));
    fflush(stdout);
    return;
  ...
  case OPR_OPT_CHI:
    print_indent(indent, ir_ofile);
    fprintf (ir_ofile, "CHI\n");
    return;

  case OPR_INTRINSIC_CALL:
    print_indent(indent, ir_ofile);
    fprintf(ir_ofile, "CALL %s (", 
        INTRINSIC_name((INTRINSIC) WN_intrinsic(wn)));
    for (i=0; i < WN_kid_count(wn); i++) {
      ir_put_expr_src(WN_kid(wn, i), with_st);
    }
...
void 
fdump_wn_src_no_st(FILE *fp, WN *wn)
{
  BOOL save_follow_st;
  FILE *save = ir_ofile;
  if (!is_initialized) IR_reader_init();
...
static void 
IR_put_func_src (WN * wn, FILE *f, BOOL with_st)
{
  FILE *save;
  if (f) {
    save = ir_ofile;
...
void 
fdump_tree_src (FILE *f, WN *wn)
{
  if (wn == NULL) {
    return;
  }
  if (WN_opcode(wn) == OPC_FUNC_ENTRY)
    IR_put_func_src (wn, f, 1);
  else if (OPCODE_is_stmt(WN_opcode(wn)) || OPCODE_is_scf(WN_opcode(wn))) {
    FILE *save = f;
    ir_ofile = f;
    ir_put_stmt_src(wn, 1, 1);
    fflush (f);
    ir_ofile = save;
  } else {
    FILE *save = f;
    ir_ofile = f;
    ir_put_expr_src(wn, 1);
    fprintf(f, "\n");
    fflush (f);
    ir_ofile = save;
  }
...
void 
fdump_tree_src_no_st( FILE *f, WN *wn )
{
  if (wn == NULL) {
    return;
  }
  if (WN_opcode(wn) == OPC_FUNC_ENTRY) {
    IR_put_func_src (wn, f, 0);
  }
  else if (OPCODE_is_stmt(WN_opcode(wn)) || OPCODE_is_scf(WN_opcode(wn))) {
    FILE *save = ir_ofile;
    ir_ofile = f;
    ir_put_stmt_src(wn, 1, 0);
    fflush (f);
    ir_ofile = save;
  } else {
    FILE *save = f;
    ir_ofile = f;
    ir_put_expr_src(wn, 0);
    fprintf(f, "\n");
    fflush (f);
    ir_ofile = save;
  }
...
/* dump a WN tree in C-style */
void 
dump_tree_src (WN *wn)
{
  fdump_tree_src (stdout, wn);
}

/* dump a WN tree with AUX_ID in C-style */
void 
dump_tree_src_no_st (WN *wn)
{
  fdump_tree_src_no_st (stdout, wn);
}
