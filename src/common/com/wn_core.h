...
#ifdef TARG_NVISA
/* we use one bit for uniform flag */
#define MAX_FIELD_ID 0x1ffe
#define UNKNOWN_FIELD_ID 0x1fff
#else
...
public:
   ...
   /* NB: We make all the bitfields [U]INT64 to ensure that Visual C++ lays them out contiguously.
          If any of the types are smaller, WN::common will actually get larger.  */

   struct {
      UINT64            wn_operator : 8;  /* 8 bits of operator       */
      UINT64            rtype       : 6;  /* result */
#ifdef TARG_NVISA
      /* use one bit for uniform flag; note that this reduces bit_size to 64;
       * that seems to be okay given that we don't have 128bit types. */
      UINT64            kid_count   :13; /* gives kid_count for free */
      UINT64            uniform_load:1;  /* iload is an uniform load */
#else
      UINT64            kid_count   :14; /* gives kid_count for free */
#endif
      INT64             map_id      :30;
      UINT64            desc        : 6;  /* descriptor type */
   } common;
...
inline OPERATOR   WN_operator (const WN* wn) { return (OPERATOR)wn->common.wn_operator; }
...
#ifdef TARG_NVISA
/* we steal a bit for load_uniform */
inline UINT	  WN_bit_size (const WN* wn) { return wn->common.kid_count & 0x3f; }
#else
...
#ifdef TARG_NVISA
/* we steal a bit for load_uniform */
#define WN_bit_size(x)		((x)->common.kid_count & 0x3f)
#else
...
WN_opcode (const WN *wn) {
   return OPCODE_make_op ((OPERATOR)wn->common.wn_operator,
			  wn->common.rtype,
			  wn->common.desc);
}
...
inline TY_IDX WN_ty(const WN *wn)
{
  OPERATOR opr;
  opr = WN_operator(wn);
  Is_True (OPERATOR_has_1ty(opr) || OPERATOR_has_2ty(opr), ("no ty on operator"));
...
#ifdef TARG_NVISA
/* use kid_count field to store uniform bit for LDID and ILOAD */
#define WN_LOAD_UNIFORM_FLAG      0x1
#define WN_load_flag(wn)          (wn)->common.uniform_load
#define WN_Load_Uniform(wn)       (WN_load_flag(wn) & WN_LOAD_UNIFORM_FLAG)
#define WN_Set_Load_Uniform(wn)   (WN_load_flag(wn) |= WN_LOAD_UNIFORM_FLAG)
#define WN_Reset_Load_Uniform(wn) (WN_load_flag(wn) &= ~WN_LOAD_UNIFORM_FLAG)
#endif /* TARG_NVISA */
