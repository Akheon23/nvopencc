...
inline ST_CLASS
ST_sym_class (const ST& s)		{ return (ST_CLASS)s.sym_class; }
inline void
Set_ST_sym_class (ST& s, ST_CLASS c)	{ s.sym_class = c; }

inline ST_SCLASS
ST_storage_class (const ST& s)		{ return (ST_SCLASS)s.storage_class; }
inline void
Set_ST_storage_class (ST& s, ST_SCLASS sc)	{ s.storage_class = sc; }

inline ST_SCLASS
ST_sclass (const ST& s)			{ return (ST_SCLASS)s.storage_class; }
inline void
Set_ST_sclass (ST& s, ST_SCLASS sc)	{ s.storage_class = sc; }

inline ST_EXPORT
ST_export (const ST& s)			{ return (ST_EXPORT)s.export_class; }
inline void
...
#ifdef TARG_NVISA
inline ST_MEMORY
ST_memory_space (const ST* s)	{ return (ST_MEMORY)s->memory_space; }
...
inline BOOL
ST_in_surface_mem (const ST* s)     { return (s->memory_space == MEMORY_SURFACE); }
inline void
Set_ST_in_surface_mem (ST *s)       { s->memory_space = MEMORY_SURFACE; }


inline INT32
ST_get_restrict_group_id (const ST *s) { return s->_restrict_group_id; }

inline void
ST_set_restrict_group_id (ST *s, INT32 gid) { s->_restrict_group_id = gid; }
...
inline void
Set_PU_thread_limit (PU& pu, UINT16 l)  { pu.thread_limit = l; }

inline UINT16
PU_block_limit(const PU& pu)		{ return pu.block_limit; }
inline void
Set_PU_block_limit (PU& pu, UINT16 l)   { pu.block_limit = l; }
#endif /* TARG_NVISA */
...
inline TY_KIND
TY_kind (const TY& ty)			        { return (TY_KIND)ty.kind; }
inline void
...
inline LABEL_KIND
LABEL_kind (const LABEL& lbl)		{ return (LABEL_KIND)lbl.kind; }
inline void
