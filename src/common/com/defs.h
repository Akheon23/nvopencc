 ...
 * $Source: /scratch/mee/Patch0002-taketwo/kpro64-pending/common/com/SCCS/s.defs.h $
 *
 * Revision history:
...
#ifdef _KEEP_RCS_ID
static char *defs_rcs_id = "$Source: /scratch/mee/Patch0002-taketwo/kpro64-pending/common/com/SCCS/s.defs.h $ $Revision: 1.6 $";
#endif /* _KEEP_RCS_ID */
...
#if (defined(_COMPILER_VERSION) && (_COMPILER_VERSION >= 400) && _SGIAPI) || defined(__GNUC__) || defined(_MSC_VER)
#define HOST_SUPPORTS_QUAD_FLOAT 1
#else
#define HOST_SUPPORTS_QUAD_FLOAT 0
#endif

/* Since GPUs do not (currently) support quad-precision floating point, we don't
   really care if the host compiler toolchain does.  */
#if HOST_SUPPORTS_QUAD_FLOAT && !defined(TARG_NVISA)
...
#ifdef _MSC_VER
  #define ALIGN_SPEC(N) __declspec(align(N))
#else
  #define ALIGN_SPEC(N) __attribute__((aligned(N)))
#endif
