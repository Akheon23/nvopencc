...
public:
    ...
    /* NB: We use mUINT8 (instead of the various enumerations) as the 
     * underlying type in order to get a tighter packing, and one more 
     * consistent across compiler toolchains.  */
    
    mUINT8 sym_class;			// class info
    mUINT8 storage_class;		// storage info
#ifdef TARG_NVISA
    mUINT8 export_class : 4;		// export class of the symbol
    mUINT8 memory_space: 4;		// memory space of the symbol
#else
    mUINT8 export_class;		// export class of the symbol
#endif
...
#ifdef TARG_NVISA
    //
    // the _restrict_group_id is used for propagating restrict attribute from
    // callee's formal parameters. 
    // The field is initialized to 0, and will be set during inlining.
    // Each inline instance will have unique group id within the caller.
    // The group id will be used in alias analysis to enforce the scoping of
    // restrict attribute.
    //
    INT32 _restrict_group_id;         
...
struct LABEL
{
    ...
    mUINT32    flags:24;
    mUINT32    kind:8;
    UINT32     _filler;  /* to ensure entire 16-byte struct is initialized/copied.  */

    // operations
    
    LABEL () {Fail_FmtAssertion("LABEL default constructor must not be called.");}

    LABEL (STR_IDX idx, LABEL_KIND k) : name_idx (idx), kind (k), _filler(0) {}
...
public:
    ...
    /* NB: The bitfields are share the underlying UINT32 type so that Microsoft C++ lays them
           out contiguously.  */
    UINT32 kind : 8;			// kind of type
    UINT32 mtype : 8;			// WHIRL data type
    UINT32 flags : 16;			// misc. attributes
...
struct FILE_INFO
{
    /* NB: We use UINT64 as the underlying type for all fields, to ensure
       that Visual C++ packs them all into 64 bits.  */
    UINT64 flags : 32;		// misc. attributes
    UINT64 gp_group : 8;	// gp group id
    UINT64 unused : 24;		// filler, must be zero
