   ...
   if (op == OPR_RECIP) {
	...
	break;
	 ...
	 break;
       ...
       case OPR_SQRT:
#ifndef TARG_NVISA  // defer to back end
	 ...
	 break;
      ...
      // It is not safe in targ nvisa because if the input is -0,
      // the expression sqrt(1/x) evaluates to NaN, while rsqrt evaluates to -Inf. 
...
#ifdef TARG_NVISA
        /* we don't want this conversion since WOPT cannot handle U8U2LDIDs */
        if (!(((opc == OPC_U8U4CVT) || (opc == OPC_I8I4CVT)) &&
              (MTYPE_byte_size(inter_type) == 4))) 
        {
#endif
...
#ifdef TARG_NVISA
            if (MTYPE_is_float(dest_type) && MTYPE_is_float(source_type)) {
              // something like F8F8TRUNC(F8F4CVT(a))
              // need to preserve the trunc which does rounding to int.
            }
            else
#endif
	    if (Is_Valid_Opcode_Parts(OPR_CVT,dest_type,source_type)) {
...
#if defined(TARG_X8664) || defined(TARG_NVISA)
      // bug 11400: if any operand is signed type, make SUB signed
		       ...
		       simpnode k0, simpnode k1, BOOL k0const, BOOL k1const)
{
   ...
   if (Enable_Cfold_Aggressive && Roundoff_Level >= ROUNDOFF_SIMPLE) {
	 ...
	 SIMP_DELETE_TREE(k0);
#if !defined(TARG_X8664) && !defined(TARG_NVISA) // RSQRT+MULTIPLY is faster than doing a SQRT on x86-64.
		      ...
		      simpnode k0, simpnode k1, BOOL k0const, BOOL k1const)
{
   ...
   if (k1const) {
      // any target can have int or str, so check for both
...
#if defined(TARG_NVISA)
   // Similar to binary case, if c0 is signed, result should be signed.

   // What does unsigned neg mean?  In other places we want neg result to be
   // negative (signed), so do that here too.
   if (OPCODE_operator(opc) == OPR_NEG && MTYPE_is_unsigned(OPCODE_rtype(opc)))
   {
     opc = OPC_FROM_OPR(OPR_NEG, MTYPE_complement(OPCODE_rtype(opc)));
   }
   // Can also have problem if doing U8U4CVT and c0 is negative, 
   // because that will then be treated as U8(U4(neg)) so won't sign-extend.
   if (OPCODE_operator(opc) == OPR_CVT && MTYPE_is_unsigned(OPCODE_desc(opc))
     && MTYPE_signed(TCON_ty(c0)))
   {
     opc = OPCODE_make_op(OPR_CVT, OPCODE_rtype(opc),
                                   MTYPE_complement(OPCODE_desc(opc)));
   }
#endif
...
#endif
#if defined(TARG_X8664) || defined(TARG_NVISA) 
        // bug 11830: if any operand is 64-bit, result should be
