  ...
  for (arg_pos = 0, arg_begin = arg_list; *arg_begin != 0; arg_pos++) {
    ...
    if (num_dims > 0) {
      ...
      *buf++ = '(';
      ...
      /* Skip element size */
      if (strtol(arg_begin, &arg_begin, 10)) {}
      /* if needed to fool gcc 4.3.2 because it does not accept
       * the (void)strol cast which is most likelly a bug
       */
  ...
  for (arg_pos = 0, arg_begin = arg_list; *arg_begin != 0; arg_pos++) {
    ...
    if (num_dims > 0) {
      ...
      *buf++ = '(';
      ...
      /* Skip element size */
      if (strtol(arg_begin, &arg_begin, 10)) {}
      /* if needed to fool gcc 4.3.2 because it does not 
       * accept the cast (void)strtol which is most likelly a bug
       */
