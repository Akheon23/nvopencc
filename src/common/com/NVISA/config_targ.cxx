...
Preconfigure_Target ( void )
{
  ...
  WOPT_Enable_Limited_Preopt_Copy_Prop = TRUE; // do limited cprop in pre_opt pass
  Olimit = 45000; // we need higher limit else too many stack variables
  WOPT_Enable_Folded_Scalar_Limit = 4000; // higher limit seems to be okay
  UseAlignedCopyForStructs = TRUE; // no support for unaligned
  Split_BB_Length = 5000; // allow higher limit
  return;
}
...
Prepare_Target ( void )
{
  ...
  */
  ...
  if ( ISA_Name != NULL ) {
    ...
    else if ( strcasecmp ( ISA_Name, "compute_20" ) == 0 ) {
      isa = TARGET_ISA_compute_20;
    } 
    else if ( strcasecmp ( ISA_Name, "compute_30" ) == 0 ) {
      isa = TARGET_ISA_compute_30;
    } 
    ...
    else if ( strcasecmp ( ISA_Name, "sm_20" ) == 0 ) {
      isa = TARGET_ISA_compute_20;
    } 
    else if ( strcasecmp ( ISA_Name, "sm_30" ) == 0 ) {
      isa = TARGET_ISA_compute_30;
    } 
    else {
