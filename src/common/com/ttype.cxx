 ...
 * $Source: /scratch/mee/Patch0002-taketwo/kpro64-pending/common/com/SCCS/s.ttype.cxx $
 *
 * Revision history:
...
Equivalent_Types (TY_IDX t1, TY_IDX t2, QUAL_CHECK consider_qualifiers)
{
    ...
    case KIND_STRUCT:
    {
	  ...
	  match_q = (TY_return_in_mem (t1) == TY_return_in_mem (t2) &&
		   ...
		   match_q);
#endif
      FLD_ITER fld_iter1 = Make_fld_iter (TY_fld (Ty_Table[t1]));
      FLD_ITER fld_iter2 = Make_fld_iter (TY_fld (Ty_Table[t2]));
      ...
      while(1) {
        ...
        TY_IDX fty_idx1 = FLD_type(fld1);
        TY_IDX fty_idx2 = FLD_type(fld2);

        match_q = match_q && (Equivalent_Types (fty_idx1, fty_idx2,
                                   consider_qualifiers));

        if (FLD_last_field (fld1) && FLD_last_field (fld2)) {
            return match_q;
        } else if (FLD_last_field (fld1) || FLD_last_field (fld2)) {
            return FALSE;
        } else if (!match_q) {
            return FALSE;
        } else {
