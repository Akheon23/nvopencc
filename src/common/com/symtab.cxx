...
// given a struct ty and a field id, return the ty for that field
// if not found, return struct ty.
TY_IDX
Get_Field_Type (TY_IDX struct_ty, UINT field_id)
{
  FmtAssert(Is_Structure_Type(struct_ty), ("not a struct"));
  if (field_id == 0)
    return struct_ty;

  UINT cur_field_id = 0;
  FLD_HANDLE fld = FLD_get_to_field(struct_ty, field_id, cur_field_id);
  if (!fld.Is_Null())
    return FLD_type(fld);
  else
    return struct_ty;
...
// Rather than pass in only f90 pointer flag, 
// pass in whole flag field so can distinguish between
// f90 ptr, global mem ptr, and whatever else we want.
// But Make_Pointer_Type will not pass in any flag
// so most flags will match.
...
// Normally find a specific field that matches offset;
// if offset is within an enclosing field, return mtype of enclosing field 
// and return non-zero sub-offset within that field.
TYPE_ID
Mtype_For_Type_Offset (TY_IDX ty, INT64 offset, UINT *suboffset, SRCPOS srcpos)
{
  /* Visual Studio 2005 crashes at -O2 when the fld_Iter declaration is 
   * inside the case block below... */
  FLD_ITER fld_iter;
  *suboffset = 0; // default for when we find the matching field
  FLD_HANDLE prev_fld;
  switch (TY_kind(ty)) {
  case KIND_STRUCT:
    {

      if (offset >= TY_size(ty)) {
        ErrMsgSrcpos(EC_Out_Of_Bound_Access_To_Structure, srcpos, TY_name(ty));
        return MTYPE_V; 
      }  
      ...
      do {
        ...
        else if (FLD_ofst(fld) > offset) {
          // went past the offset, need to return enclosing field.
          *suboffset = offset - FLD_ofst(prev_fld);
          return TY_mtype(FLD_type(prev_fld));
        }
        else {
          prev_fld = fld;
        }
      } while (!FLD_last_field(fld_iter++));
      if (offset > FLD_ofst(prev_fld) && offset < TY_size(ty)) {
        // return last (enclosing) field
        *suboffset = offset - FLD_ofst(prev_fld);
        return TY_mtype(FLD_type(prev_fld));
      }
  ...
  case KIND_ARRAY:
    ...
    return Mtype_For_Type_Offset (TY_etype(ty),
        offset % TY_size(TY_etype(ty)), suboffset, srcpos);
  default:
    *suboffset = offset; // usually 0, unless cast
    return TY_mtype(ty);
  }
}
...
ST::Print (FILE *f, BOOL verbose) const
{
	...
	if (memory_space == MEMORY_PARAM)
                fprintf (f, " __param__");
	if (memory_space == MEMORY_SURFACE)
                fprintf (f, " __surface__");
#endif /* TARG_NVISA */
...
{
    unsigned int pflags = (unsigned int)flags;
    fprintf (f, "gp_group: %d, flags: 0x%08x", (int)gp_group, pflags);
    if (flags) {
...
#ifdef TARG_NVISA
      // clear addr_saved from formals and pstatic too
      else if( (ST_sclass(*entry) == SCLASS_FORMAL) && ST_addr_saved(*entry))
        Clear_ST_addr_saved(*entry); 
      else if( (ST_sclass(*entry) == SCLASS_PSTATIC) && ST_addr_saved(*entry))
        Clear_ST_addr_saved(*entry); 
#endif
    } ;
}; // clear_addr_flag_op  
...
Initialize_Special_Global_Symbols ()
{
	...
	TYPE_ID mtype = TY_mtype (ty);
	if (MTYPE_To_TY (mtype) != 0)
	    continue; // ignore if ty is copy of earlier type, e.g. long int
