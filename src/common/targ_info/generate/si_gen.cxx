...
std::vector<char*> TOP_SCHED_INFO_MAP::top_sched_info_ptr_map(TOP_count,(char*)"0");
...
void TOP_SCHED_INFO_MAP::Create_Dummies( void )
{
  ...
  for ( int i = 0; i < TOP_count; ++i ) {
    if ( TOP_is_dummy((TOP)i) ) {
      if ( !dummies ) {
	dummies = new INSTRUCTION_GROUP((char*)"Dummy instructions");
...
#ifdef TARG_NVISA
	//No action now, since scheduling info is not used.
#else
