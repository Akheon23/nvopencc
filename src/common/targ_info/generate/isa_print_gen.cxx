...
typedef enum {
	...
	TYPE    = 2,                    // the insn type (.f32, etc.)
	OPND    = 3,                    // OPND+n => operand n
#else
	OPND    = 2,			// OPND+n => operand n
#endif  /* TARG_X8664 || TARG_NVISA */	
...
static const char * const interface[] = {
  ...
  " *",
#ifdef TARG_NVISA
  " *   const char *ISA_PRINT_AsmType(TOP topcode)",
  " *       Returns the assembly language type(s) associated with"
  " *       <topcode>.",
  " *",
#endif  /* TARG_NVISA */  
...
/////////////////////////////////////
{
  ...
  if (!initialized) {
	...
	comp_name[i] = "ISA_PRINT_COMP_type";
#endif  /* TARG_NVISA */
      } else if (i == OPND) {
...
/////////////////////////////////////
{
  ...
  current_print_desc->arg[current_print_desc->args] = TYPE;
  current_print_desc->args++;
}
#endif  /* TARG_NVISA */
...
//  See interface description.
/////////////////////////////////////
void Set_AsmType_Func(const char *(*asmtype_func)(TOP topcode))
{
  asmtype = asmtype_func;
}
#endif  /* TARG_NVISA */
...
#endif
#ifdef TARG_NVISA
	Print_Name(TYPE), TYPE, "Instruction type specifier",
#endif  /* TARG_NVISA */
...
#ifdef TARG_NVISA
  if (asmtype) {
    fprintf(cfile, "\nconst char * const ISA_PRINT_asmtype[] = {\n");
    for (top = 0; top < TOP_count; ++top) {
      fprintf(cfile, "  \"%s\",\n", asmtype((TOP)top));
    }
    fprintf(cfile, "  \"UNDEFINED\"\n"
    ...
    fprintf(hfile, "\ninline const char *ISA_PRINT_AsmType(TOP topcode)\n"
		   "{\n"
		   "  extern const char * const ISA_PRINT_asmtype[];\n"
		   "  return ISA_PRINT_asmtype[(INT)topcode];\n"
		   "}\n");

    fprintf(efile, "ISA_PRINT_asmtype\n");
  } else {
    fprintf(hfile, "\ninline const char *ISA_PRINT_AsmType(TOP topcode)\n"
		   "{\n"
		   "  return "";\n"
