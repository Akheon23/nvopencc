...
//  $Source: /scratch/mee/Patch0002-taketwo/kpro64-pending/common/targ_info/generate/SCCS/s.abi_properties_gen.cxx $
...
/////////////////////////////////////
{
...
#endif // ISA_REGISTER_MAX < NUM_REGISTERS_LIMIT
...
#if ISA_REGISTER_MAX >= NUM_REGISTERS_LIMIT
  //
  // Generate the new accessor for the properties
  //

  fprintf(hfile, "\nextern %s ABI_PROPERTIES_ABI_Get_Flags(\n", Type_Name(prop_count));
  fprintf(hfile, "  ISA_REGISTER_CLASS rc,\n");
  fprintf(hfile, "  INT reg);\n");

  fprintf(cfile, "\n%s ABI_PROPERTIES_ABI_Get_Flags(\n", Type_Name(prop_count));
  fprintf(cfile, "  ISA_REGISTER_CLASS rc,\n");
  fprintf(cfile, "  INT reg)\n");
  fprintf(cfile, "{\n");

  fprintf(cfile, "  switch (ABI_PROPERTIES_ABI_Value) {\n");
  ...
  for (abi_iter = abis.begin(); abi_iter != abis.end(); ++abi_iter) {
    ...
    fprintf(cfile,"    case ABI_PROPERTIES_ABI_%s:\n",abi->name);

    fprintf(cfile,"      switch(rc) {\n");    
    ...
    if (count != 0) {
      ...
      for (rc = 0; rc <= ISA_REGISTER_CLASS_MAX; ++rc) {
        ...
        for (reg = 0; reg <= ISA_REGISTER_MAX; ++reg) {
          ...
          // if register is not the last one check if mask changed to emit
          // an if condition on the range of registers with the current mask
          if (reg != ISA_REGISTER_MAX) {
            ...
            {
              ABI_PROPERTY_INFO api = *api_iter;
              if (api.rc == rc && (reg + 1) >= api.minreg && (reg + 1) <= api.maxreg) {
                ABI_PROPERTY prop = api.aprop;
                new_mask |= prop->val;
              }
            }
  ...
  fprintf(hfile, "\ninline const char *ABI_PROPERTY_Reg_Name(\n"
		 ...
		 "}\n");
#else // ISA_REGISTER_MAX < NUM_REGISTERS_LIMIT
  fprintf(hfile, "\nextern const char *ABI_PROPERTY_Get_Reg_Name(\n"
                 "  ISA_REGISTER_CLASS rc,\n"
                 "  INT reg);\n\n");

  fprintf(cfile, "\n#include \"targ_isa_registers.h\"\n\n");
  fprintf(cfile, "const char *ABI_PROPERTY_Get_Reg_Name(\n"
                 "  ISA_REGISTER_CLASS rc,\n"
                 "  INT reg)\n"
                 "{\n");
  fprintf(cfile, "\n  const ISA_REGISTER_CLASS_INFO *isa_reg_class_info;\n\n");
  fprintf(cfile, "  switch (ABI_PROPERTIES_ABI_Value) {\n");
  ...
  for (abi_iter = abis.begin(); abi_iter != abis.end(); ++abi_iter) {
    ...
    for (rc = 0; rc <= ISA_REGISTER_CLASS_MAX; ++rc) {
      ...
      fprintf(cfile,"             isa_reg_class_info = ISA_REGISTER_CLASS_Info(rc);\n");
      fprintf(cfile,"             return ISA_REGISTER_CLASS_INFO_Reg_Name(isa_reg_class_info, reg);\n");
      fprintf(cfile,"          }\n");
    }
    fprintf(cfile,"      }\n");
  }
  ...
  fprintf(hfile, "  INT reg)\n");
  fprintf(hfile, "{\n");
  fprintf(hfile, "  return ABI_PROPERTY_Get_Reg_Name(rc, reg);\n");
  fprintf(hfile, "}\n"); 
#endif // ISA_REGISTER_MAX < NUM_REGISTERS_LIMIT
	...
	fprintf(hfile, "  return FALSE;\n"
  	...
  	fprintf(hfile, "  extern const ABI_PROPERTIES *ABI_PROPERTIES_target_props;\n"
  		       "  return (  ABI_PROPERTIES_target_props->reg_flags[rc][reg]\n"
                       ...
                       prop->name);
#else // ISA_REGISTER_MAX < NUM_REGISTERS_LIMIT
	fprintf(hfile, "  return (  ABI_PROPERTIES_ABI_Get_Flags(rc, reg)\n"
		       "          & ABI_PROPERTY_%s) != 0;\n"
		       "}\n",
		       prop->name);
#endif // ISA_REGISTER_MAX < NUM_REGISTERS_LIMIT
