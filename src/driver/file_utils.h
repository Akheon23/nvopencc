...
extern boolean_t file_exists (char *path);

/* check whether file exists and has any executable bits set */
extern boolean_t is_executable (char *path);

/* check whether is a directory */
extern boolean_t is_directory (char *path);

/* check whether is a directory */
extern boolean_t want_directory (char *path);

/* check whether directory is writable */
extern boolean_t directory_is_writable (char *path);
...
/* Windows filename checking is a bit more involved... */
#ifdef _MSC_VER
/* NB: This should probably go back to Commieland... */
static __inline int is_dir_separator(char x) {
  char ds = x;
  return ds == '/' || ds == '\\';
}
static __inline int is_absolute_file_name(const char *x) {
  const char *fn = x; 
  return is_dir_separator(fn[0]) || fn[1] == ':';
}
#else
