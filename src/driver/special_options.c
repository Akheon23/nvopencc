...
set_defaults (void)
{
	...
	fmath_errno = 0; /* don't set errno when expanding math in fe */
#endif
}
...
add_special_options (void)
{
	...
	buffer_t buf;
	char *s;
	boolean_t undefined_olevel_flag = FALSE; 
	...
	remove_phase_for_option(O_MDtarget,P_f90_cpp);
        ...
        /* open64 group options get passed to gfec for version 3, 
         * wgen for version 4, but not to cc1 front end for version 4. */
        if (gnu_major_version == 4) {
	  remove_phase_for_option(O_OPT_,P_spin_cc1);
	  remove_phase_for_option(O_OPT_,P_spin_cc1plus);
	  add_phase_for_option(O_OPT_,P_wgen);
	  remove_phase_for_option(O_TARG_,P_spin_cc1);
	  remove_phase_for_option(O_TARG_,P_spin_cc1plus);
	  add_phase_for_option(O_TARG_,P_wgen);
	  remove_phase_for_option(O_TENV_,P_spin_cc1);
	  remove_phase_for_option(O_TENV_,P_spin_cc1plus);
	  add_phase_for_option(O_TENV_,P_wgen);
	  remove_phase_for_option(O_LANG_,P_spin_cc1);
	  remove_phase_for_option(O_LANG_,P_spin_cc1plus);
	  add_phase_for_option(O_LANG_,P_wgen);
	  remove_phase_for_option(O_DEBUG_,P_spin_cc1);
	  remove_phase_for_option(O_DEBUG_,P_spin_cc1plus);
	  add_phase_for_option(O_DEBUG_,P_wgen);
	  remove_phase_for_option(O_INTERNAL_,P_spin_cc1);
	  remove_phase_for_option(O_INTERNAL_,P_spin_cc1plus);
	  add_phase_for_option(O_INTERNAL_,P_wgen);
        }
	if (use_ftpp == TRUE) {
	...
	if (ipa == TRUE) {
		...
		if (option_was_seen(O_multicore)) {
		  prepend_option_seen(flag);
		  flag = add_string_option(O_PHASE_, "cmc");
		}
#else
