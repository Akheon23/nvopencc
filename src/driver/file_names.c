...
char *
construct_given_name (char *src, char *suffix, boolean_t keep)
{
	char *s;
...
{
	char *save_dir, *save_path = 0, *final_path;
	...
	if ((save_dir = getenv("OPEN64_PROBLEM_REPORT_DIR")) == NULL &&
		...
		if (-1 == asprintf(&save_dir, "%s/.open64-bugs", home)) {}
                /* added to please gcc 4.3.2 which does not accept the cast
                 * (void)asprintf which is a bug in my opinion
                 */
	}
	#endif
	...
	if (-1 == asprintf(&save_path, "%s/%s_error_XXXXXX", save_dir, program_name)) {}
        /* added to please gcc 4.3.2 which does not accept the cast
         * (void)asprintf which is a bug in my opinion
         */
...
no_report:	
	...
	if (-1 == asprintf(&final_path, "%s%s", save_path, suffix)) {}
        /* added to please gcc 4.3.2 which does not accept the cast
         * (void)asprintf which is a bug in my opinion
         */
...
cleanup (void)
{
	...
	int status;
        ...
        if (has_errors() && outfile != NULL && !keep_flag) {
          /* remove outfile so dependency still seen */
          mark_for_cleanup (outfile);
        }
