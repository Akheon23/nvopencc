...
boolean_t multiple_source_files = FALSE;

boolean_t keep_mp = FALSE;
boolean_t keep_list = FALSE;
boolean_t keep_cif = FALSE;
boolean_t keep_listing = FALSE;
boolean_t auto_parallelize = FALSE;
boolean_t Gen_feedback = FALSE;
boolean_t Use_feedback = FALSE;
boolean_t Disable_open_mp = FALSE;
boolean_t Disable_old_mp = FALSE;
boolean_t O3_flag = FALSE;
boolean_t use_cpp = FALSE;
#ifdef KEY
boolean_t expand_ftpp_macros = TRUE;	// bug 2258
#else
boolean_t expand_ftpp_macros = FALSE;
#endif
int     fortran_line_length = 72; /* Fortran line length */
char roundoff=0;
boolean_t nocpp_flag = FALSE;
...
static string_list_t *ipl_cmds = 0; /* record the user options that needed
				       to be passed to ipl */
static boolean_t string_md = FALSE;
static boolean_t string_mmd = FALSE;
extern string_list_t *feedback_files;

#if 0
extern boolean_t is_replacement_combo (int);
#endif
...
static void set_f90_source_form(string_list_t *args,boolean_t set_line_length) ;
#ifdef KEY
...
add_arg(string_list_t *args, const char *format, ...)
{
	...
	if (-1 == vasprintf(&arg, format, ap)) {}
        /* added to please gcc 4.3.2 which does not accept the cast
         * (void)vasprintf which is a bug in my opinion
         */
	add_string(args, arg);
	free(arg);
...
set_library_paths(string_list_t *args)
{
	...
	if (abi == ABI_N32) {
               if (-1 == asprintf(&our_path, "%s/" LIBPATH "/32", 
                         global_toolroot)) {}
               /* added to please gcc 4.3.2 which does not accept the cast
                * (void)vasprintf which is a bug in my opinion
                */
	} else {
               if (-1 == asprintf(&our_path, "%s/" LIBPATH, 
                                  global_toolroot)) {}
               /* added to please gcc 4.3.2 which does not accept the cast
                * (void)vasprintf which is a bug in my opinion
                */
...
boolean_t platform_is_64bit(void)
{
	static boolean_t _64bit_set;
	static boolean_t _64bit;
...
{
	static boolean_t native;
	static boolean_t native_set;
...
add_file_args (string_list_t *args, phases_t index)
{
	...
	case P_cplus_gfe:
		...
		if (option_was_seen(O_multicore))
		  add_string(args, "-fmulticore");
#endif
#ifdef TARG_NVISA
                if (index == P_spin_cc1plus) {
                        // we do not need exception handling for now
                        add_string(args, "-fno-exceptions");
                }
#endif //TARG_NVISA
		if (sse2 == TRUE)
		...
		if( abi == ABI_N32 ) {
		  add_string(args, "-m32");
		  add_string(args, "-malign-double");
                } else if (( abi == ABI_64 )&&( index == P_spin_cc1plus )) {
                  add_string(args, "-m64");
                  add_string(args, "-mno-align-double");
                }
#elif defined(TARG_MIPS)
		// endianness
	...
	case P_ipl:
        ...
        case P_ipa:
                /* input is list of object files, or single .o file from ipl */
                if (the_file != NULL) {
		  add_string(args, construct_name(the_file,"o"));
		  input_source = construct_name(the_file,"BI");
                } else {
	          append_objects_to_list (args);
                }
                /* output files should be suffixed with BI */
		add_string(args, "-osuffix,BI");
                break;
#endif /* NVISA */
	case P_be:
#if defined(TARG_NVISA)
...
#ifdef TARG_NVISA
                /* when go through ipl->ipa->be, input is .BI */
...
add_libgcc_s(string_list_t *args)
{
	...
	if (!path_set) {
		for (p = get_library_dirs()->head; p != NULL; p = p->next) {
			free(name);
			if (-1 == asprintf(&name, "%s/lib%s.so", p->name, 
                                           libgcc_s_std)) {}
                        /* added to please gcc 4.3.2 which does not accept 
                         * the cast (void)asprintf which is a bug in my opinion
                         */
...
{
#ifdef TARG_X8664 
        extern boolean_t link_with_mathlib;
...
#if defined(TARG_NVISA) /* nvisa hard-codes the front ends that are used */
	c_fe = P_c_gfe;
	cplus_fe = P_spin_cc1plus;
#else
	c_fe = (gnu_major_version == 4) ? P_spin_cc1 : P_c_gfe;
	cplus_fe = (gnu_major_version == 4) ? P_spin_cc1plus : P_cplus_gfe;
#endif
...
#ifdef TARG_NVISA
                case P_ipa:
                        /* this is simple case where only one be invocation */
			add_phase (next_phase);
			next_phase = be_phase;
			break;
#endif
...
init_phase_info (void)
{
	...
	if (old_ld_library_path) {
	  ...
	  if (-1 == asprintf(&old_ld_library_path, "%s;%s", old_ld_library_path,
...
phases_t
determine_ld_phase (boolean_t run_ipa) {
        phases_t ldphase;
        if (run_ipa) {
                ldphase = P_ipa_link;
        }
...
run_ld (void)
{
	...
	if (ipa == TRUE) {
        ...
        if (abi == ABI_N32) {
          if (-1 == asprintf(&our_path, "%s" LIBPATH "/32", root_prefix)) {}
          /* added to please gcc 4.3.2 which does not accept
           * the cast (void)asprintf which is a bug in my opinion
           */
        } else {
          if (-1 == asprintf(&our_path, "%s" LIBPATH, root_prefix)) {}
          /* added to please gcc 4.3.2 which does not accept
           * the cast (void)asprintf which is a bug in my opinion
           */
        }
...
run_compiler (void)
{
	int i;
	string_list_t *args;
	boolean_t inst_info_updated = FALSE;
	boolean_t cmd_line_updated = FALSE;
...
#ifdef __MINGW32__
                          /* no unsetenv on mingw, use putenv name= */
 ...
 */
static void
set_f90_source_form(string_list_t *args,boolean_t set_line_length)
{
   char buf[16];
