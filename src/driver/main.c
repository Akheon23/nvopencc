...
boolean_t print_help = FALSE;
char *help_pattern;
boolean_t debug;
boolean_t nostdinc = TRUE;
int show_version;
boolean_t show_copyright;
boolean_t dump_version;
boolean_t show_search_path;
boolean_t show_defaults;
...
static void display_version (boolean_t dump_version_only);

static string_list_t *files;
static string_list_t *file_suffixes;
string_list_t *feedback_files;

boolean_t parsing_default_options = FALSE;
boolean_t drop_option;

#ifdef TARG_X8664
boolean_t link_with_mathlib;
#endif
...
main (int argc, char *argv[])
{
	...
	if (show_copyright) {
	    fprintf(stderr, "Portions Copyright (c) 2005-2010 NVIDIA Corporation\n");
...
#ifdef TARG_NVISA
	if (multiple_source_files && ipa == TRUE) {
                /* run all files through ipl first */
		last_phase = P_ipl;
	} 
#endif /* NVISA */
...
read_compiler_defaults(FILE *f, string_list_t *default_options_list)
{
  ...
  int count = 0;
  int line = 0;
  boolean_t in_single_quotes = FALSE;
  boolean_t in_double_quotes = FALSE;
  boolean_t follows_escape_char = FALSE;
  boolean_t in_comment = FALSE;
  ...
  while (fgets(buf, max_len, f) != NULL) {
    boolean_t end_of_line = FALSE;
    boolean_t continuation = FALSE;	// '\' at end of line
    line++;

    // Parse one line.
    for (p = buf; *p != '\0' && !end_of_line; p++) {
      boolean_t add_new_option = FALSE;
...
append_default_options (int *argc, char *(*argv[]))
{
  ...
  if (compiler_defaults_path == NULL) {
    ...
    if (exe_dir) {
      if (-1 == asprintf(&compiler_defaults_path, "%.*s/etc",
	        (int)(strlen(exe_dir) - 4), exe_dir)) {}
      /* used because gcc 4.3.2 does not accept the cast
...
print_search_path ()
{
	...
	if (abi == ABI_N32) {
	#ifdef PSC_TO_OPEN64
		if (-1 == asprintf(&our_path, "%s/lib/" OPEN64_FULL_VERSION "/32",
			 root_prefix)) {}
                /* used to please gcc 4.3.2 which does not accept the cast
                 * (void)asprintf which in my opinion is a bug
                 */
	} else {
		if (-1 == asprintf(&our_path, "%s/lib/" OPEN64_FULL_VERSION, root_prefix)) {}
                /* used to please gcc 4.3.2 which does not accept the cast
                 * (void)asprintf which in my opinion is a bug
                 */
...
get_gcc_version(int *v, int nv)
{
	...
	if (version[0] == '\0') {
#ifdef __MINGW32__
#ifdef _MSC_VER
                sprintf(version, "Microsoft C++ %.2f", _MSC_VER / 100.0f);
#else
		...
		char *c;
		if (fread(version, 1, sizeof(version) - 1, fp)) {}
                /* used to please gcc 4.3.2 which does not accept the cast
                 * (void)asprintf which in my opinion is a bug
                 */
		pclose(fp);
...
static void
display_version (boolean_t dump_version_only)
{
  int gcc_version;
