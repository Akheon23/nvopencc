...
static lang_info_t language_info[] = {
	...
	{'C',	0x00000004,	{"CC", "openCC", OPEN64_NAME_PREFIX "CC", OPEN64_NAME_PREFIX "c++"}},	/* c++ */
#else
...
typedef struct phase_struct {
	...
	char *name;
	char *dir;
	boolean_t set_ld_library_path; /* true if need to set LD_LIBRARY_PATH
					to "dir" */
	boolean_t find_dir_by_path; /* find phase dir from $PATH */
} phase_info_t;
...
static phase_info_t phase_info[] = {
   ...
   {'i',  0x0000000030000000LL,	"",	"",		TRUE, FALSE},	/* ipl, inline*/
#if defined(TARG_NVISA)
   {'P',  0x0000000040000000LL,	"ipa",	PHASEPATH,	TRUE, FALSE},	/* ipa */
#endif
...
get_phases_real_path (void) {
       ...
       for (i = P_NONE; i < P_LAST; i++) {
               ...
               p = realpath (phase_info[i].dir, pathstr);
