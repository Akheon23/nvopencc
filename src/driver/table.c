...
typedef struct opt_info_rec {
	...
	boolean_t toggle;		/* has toggle action */
	boolean_t internal;	/* not available to user */
	mask_t languages;
	mask_t phases;
 ...
 */
static boolean_t table_for_phase = FALSE;
...
static void read_table(void) 
{
	...
	i++;
	    ...
	    if (EMPTY(line1)) 
              ...
              continue;
	    else if (line1[0] == ' ' && line1[1] == NIL)
              continue;
	    else if (line1[0] == '\t')
                internal_error("unexpected line (help msg?): %s", line1);
	    else if (line1[0] != '-' && line1[0] != 'I')
                internal_error("unexpected line: [%s]", line1);

	    if (get_line(line2) == EOF)
                internal_error("unexpected EOF for line2");

            s = malloc(strlen(line1) + strlen(line2) + 4);
            if (s == NULL)
              internal_error("memory allocation failed");
	    sprintf(s, "%s %c %s", line1, FIELD_DIVIDER, line2);
