...
boolean_t show_flag = FALSE;
boolean_t show_but_not_run = FALSE;
boolean_t execute_flag = TRUE;
boolean_t time_flag = FALSE;
boolean_t prelink_flag = TRUE;
boolean_t quiet_flag = TRUE;
boolean_t run_m4 = FALSE;
static boolean_t ran_twice = FALSE;
...
get_binutils_lib_path(void)
{
	...
	if (asprintf(&my_path, "%s/%s", get_executable_dir(), 
		binutils_library_path) == -1) 
	{
            internal_error("cannot allocate memory");
            do_exit(RC_SYSTEM_ERROR);
        }
...
run_phase (phases_t phase, char *name, string_list_t *args)
{
	...
	const boolean_t uses_message_system = 
	...
	if (forkpid == 0) {
		...
		if (rld_path != 0) {
		    if (asprintf(&my_path, "%s:%s", my_path, rld_path) == -1) 
		    {
                        internal_error("cannot allocate memory");
                        do_exit(RC_SYSTEM_ERROR);
                    }
		...
		if (ld_library_path) {
                    if (asprintf(&l_path, "%s:%s", my_path, ld_library_path) == -1) 
		    {
                        internal_error("cannot allocate memory");
                        do_exit(RC_SYSTEM_ERROR);
                    }
		...
		if (ld_libraryn32_path) {
                    if (asprintf(&l32_path, "%s:%s", my_path, ld_libraryn32_path) == -1) 
		    {
                        internal_error("cannot allocate memory");
                        do_exit(RC_SYSTEM_ERROR);
                    }
