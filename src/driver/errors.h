...
extern boolean_t print_warnings;		/* whether to print warning msgs */
extern boolean_t fullwarn;		/* whether to print all warnings */
extern boolean_t pass_exit_codes;         /* If true, remember the highest
					 * exit code. */

#ifdef _MSC_VER
#define FORMAT_SPEC(M, N)
#else
#define FORMAT_SPEC(M, N)  __attribute__((format (printf, (M), (N))))
#endif
...
extern boolean_t has_errors (void);
/* has_current_errors returns true if were any errors in current compile */
extern boolean_t has_current_errors (void);
