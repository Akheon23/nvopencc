...
Expand_OP (OPCODE opcode, TN *result, TN *op1, TN *op2, TN *op3, VARIANT variant, OPS *ops)
{
  ...
  case OPR_LT:
#ifdef TARG_NVISA
	Expand_Less (result, op1, op2, variant, rtype, desc, ops);
#else
  ...
  case OPR_LE:
#ifdef TARG_NVISA
	Expand_Less_Equal (result, op1, op2, variant, rtype, desc, ops);
#else
	if (MTYPE_is_float(desc))
  ...
  case OPR_EQ:
#ifdef TARG_NVISA
	Expand_Equal (result, op1, op2, variant, rtype, desc, ops);
#else
	if (MTYPE_is_float(desc))
  ...
  case OPR_NE:
#ifdef TARG_NVISA
	Expand_Not_Equal (result, op1, op2, variant, rtype, desc, ops);
#else
	if (MTYPE_is_float(desc))
  ...
  case OPR_GE:
#ifdef TARG_NVISA
	Expand_Greater_Equal (result, op1, op2, variant, rtype, desc, ops);
#else
	if (MTYPE_is_float(desc))
  ...
  case OPR_GT:
#ifdef TARG_NVISA
	Expand_Greater (result, op1, op2, variant, rtype, desc, ops);
#else
	if (MTYPE_is_float(desc))
  ...
  case OPR_CVT:
	...
	if (MTYPE_is_float(rtype) && MTYPE_is_float(desc)) {
#if defined(TARG_X8664) || defined(TARG_SL) || defined(TARG_MIPS) || defined(TARG_NVISA)
		...
		// desc is bool
#ifdef TARG_NVISA
		Expand_Bool_To_Int (result, op1, variant, rtype, ops);
#else
  ...
  case OPR_RND:
#ifdef TARG_NVISA 
	// use common routine cause can have float->float too
	Expand_Round (result, op1, rtype, desc, ops);
#else
  ...
  case OPR_TRUNC:
#ifdef TARG_NVISA 
	Expand_Trunc (result, op1, rtype, desc, ops);
#else
  ...
  case OPR_CEIL:
#ifdef TARG_NVISA 
	Expand_Ceil (result, op1, rtype, desc, ops);
#else
  ...
  case OPR_FLOOR:
#ifdef TARG_NVISA 
	Expand_Floor (result, op1, rtype, desc, ops);
#else
#ifdef TARG_X8664
  ...
  case OPR_SELECT:
#ifdef TARG_NVISA
	Expand_Select (result, op1, op2, op3, rtype, variant, ops);
#else
  ...
  case OPR_NMSUB:
#ifdef TARG_NVISA
        Expand_Madd (opcode, result, op1, op2, op3, ops);
        break;
#else
	// fallthru
#endif
  case OPR_RECIP:
#ifdef TARG_NVISA
	Expand_Recip (result, op1, rtype, ops);
	break;
#else
	// fallthru
#endif
  case OPR_RSQRT:
#ifdef TARG_NVISA
	Expand_Recip_Sqrt (result, op1, rtype, ops);
	break;
#else
	// fallthru
#endif
