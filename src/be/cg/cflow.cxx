...
Redundant_Logif(BB *pred, BB *succ, BOOL *pnegated)
{
  ...
  if (pred_cmp != succ_cmp) {
    ...
    // same comparison op, but the branches may be predicated and negated
    // versions of each other
    OP *pred_br = BB_branch_op(pred);
    OP *succ_br = BB_branch_op(succ);
    if (pred_br && succ_br) {
      if (OP_code(pred_br) == TOP_bra_p && OP_code(succ_br) == TOP_bra_np)
        negated = TRUE;
      else if (OP_code(pred_br) == TOP_bra_np && OP_code(succ_br) == TOP_bra_p)
        negated = TRUE;
    }
  }
#endif
