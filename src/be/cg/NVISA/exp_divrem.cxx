				  ...
				  TYPE_ID mtype, OPS *ops)
{
  ...
  if (is_signed)
  {
      ...
      TN *p1;
      TOP opc = (TOP) (TOP_setp_cmp_s8 + Mtype_Index(mtype));
      ...
      if (mtype == MTYPE_F4) {
        Build_OP (opc, p1, Gen_Enum_TN (ECV_cmp_lt),
                  Gen_Default_Enum_TN_ftz (),
                  numer_tn, zero_tn, ops);
      } else {
        Build_OP (opc, p1, Gen_Enum_TN (ECV_cmp_lt), numer_tn, zero_tn, ops);
      }  
...
Expand_Divide (TN *result, TN *src1, TN *src2, TYPE_ID mtype, OPS *ops)
{
  ...
  if (TN_Can_Use_Constant_Value (src2, mtype, &val)) {
      // earlier it will have done 32bit constants.
      // convert powers of 2 to shifts 
      if (Is_Power_Of_2( val, mtype)) {
        Expand_Power_Of_2_Divide (result, src1, val, mtype, ops);
        return;
      }
      else {
  ...
  if (opc == TOP_div_fround_ftz_f32) {
    Build_OP (opc, result,
              Gen_Default_Enum_TN_prec_div (),
              Gen_Default_Enum_TN_ftz (),
              src1, src2, ops);
  } else if (opc == TOP_div_f64) {
    Build_OP (TOP_div_rn_f64, result, src1, src2, ops);
  } else {
...
Expand_Mod (TN *result, TN *src1, TN *src2, TYPE_ID mtype, OPS *ops)
{
  ...
  {
      if (src2_val == 0) 
         ErrMsgSrcpos(EC_Ill_Modulus, current_srcpos);   /* Warn if modulus against 0 */
      else
      {
