...
Tid_TN (INT i)
{
  ...
  // return either 16bit or 32bit tid
  if (Target_ISA >= TARGET_ISA_compute_20)
    ...
    return Build_Dedicated_TN(ABI_PROPERTY_tid1_Register_Class, 
	ABI_PROPERTY_tid1_First_Register + i + REGISTER_MIN, 0);
}
...
Ntid_TN (INT i)
{
  ...
  if (Target_ISA >= TARGET_ISA_compute_20)
    ...
    return Build_Dedicated_TN(ABI_PROPERTY_ntid1_Register_Class, 
	ABI_PROPERTY_ntid1_First_Register + i + REGISTER_MIN, 0);
}
...
Ctaid_TN (INT i)
{
  ...
  if (Target_ISA >= TARGET_ISA_compute_20)
    ...
    return Build_Dedicated_TN(ABI_PROPERTY_ctaid1_Register_Class, 
	ABI_PROPERTY_ctaid1_First_Register + i + REGISTER_MIN, 0);
}
...
Nctaid_TN (INT i)
{
  ...
  if (Target_ISA >= TARGET_ISA_compute_20)
    ...
    return Build_Dedicated_TN(ABI_PROPERTY_nctaid1_Register_Class, 
	ABI_PROPERTY_nctaid1_First_Register + i + REGISTER_MIN, 0);
}
...
Clock64_TN (void)
{
  return Build_Dedicated_TN(ABI_PROPERTY_clock64_Register_Class, 
	ABI_PROPERTY_clock64_Register + REGISTER_MIN, 0);
}

inline TN*
PM_TN (INT i)
{
  FmtAssert((i >= 0 && i <= 3), ("Ntid_TN must be 0,1,2,3"));
  return Build_Dedicated_TN(ABI_PROPERTY_pm_Register_Class, 
    ABI_PROPERTY_pm_First_Register + i + REGISTER_MIN, 0);
}

inline TYPE_ID
Mtype_Of_RegClass (ISA_REGISTER_CLASS rc)
{
  // since regs are unsigned, use unsigned types
  switch (rc) {
  case ISA_REGISTER_CLASS_integer16: return MTYPE_U2;
