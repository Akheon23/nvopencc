...
static INT64 Find_Const_Offset (WN *expr);
static WN *Find_Initializer (ST *st, INT64 offset);
static ST *Find_Const_Initializer (ST *st, INT64 offset);
 ...
 */
{
  ...
  if (TOP_is_memory_8bit(topcode)) return 1;
  else if (TOP_is_memory_16bit(topcode)) return 2;
  else if (TOP_is_memory_32bit(topcode)) return 4;
  else if (TOP_is_memory_64bit(topcode)) return 8;
  ...
  OP **compare_op)
{
  ...
  *tn1 = OP_opnd(*compare_op, 0);
  ISA_ENUM_CLASS_VALUE CmpOp = TN_is_enum(*tn1)? TN_enum(*tn1): ECV_UNDEFINED;
  mTOP opcode = OP_code(*compare_op);
  INT arg_index = 0;
  
  switch (CmpOp) {  
    case ECV_cmp_eq:
      switch (opcode) {
        case TOP_setp_cmp_s16:
        case TOP_setp_cmp_s32:
		variant = V_BR_I4EQ;
		break;
        case TOP_setp_cmp_s64:
		variant = V_BR_I8EQ;
		break;
        case TOP_setp_cmp_u16:
        case TOP_setp_cmp_u32:
		variant = V_BR_U4EQ;
		break;
        case TOP_setp_cmp_u64:
		variant = V_BR_U8EQ;
		break;
        case TOP_setp_cmp_ftz_f32:
		variant = V_BR_FEQ;
		++arg_index;
		break;
        case TOP_setp_cmp_f64:
		variant = V_BR_DEQ;
		break;
	default:
	        goto invalid_compare_op;
      }
      ++arg_index;
      break;
    case ECV_cmp_ne:
  ...
  *tn1 = OP_opnd(*compare_op, arg_index + 0);
  *tn2 = OP_opnd(*compare_op, arg_index + 1);
...
static const WN* asm_wn_stmt;

// must be called once per asm
void
CGTARG_Init_Asm_Constraints (const WN *asm_wn)
{
...
// look through intermediates for texture tn that matches temp_st;
// returns tn in ret_tn.
static BOOL
Look_For_Texture_TN (OP *op, ST **temp_st, TN **ret_tn)
{
  if (OP_store(op)) {
    TN *sym_tn = OP_opnd(op, OP_find_opnd_use(op,OU_base));
    if (TN_is_symbol(sym_tn) && TN_var(sym_tn) == *temp_st) {
      *ret_tn = OP_opnd(op, OP_find_opnd_use(op,OU_storeval));
      if ((TN_in_texture_mem(*ret_tn) || TN_in_surface_mem(*ret_tn)) 
        && TN_home(*ret_tn) != NULL) 
      {
        ...
        *ret_tn = Gen_Symbol_TN (WN_st(TN_home(*ret_tn)), 0, 0);
        ...
        // might have sequence of load/stores and need to trace
        // back to original tn.
        DevWarn("found store that wasn't texture, so keep looking");
      }
    }
  }
  else if (OP_load(op)) {
    if (OP_result(op,0) == *ret_tn) {
      TN *sym_tn = OP_opnd(op, OP_find_opnd_use(op,OU_base));
      if (ST_is_temp_var(TN_var(sym_tn))
       // __T and __cuda are cudafe-generated temps
       || strncmp(ST_name(TN_var(sym_tn)), "__cuda___", 9) == 0
       || strncmp(ST_name(TN_var(sym_tn)), "__T", 3) == 0)
      {
        *temp_st = TN_var(sym_tn);
        DevWarn("now look for %s", ST_name(*temp_st));
      }
    }
  }
  else if (OP_copy(op)) {
    TN *tmp = OP_result(op,0);
    if ((TN_in_texture_mem(tmp) || TN_in_surface_mem(tmp)) 
        && TN_home(tmp) != NULL) 
    {
      DevWarn("copy of texture ret_tn");
      *ret_tn = Gen_Symbol_TN (WN_st(TN_home(tmp)), 0, 0);
      return TRUE;
    }
  }
  return FALSE;
...
// -----------------------------------------------------------------------
// Given a WHIRL expression, return the constant offset value that it
// computes, or -1 if not constant (or we don't know how to traverse it.)
// -----------------------------------------------------------------------
static INT64
Find_Const_Offset (WN *expr) {
  INT64 v1, v2;
  FmtAssert(expr, ("no tree in Find_Const_Offset"));
  
  switch (WN_operator (expr)) {
    case OPR_INTCONST:
      return WN_const_val (expr);
    case OPR_MPY:
      v1 = Find_Const_Offset (WN_kid0 (expr));
      v2 = Find_Const_Offset (WN_kid1 (expr));
      return (v1 >= 0 && v2 >= 0? v1 * v2: -1);
    case OPR_LDID:
      return Find_Const_Offset (Find_Initializer (WN_st (expr), WN_offset (expr)));
    case OPR_CVT:
      // TODO:  should cast the value, but probably not necessary in these cases
      return Find_Const_Offset(WN_kid0(expr));
    default:
      DevWarn("unexpected tree in Find_Const_Offset");
      return -1;
  }
}

// -----------------------------------------------------------------------
// Given a variable and an offset into it, find a WHIRL expression used
// to initialize it in the current function, or return NULL otherwise.
// -----------------------------------------------------------------------
static WN *
Find_Initializer (ST *st, INT64 offset) {
  if (offset == -1) {
    return NULL;
  }
  
  // first try to search backwards, cause faster and more accurate;
  // if not found, then search whole tree, taking first initializer.
  WN *stmt;
  stmt = WN_prev(asm_wn_stmt);
  for (stmt = WN_prev(asm_wn_stmt); stmt != NULL; stmt = WN_prev(stmt)) {
    if (WN_operator (stmt) == OPR_STID
        && WN_st (stmt) == st
        && WN_offset (stmt) == offset) 
    {
      return WN_kid0 (stmt);
    }
    if (WN_operator (stmt) == OPR_ISTORE) {
      WN *dest = WN_kid1 (stmt);
      if (dest && WN_operator (dest) == OPR_LDA
          && WN_st (dest) == st
          && WN_offset (dest) == offset) 
      {
        return WN_kid0 (stmt);
      }
    }
  ...
  for (WN_ITER *wni = WN_WALK_StmtIter (PU_Info_tree_ptr (Current_PU_Info));
    ...
    if (WN_operator (stmt) == OPR_STID
        && WN_st (stmt) == st
        && WN_offset (stmt) == offset) {
      WN *init = WN_kid0 (stmt);
      WN_WALK_Abort (wni);  // reclaim memory!  
      return init;
    }
    if (WN_operator (stmt) == OPR_ISTORE) {
      WN *dest = WN_kid1 (stmt);
...
// Look in initv for const st.
// TY_IDX should be TY of ST associated with inv block.
static ST *
Get_Initv_Symbol (INITV_IDX inv, TY_IDX ty, INT64 offset)
{
  if (INITV_kind(inv) == INITVKIND_BLOCK) {
    INT index; // index into array
    if (TY_kind(ty) == KIND_ARRAY) {
      index = offset / TY_size(TY_etype(ty));
    } else {
      FmtAssert(FALSE, ("s constraint unexpected TY type"));
    }
    inv = INITV_blk(inv);
    for (INT i = 0; i < index; ++i) 
      inv = INITV_next(inv);
  }
  FmtAssert(INITV_kind(inv) == INITVKIND_SYMOFF, 
    ("s constraint operand not a string?"));
  return &St_Table[INITV_st(inv)]; 
}

// -----------------------------------------------------------------------
// Given a variable and an offset into it, find a constant initializer
// for that location in the current function, or return NULL otherwise.
// -----------------------------------------------------------------------
static ST *
Find_Const_Initializer (ST *st, INT64 offset) {          
  
  WN *load = Find_Initializer (st, offset);

  if (!load) {
    INITV_IDX inv = ST_is_const_and_has_initv(st);
    if (inv) {
      return Get_Initv_Symbol(inv, ST_type(st), offset);
    }
    return NULL;
  }
  ...
  if (WN_operator (load) == OPR_ILOAD
    ...
    /* Here, 'st' will most likely be an array. */
    st = WN_st (WN_kid0 (WN_kid0 (load)));
    offset = Find_Const_Offset (WN_kid1 (WN_kid0 (load)));
    if (offset == -1) {
      return NULL;
    }
    return Find_Const_Initializer (st, offset);
  }
			   ...
			   TYPE_ID type)
{
  ...
  else if (strchr("m", *constraint) || strchr("g", *constraint))
  {
	...
	if (ST_class(WN_st(load)) == CLASS_PREG) {
        ...
        FmtAssert(load, ("asm m constraint must have load of simple variable"));
        FmtAssert(WN_has_sym(load), ("asm m constraint must have simple variable"));
	ret_tn = Gen_Symbol_TN (WN_st(load), 0, 0);
        // This is a hack, but if is a compiler temp,
        // including temps that cudafe generates like __T,
	...
	// This can happen with -O0 compiles since no wopt to propagate syms.
        if (ST_is_temp_var(WN_st(load))
         || (strncmp(ST_name(WN_st(load)), "__cuda___T", 10) == 0)
         || (ST_sclass(WN_st(load)) == SCLASS_AUTO
          && strncmp(ST_name(WN_st(load)), "__T", 3) == 0)) 
        {
          OP *op;
          ST *temp_st = WN_st(load);
          BOOL found = FALSE;
          DevWarn("asm m param is __T temp, so replace with earlier def");
          FOR_ALL_OPS_OPs_REV(&New_OPs,op) {
            found = Look_For_Texture_TN (op, &temp_st, &ret_tn);
            if (found) break;
          }
          if (!found) {
            // if not in current list of ops, search previous BBs
            BB *bb = Cur_BB;
            while (!found && bb) {
            ...
            Is_True(found, ("never found asm texture"));
          }
        }
    }
  }
  else if ((*constraint == 'h'))
  {
    ...
    // if load is not I2, still create I2 tn,
    // then let caller do convert.
  ...
  else if (*constraint == 'n')
  {
    if (load && WN_operator(load) == OPR_LDID) {
        // Try to search backwards for store of const into variable.
        // Cannot search backwards through whirl, so iterate whole tree.
        // This is expensive, but rare (when optimized the value should
        // be propagated into asm, so only for -O0 compile).
        ST *st = WN_st(load);
        WN_ITER* wni = WN_WALK_StmtIter(PU_Info_tree_ptr(Current_PU_Info));
        for (; wni != NULL; wni = WN_WALK_StmtNext (wni)) 
        {
          WN* stmt = WN_ITER_wn(wni);
          if (WN_operator (stmt) == OPR_STID
            && WN_st(stmt) == WN_st(load)
            && WN_offset(stmt) == WN_offset(load)
            && WN_operator(WN_kid0(stmt)) == OPR_INTCONST) 
          {
            DevWarn("found earlier load into asm symbol");
            load = WN_kid0(stmt);
            break;
          }
        }
        FmtAssert(wni, ("n constraint init not found"));
        WN_WALK_Abort(wni);  // reclaim memory!
    }
  ...
  else if (*constraint == 's')
  {
      ...
      && WN_operator(WN_kid1(WN_kid0(load))) == OPR_INTCONST)
    {
      ...
      && WN_operator(WN_kid0(load)) == OPR_LDA)
    {
      // iload(lda sym) is same as ldid(sym)
      st = WN_st(WN_kid0(load));
      offset = WN_offset(WN_kid0(load));
    }
    ...
    if (ST_class(st) != CLASS_CONST) {
      INITV_IDX inv = ST_is_const_and_has_initv(st);
      if (inv) {
        st = Get_Initv_Symbol(inv, ST_type(st), offset);
      }
      else if ((WN_operator(load) == OPR_LDID || WN_operator(load) == OPR_ILOAD)
               && ST_class(st) == CLASS_VAR) 
      {
        ...
        // This is expensive, but rare (when optimized the value should
        // be propagated into asm, so only for -O0 compile).
        ST *st0 = Find_Const_Initializer (st, offset);
        
        if (st0) {
          DevWarn ("found earlier load into asm symbol");
          st = st0;
        } else {
          FmtAssert (FALSE, ("s constraint init not found"));
        }
      }
    }
...
/* For each TN, store the first BB in which it appears into tn_bb_use.  If the given TN 
   subsequently appears in another BB, we will not recycle the virtual register it is
   assigned.  */
struct tn_hash {
  size_t operator()(const TN *tn) const { return (size_t)tn; }
};
static hash_map<TN *, BB *, tn_hash> tn_bb_use;
static vector<TN *> regs_to_reclaim;
...
Assign_Virtual_Register (TN *tn)
{
  ...
  if (TN_Is_Allocatable(tn) && TN_register(tn) == REGISTER_UNDEFINED) {
    ...
    // update "highest" reg allocated in this class so far
    last_reg_allocated[cl] = MAX (last_reg_allocated[cl], reg);
...
/* For TNs used only in a single basic block, record the basic block where they are used.  For TNs
   used in multiple basic blocks, assign a virtual register right away.  */
static void
Assign_Register_If_MultiBlock (TN *tn, BB *bb, BOOL force_multi) {
  if (TN_Is_Allocatable(tn) && TN_register(tn) == REGISTER_UNDEFINED) {
    BB *last = tn_bb_use[tn];
    if (!last) {
      tn_bb_use[tn] = bb;
    } else if (last != bb || force_multi) {
      Assign_Virtual_Register (tn);
    }
  }
}

/* Assign a virtual register to a TN appearing in only one basic block, and immediately mark said
   register for recycling upon exit from the basic block.  */
static void
Assign_Register_If_SingleBlock (TN *tn) {
  if (TN_Is_Allocatable(tn) && TN_register(tn) == REGISTER_UNDEFINED) {
    Assign_Virtual_Register (tn);
    regs_to_reclaim.push_back (tn);
  }
}

/* Upon exiting a basic block, recycle all registers that were used only in that block.  Note that
   multi-block TN register assignments must have happened earlier and are not subject to recycling.  */   
static void
Recycle_SingleBlock_Registers (void) {
  while (regs_to_reclaim.size ()) {
    TN *tn = regs_to_reclaim.back ();
    regs_to_reclaim.pop_back ();
    ISA_REGISTER_CLASS cl = TN_register_class (tn);
    REGISTER reg = TN_register (tn);
    avail_set[cl] = REGISTER_SET_Union1 (avail_set[cl], reg);
  }
}
...
Assign_Virtual_Registers(void)
{
  ...
  /* Since we usually inline function calls inside the GPU, this can lead to an explosion in the number
     of virtual (PTX) registers that is required.  Once the size of a register file reaches into the
     tens of thousands (!), Open64 has difficulties staying up on its feet.
     
     Ergo, we need some quick-and-dirty method for reducing the number of required virtual registers by
     allowing them to be reused (gasp!).  Since register liveness information for basic blocks has not
     yet been computed at this point, we need to roll our own hack.  */

  /* NB: For now, we recycle registers only if -OPT:reuse_regs=true is specified (this option is off
     by default), or if not recycling registers is very likely to result in running out of them.  */
  BOOL reg_reuse = (OPT_Enable_Reg_Reuse_Set? OPT_Enable_Reg_Reuse: Last_TN > ISA_REGISTER_MAX);

  if (reg_reuse) {
    for (bb = REGION_First_BB; bb != NULL; bb = BB_next(bb)) {
      FOR_ALL_BB_OPs (bb, op) {
        /* If we are dealing with a load from shared memory, Rematerialize_GRF() may wind up inserting
           additional copies of this instruction in other basic blocks in order to make the loads
           adjacent to their uses.  So, we must be conservative and assume that any registers used
           to compute the address of the load must remain live across multiple basic blocks.  */
      	BOOL may_need_in_other_bb = (CG_rematerialize_grf && OP_load(op) && OP_is_shared_memory(op));
        for (INT i = 0; i < OP_results(op); i++) {
          tn = OP_result(op,i);
          Assign_Register_If_MultiBlock (tn, bb, FALSE);
        }
        for (INT i = 0; i < OP_opnds(op); i++) {
          tn = OP_opnd(op,i);
          Assign_Register_If_MultiBlock (tn, bb, may_need_in_other_bb);
        }
      }
    }
  }
  ...
  for (bb = REGION_First_BB; bb != NULL; bb = BB_next(bb)) {
    FOR_ALL_BB_OPs (bb, op) {
      for (INT i = 0; i < OP_results(op); i++) {
        tn = OP_result(op,i);
        if (reg_reuse) {
          Assign_Register_If_SingleBlock (tn);
        } else {
          Assign_Virtual_Register (tn);
        }
      }
...
CG_LOOP_BACKPATCH *CG_LOOP_Backpatch_Next(CG_LOOP_BACKPATCH *bp) { return bp->next; }
#if 0
...
ST * CGSPILL_OP_Spill_Location (OP *op) { return NULL; }
...
/* In function tnutil.cxx:Is_OP_Cond(), Visual C++ will generate a call to
   Is_Delay_Slot_Op(op, bb) even though the controlling branch is never
   taken!  We provide a stub for Is_Delay_Slot_Op(op, bb) here so
   that the linker does not complain.  */
BOOL
Is_Delay_Slot_Op (OP *op, BB *bb)
{
  FmtAssert(FALSE, ("Routine Is_Delay_Slot_Op(op, bb) should not be called"));
  return FALSE;
}

INT16 CGPREP_Same_Res_Opnd(OP *op) { return -1; }
...
Sizeof_Load_Store (OP *op)
{
    if (TOP_is_memory_8bit(OP_code(op)))
        ...
        FmtAssert(FALSE, ("unexpected load/store"));
}

// do two memory ops alias?
BOOL
Memory_Ops_Alias (OP *op1, OP *op2)
{
 if(OP_memory(op1) && OP_memory(op2)) {
   TN *space1_tn = OP_Space_TN(op1);
   TN *space2_tn = OP_Space_TN(op2);
   if (space1_tn != space2_tn 
       && TN_enum(space1_tn) != ECV_space_none
       && TN_enum(space2_tn) != ECV_space_none)
   {
     return FALSE; // different specific spaces
   }
   WN *op1_wn = Get_WN_From_Memory_OP(op1);
   WN *op2_wn = Get_WN_From_Memory_OP(op2);
   if(op1_wn == NULL || op2_wn == NULL)
     return TRUE;
   ALIAS_RESULT result = Aliased(Alias_Manager, op1_wn, op2_wn);
   if (result == SAME_LOCATION) {
     return TRUE;
   }
   else if (result == NOT_ALIASED) {
     return FALSE;
   }
   else { // POSSIBLY_ALIASED
     // array indexes typically alias with whole array,
     // so add tighter check that offsets do not overlap if base is same
     TN *base_tn = OP_opnd(op1, OP_find_opnd_use(op1,OU_base));
     if (base_tn == OP_opnd(op2, OP_find_opnd_use(op2,OU_base))
         && TN_has_one_def(base_tn))
     {
       TN *offset1_tn = OP_opnd(op1, OP_find_opnd_use(op1, OU_offset));
       TN *offset2_tn = OP_opnd(op2, OP_find_opnd_use(op2,OU_offset));
       if (TN_has_value(offset1_tn) && TN_has_value(offset2_tn)) {
         // check if offsets+size overlap
         INT64 offset1 = TN_value(offset1_tn);
         INT64 offset2 = TN_value(offset2_tn);
         if (offset1 == offset2) {
           return TRUE;
         } else if (offset1 > offset2
           && offset1 < (offset2 + Sizeof_Load_Store(op2)))
         {
           return TRUE;
         } else if (offset1 < offset2
           && (offset1 + Sizeof_Load_Store(op1)) > offset2)
         {
           return TRUE;
         } else {
           return FALSE; // offsets don't overlap
         }
       }
       else {
...
// Check if safe to move the load between first and last op.
// Assumes that first_op and last_op are in same bb.
BOOL
Safe_To_Move_Load (OP *load_op, OP *first_op, OP *last_op)
{
  BOOL trace = FALSE;
  if (first_op == NULL || last_op == NULL) {
    return TRUE; // zero range
  }
  FmtAssert(OP_bb(first_op) == OP_bb(last_op), ("ops not in same bb?"));
  TN *load_tn = OP_result(load_op,0);
  // search first..last_op inclusive, so go till next(last)
  for (OP *op = first_op ; op != NULL && op != OP_next(last_op); op = OP_next(op)) 
  {
    if (OP_Defs_TN(op, load_tn)) {
      if (OP_code(op) == OP_code(load_op)) {
        BOOL same = TRUE;
        for (INT i = 0; i < OP_opnds(op); ++i) {
          if (OP_opnd(op,i) != OP_opnd(load_op,i))
            same = FALSE;
        }
        if (same) continue; // okay if same op
      }
      if (trace) 
        fprintf(TFile, "not safe_to_move_load tn%d cause def in bb%d\n", 
                TN_number(load_tn), BB_id(OP_bb(first_op)));
      return FALSE;
    }
    else if (CGTARG_Is_OP_Barrier(op)) { // e.g. syncthreads
      if (trace) 
        fprintf(TFile, "not safe_to_move_load tn%d cause barrier in bb%d\n", 
                TN_number(load_tn), BB_id(OP_bb(first_op)));
      return FALSE;
    }
    else if (OP_store(op) && Memory_Ops_Alias(load_op, op)) {
      if (trace) 
        fprintf(TFile, "not safe_to_move_load tn%d cause store in bb%d\n", 
                TN_number(load_tn), BB_id(OP_bb(first_op)));
      return FALSE;
    }
  }
...
// recursively add new predecessor bbs, stopping when already in bbs.
static void
Add_Predecessor_BBs (BB_SET *bbs, BB *bb, MEM_POOL *pool)
{
  BBLIST *edge;
  FOR_ALL_BB_PREDS(bb, edge) {
    BB *bp = BBLIST_item(edge);
    if ( ! BB_SET_MemberP(bbs, bp)) {
      bbs = BB_SET_Union1D (bbs, bp, pool);
      Add_Predecessor_BBs (bbs, bp, pool);
    }
  }
}

// return the set of BBs between start and end BB.
// Assumes that start dominates end.
BB_SET *
Find_Intermediate_BBs (BB *start, BB *end, MEM_POOL *pool)
{
  FmtAssert(BB_SET_MemberP(BB_dom_set(end), start), ("start does not dominate end"));
  BB_SET *bbs = BB_SET_Create_Empty (PU_BB_Count+2, pool);
  if (start == end) 
    return bbs;
  // to simplify checks, first add start to set, will remove at end
  bbs = BB_SET_Union1D (bbs, start, pool);
  Add_Predecessor_BBs (bbs, end, pool);
  bbs = BB_SET_Difference1D(bbs, start);
  return bbs;
}

// find latest def of tn that reaches use_op;
...
Find_Reaching_Def (TN *tn, OP *use_op)
{
  ...
  FmtAssert(BB_Has_Dominator_Info(use_bb), ("no dominator info"));
  // iterate through dominator bbs, remember last one that defines tn
  FOR_ALL_BB_SET_members (BB_dom_set(use_bb), bb) {
    FOR_ALL_BB_OPs_REV (bb, op) { // iterate in reverse to find last op
      if (OP_Defs_TN(op, tn)) {
        if (def_bb) {
          if (BB_SET_MemberP(BB_dom_set(bb), def_bb)) {
            def_bb = bb; // bb is later than def_bb
            def_op = op;
          }
        }
        else {
