...
void Create_Instruction_Map(void)
{
  ...
  InstructionMap[TOP_cvt_sat_s8_s32] = TOP_mov_s16;
  InstructionMap[TOP_cvt_sat_s8_u32] = TOP_mov_s16;
  InstructionMap[TOP_cvt_sat_u8_s32] = TOP_mov_u16;
  InstructionMap[TOP_cvt_sat_u8_u32] = TOP_mov_u16;
  InstructionMap[TOP_cvt_s32_s8] = TOP_mov_s16;
  InstructionMap[TOP_cvt_u32_u8] = TOP_mov_u16;
  InstructionMap[TOP_cvt_sat_s8_s32_b32] = TOP_mov_s16;
  InstructionMap[TOP_cvt_sat_s8_u32_b32] = TOP_mov_s16;
  InstructionMap[TOP_cvt_sat_u8_s32_b32] = TOP_mov_u16;
  InstructionMap[TOP_cvt_sat_u8_u32_b32] = TOP_mov_u16;
  InstructionMap[TOP_cvt_sat_s16_s32] = TOP_mov_s16;
  InstructionMap[TOP_cvt_sat_s16_u32] = TOP_mov_s16;
  InstructionMap[TOP_cvt_sat_u16_s32] = TOP_mov_u16;
  InstructionMap[TOP_cvt_sat_u16_u32] = TOP_mov_u16;
  InstructionMap[TOP_cvt_s32_s16] = TOP_mov_s16;
  InstructionMap[TOP_cvt_u32_u16] = TOP_mov_u16;
  InstructionMap[TOP_cvt_sat_s16_s32_b32] = TOP_mov_s16;
  InstructionMap[TOP_cvt_sat_s16_u32_b32] = TOP_mov_s16;
  InstructionMap[TOP_cvt_sat_u16_s32_b32] = TOP_mov_u16;
  InstructionMap[TOP_cvt_sat_u16_u32_b32] = TOP_mov_u16;
  ...
  InstructionMap[TOP_setp_cmp_s32] = TOP_setp_cmp_s16;
  InstructionMap[TOP_setp_cmp_u32] = TOP_setp_cmp_u16;
}
  ...
  do {
	...
	// but if inputs are 16bit can change op to wide op.
      ...
      FOR_ALL_BB_OPs_FWD (bb, op) {
        ...
        // Do this in fwd pass so all defs are seen first.
        else if (TOP_is_compare_32(OP_code(op)))
        {
          okay = TRUE;
          for (i = 0; i < OP_opnds(op); ++i) {
            TN *otn = OP_opnd(op,i);
            if (TN_is_enum(otn))   // <cmp>
              ;
            else if (TN_is_register(otn) && TN_def_is_16bit(otn))
              ;
            else if (TN_has_value(otn) 
              && ISA_LC_Value_In_Class(TN_value(otn), 
                (OP_opnd_is_signed(op,i) ? LC_i16 : LC_u16) ) )
...
// C promotion rules can cause unsigned 16bit mul to be promoted to
// signed 32bit mul, which when demoting back to 16bit we need to
// make sure that we do unsigned because of overflow differences
// (note that this doesn't matter for mul.lo.s16 since result is 16bit).
// So check if source or dest is unsigned.
static BOOL
Should_Be_Unsigned_Op (OP *op)
{
  TN *tn;
  OP *op1;
  INT i;
  ...
  for (i = 0; i < OP_opnds(op); ++i) {
    ...
    if ( ! TN_is_register(tn)) 
      continue;
    for (op1 = OP_prev(op); op1; op1 = OP_prev(op1)) {
      if (OP_result(op1,0) == tn) {
        // found def
        switch (OP_code(op1)) {
        case TOP_ld_qualifier_space_u16:
        case TOP_ld_qualifier_space_u16_o:
        case TOP_ld_qualifier_space_u16_r:
        case TOP_ld_qualifier_space_u16_b32:
        case TOP_ld_qualifier_space_u16_b32_o:
        case TOP_ld_qualifier_space_u16_b32_r:
        case TOP_ld_qualifier_space_u16_a64:
        case TOP_ld_qualifier_space_u16_a64_o:
        case TOP_ld_qualifier_space_u16_a64_r:
        case TOP_ld_qualifier_space_u16_b32_a64:
        case TOP_ld_qualifier_space_u16_b32_a64_o:
        case TOP_ld_qualifier_space_u16_b32_a64_r:
          if (tracing) fprintf(TFile, "found unsigned def\n");
          is_unsigned = TRUE;
          break;
        case TOP_cvt_sat_u16_u32:
        case TOP_cvt_sat_u16_u64:
        case TOP_cvt_sat_u16_s32:
        case TOP_cvt_sat_u16_s64:
        case TOP_cvt_sat_u16_u32_b32:
        case TOP_cvt_sat_u16_s32_b32:
        case TOP_cvt_sat_u16_u32_b64:
        case TOP_cvt_sat_u16_s32_b64:
        case TOP_cvt_u32_u16:
        case TOP_cvt_u64_u16:
          if (tracing) fprintf(TFile, "found unsigned def cvt\n");
          is_unsigned = TRUE;
          break;
        case TOP_mov_u16:
          if (tracing) fprintf(TFile, "found unsigned def mov\n");
          is_unsigned = TRUE;
          break;
        // ideally should handle case of intermediate defs
        // where we track back to original load/cvt,
        // but this seems to catch all the cases we've seen so far.
        }
      }
    }
  }
...
Modify_16bit_Ops (void)
{
  ...
  for (bb = REGION_First_BB; bb != NULL; bb = BB_next(bb)) {
    FOR_ALL_BB_OPs (bb, op) {
      ...
      // The new instruction may have fewer qualifiers
      INT enum_args = OP_fixed_opnds(op) - TOP_fixed_opnds(new_opc);
      if (newop) {
	...
	if (TN_is_register(tn)) {
      ...
      if (def_okay && use_okay && new_opc) {
        ...
        if (enum_args) {
          FmtAssert(enum_args > 0, ("converting to insn with more qualifiers"));
          // Remove any enum "arguments" present in the original insn.
          op->opr = (mTOP)new_opc;
          for (int o = 0; o < OP_fixed_opnds(op) + OP_fixed_results(op); ++o) {
            Set_OP_opnd(op, o, OP_opnd(op, o + enum_args));
          }  
          op->opnds -= enum_args;
        } else {
      ...
      else if (use_okay && TOP_is_mul_lo_32(OP_code(op)))
      {
        ...
        switch (OP_code(op)) {
        case TOP_mul_lo_s32: 
          if (Should_Be_Unsigned_Op(op))
            new_opc = TOP_mul_wide_u16; 
          else
            new_opc = TOP_mul_wide_s16;
          break;
        case TOP_mul_lo_u32: 
          new_opc = TOP_mul_wide_u16; 
          break;
        case TOP_mul_lo_s32_lit: 
          if (Should_Be_Unsigned_Op(op))
            new_opc = TOP_mul_wide_u16_lit;
          else
            new_opc = TOP_mul_wide_s16_lit;
          break;
        ...
        if (tn_def_is_16bit) {
          ...
          for (i = 0; i < OP_results(op); ++i) {
            ...
            if (TN_use_is_16bit(tn) && TN_def_is_16bit(tn)) {
              ...
              Build_OP (TOP_cvt_sat_u16_u32, tn, Gen_Enum_TN(ECV_sat_none), tmp, &ops); 
