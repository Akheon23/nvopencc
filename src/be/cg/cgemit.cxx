...
// un-ifdef this when gas can handle tags inside explicit bundle
  ...
  if (vstr_len(comment) == 0) {
    // no comment, so don't emit empty comment characters as we store ptx source
    fprintf (Asm_File, "\n");
    vstr_end(comment);
    return;
  }
...
#endif
#ifdef TARG_NVISA
  // need to check for %{b,h,x}<N> modifiers,
  // but need to verify that they are modifying this position, else ignore.
  char bmod[5];
  char hmod[5];
  char xmod[5];
  sprintf(bmod, "%%b%d", position);
  sprintf(hmod, "%%h%d", position);
  sprintf(xmod, "%%x%d", position);
#endif
  if( TN_is_register(tn) ){
...
#ifdef TARG_NVISA
    if (strstr(asm_string, bmod)) {
      // Replace any %b<num> constraint with the immediate value written in 
      // 2 hex digits, preceded with 0
      sprintf(buf, "%02x",(INT)TN_value(tn));
    } else if (strstr(asm_string, hmod)) {
      // Replace any %h<num> constraint with the immediate value written in 
      // 4 hex digits, preceded with 0
      sprintf(buf, "%04x",(INT)TN_value(tn));
    } else if (strstr(asm_string, xmod)) {
      // Replace any %x<num> constraint with the immediate value written in 
      // 8 hex digits, preceded with 0
      sprintf(buf, "%08x",(INT)TN_value(tn));
    } else {
#ifdef __MINGW32__
...
#else
#ifdef TARG_NVISA
  // check for modifiers on %<N> strings
  if (strstr(asm_string, bmod)) {
    strcpy(pattern, bmod);
  }
  else if (strstr(asm_string, hmod)) {
...
Emit_Loop_Note(BB *bb, FILE *file)
{
  ...
  if (bb == head) {
    ...
    if (info) {
      ...
      INT64 trip_count = constant_trip ? TN_value(trip_tn) :
	(INT64)WN_loop_trip_est(wn);
      if (anl_note) fprintf(file, " \"");
      else fprintf(file, ", ");
      fprintf (file, "nesting depth: %d", depth);

      if (!constant_trip && trip_count == 0) {
        fprintf (file, ", estimated iterations: unknown");
      } else {
        fprintf (file, ", %siterations: %" LL_FORMAT "d", estimated, trip_count);
      }
      if (anl_note) fprintf(file, "\"");
    }
    fputc ('\n', file);
...
#ifdef TARG_NVISA
  // print pragma unroll hint if user specified it, so ptxas doesn't override it
  if (Target_ISA >= TARGET_ISA_compute_20) {
    if (BB_loophead(bb) && BB_has_pragma(bb)) {
      ANNOTATION *ant;
      for ( ant = ANNOT_First (BB_annotations(bb), ANNOT_PRAGMA);
            ant != NULL;
            ant = ANNOT_Next (ant, ANNOT_PRAGMA))
      {
        if (WN_pragma(ANNOT_pragma(ant)) == WN_PRAGMA_UNROLL) {
          fprintf(Asm_File, "\t.pragma \"nounroll\";\n");
        }
      }
    }
  }
...
Process_Initos_And_Literals (SYMTAB_IDX stab)
{
...
#ifdef TARG_NVISA
  // rewalk whole list each time, but skip processed initos,
  // so that we emit when reach a use (which may come in later pu).
  for (i = 1; i < INITO_Table_Size(GLOBAL_SYMTAB); ++i) {
#else
  for (i = last_inito; i < INITO_Table_Size(GLOBAL_SYMTAB); ++i) {
    ...
    /* If ST is used in initialization, ensure the symbol it is initializing
     * is declared after ST. So if ST is an array of characters, then
     * store them at the start of the vector. The remaining are copied from
     * tmp_st_list in a separate pass. This is to prevent expensive insertion
     * at the front of the vector. */
    if (ST_initv_in_other_st(st) &&
        TY_kind(ST_type(st)) == KIND_ARRAY &&
        TY_is_character(TY_etype(ST_type(st))))
      st_list.push_back(st);
    else
      tmp_st_list.push_back(st);
#else
    st_list.push_back(st);
#endif
...
#ifdef TARG_NVISA
  // move the syms from tmp_st_list to st_list
  for (st_iter = tmp_st_list.begin(); st_iter != tmp_st_list.end(); ++st_iter) {
    st_list.push_back(*st_iter);
  }
#endif
...
#ifndef TARG_NVISA
  // Nvisa does not emit symbols in sections based on offsets. The sort
  // instead just shuffles the symbols around, and sometimes may cause
  // "b" of "a = b" to be emitted after the initialization of "a". So
  // disable the sorting.
  //
...
EMT_Emit_PU ( ST *pu, DST_IDX pu_dst, WN *rwn )
{
  ...
  for (i = last_global_index; i < ST_Table_Size(GLOBAL_SYMTAB); ++i) {
	...
	if (Target_ISA >= TARGET_ISA_compute_20 
        ...
        {
         // print information about extern variables
         CGEMIT_Print_Variable(sym);
        }          
#endif
  }
  ...
  */
  ...
  FOREACH_SYMBOL (CURRENT_SYMTAB, sym, i) {
    ...
    if (Assembly) {
#ifdef TARG_NVISA 
     if (Target_ISA>=TARGET_ISA_compute_20)	// not needed when no stack
#endif
  ...
  char *options)	/* Option string used for compilation */
{
  ...
  if (Assembly) {
...
#endif
#ifdef TARG_NVISA
    CGEMIT_Version_Info(process_name);
    CGEMIT_Global_Decls();
#else
    ...
    //#if  defined(__linux__) || !defined(TARG_NVISA)
...
#elif defined(VENDOR_OSP) || defined(TARG_NVISA)
