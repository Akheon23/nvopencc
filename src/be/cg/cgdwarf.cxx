...
#ifdef TARG_NVISA
#include "register.h"
extern REGISTER find_register_from_st (ST *st, ISA_REGISTER_CLASS *rclass);
bool seen_inlined_subp=false;
#endif
  ...
  Dwarf_Half loc_attr)
{
  ...
  if (ST_sym_class(st) == CLASS_NAME) return;
    ...
    // However, it does need the address class info.

    if (ST_sclass(st) == SCLASS_COMMON && ST_export(st) == EXPORT_PREEMPTIBLE &&
        ST_is_weak_symbol(st) && ST_in_shared_mem(st))
    {
      dwarf_add_AT_unsigned_const (dw_dbg, die, DW_AT_address_class, DW_ADDR_shared_space, &dw_error);
      return;
    }
#endif
...
#ifdef __linux__
          if (CGEXP_put_locals_in_registers) 
#else
	  if (1)
#endif
          {
  	     /* output DW_OP_regx reg_number here */
            REGISTER reg; 
            ISA_REGISTER_CLASS rclass;

            reg = find_register_from_st (st, &rclass);
            
	    if (reg == REGISTER_UNDEFINED)
	      {
		ErrMsg(EC_DWARF_Bad_Register_Number, ST_name(st));
              }
             else
	       {   
                  dwarf_add_expr_gen_regname (expr, DW_OP_regx, (char *)REGISTER_name(rclass, reg)
			      , &dw_error);
                  st_in_reg = TRUE;
               } 
	  }
  ...
  // add memory space info
  {
    ...
    else if (ST_in_surface_mem(st)) space = DW_ADDR_surf_space;
    else space = DW_ADDR_none;

    if (st_in_reg) 
       space = DW_ADDR_reg_space;
...
#ifdef TARG_NVISA
  if (tag == DW_TAG_lexical_block && seen_inlined_subp) 
    return die; 
#endif
...
#ifdef TARG_NVISA
  /* For GPU, DWARF children tags should always be generated */
   visit_children=TRUE; 
#else
...
#ifdef TARG_NVISA
  seen_inlined_subp = subprogram_def_is_inlined(PU_attr);
#endif
		     ...
		     INT        high_pc)
{
  ...
  if (subprogram_def_is_inlined(PU_attr)) {
	...
	put_concrete_subprogram(pu_dst,
				...
				idie);
#ifdef TARG_NVISA
        // copy any formal_params or locals to the concrete instance too.
        // pu_dst is the original sub, check its children
        Dwarf_P_Die parent = idie;
        Dwarf_P_Die left = NULL;
        for (DST_INFO_IDX idx = DST_first_child (pu_dst);
             !DST_IS_NULL(idx);
             idx = DST_INFO_sibling(DST_INFO_IDX_TO_PTR(idx)))
        {
          info = DST_INFO_IDX_TO_PTR (idx);
          if (DST_INFO_tag(info) == DW_TAG_formal_parameter
           || DST_INFO_tag(info) == DW_TAG_variable) 
          {
	    if (Trace_Dwarf) fprintf(TFile, "copy formal/var to concrete\n");
            // must be one non-null node to attach to, either parent or left:
	    left = dwarf_new_die (dw_dbg, DST_INFO_tag(info), 
              parent, NULL, left, NULL, &dw_error);
            parent = NULL; // will now attach to left
            Write_Attributes (
              DST_INFO_tag(info), 
              DST_INFO_flag (info), 
              DST_INFO_attributes(info), 
              left);
            put_reference (idx, DW_AT_abstract_origin, left);
          }
        }
#endif // NVISA
	PU_die = idie;
...
print_source (SRCPOS srcpos)
{
  ...
  if (USRCPOS_filenum(usrcpos) != cur_file_index) {
    ...
    for (i = 0; i < cur_file->max_line_printed; i++) {
      ...
      /* if used to fool gcc 4.3.2 because it does not accept the cast
       * (void)fgets which is a bug in my opinion
       */
    }
  }
...
struct ALIGN_SPEC(1) UINT32_unaligned {
  UINT32 val;
};

struct ALIGN_SPEC(1) UINT64_unaligned {
  UINT64 val;
};
		...
		Dwarf_Unsigned cur_byte_in)
{
    ...
    for (i = 0; i < nlines_this_reloc; i++) {
      ...
      // use separate fputs in case AS_ macro is not simple
      fputs("\t", asm_file);
