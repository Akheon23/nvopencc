...
//            CG_VALUE_Annotate(RID_cginfo(Current_Rid),PROFILE_PHASE_LAST,FALSE,FALSE);
  ...
  if (Target_ISA >= TARGET_ISA_compute_20 && !CG_oldstyle_params) {
    // rewrite for new calling convention
    Modify_Parameter_Accesses(); 
  }
  ...
  if (CG_opt_level > 0) {
    ...
    // Do register pressure targeted optimizations
    if (CG_rp_opts) {
      Set_Error_Phase("CG-RP optimization phase");
      Rp_Core *rp = Rp_Init();
      {
        rp->Do_Local_Scheduling();
      }
      Rp_Fini();
      Check_for_Dump ( TP_RP, NULL );
    }
  }
  ...
  if (CG_opt_level > 0) {
    ...
    if (CG_rematerialize_grf) {
      ...
      Set_Error_Phase("Extended Block Optimizer - rematerialize grf");
      Rematerialize_GRF();
      Check_for_Dump ( TP_EBO, NULL );
    }
    if (CG_rematerialize_addr) {
      // iterate thru code, see if can rematerialize immediates into addresses
      Set_Error_Phase("Extended Block Optimizer - rematerialize addr");
      Rematerialize_Addr();
      Check_for_Dump ( TP_EBO, NULL );
    }
    GRA_LIVE_Recalc_Liveness(region ? REGION_get_rid( rwn) : NULL);
    ...
    if (CG_dump_maxlivein) {
      fprintf(stderr, "MAXLIVEIN %d %s %d\n", Target_ISA,
              Cur_PU_Name, Calculate_Maxlivein());
    }

    if (CG_dump_fatpoint) {
      fprintf(stderr, "FATPOINT %d %s %d\n", Target_ISA,
              Cur_PU_Name, Calculate_Fatpoint(&MEM_local_nz_pool));
    }
