...
BOOL CGEXP_put_locals_in_registers = TRUE;

BOOL CG_vector_loadstore = TRUE;	// emit vector loads and stores

BOOL CG_rematerialize_grf = TRUE;
BOOL CG_rematerialize_sreg = FALSE; // rematerialize uses of sregs
BOOL CG_rematerialize_addr = TRUE;

BOOL CG_rp_opts = FALSE;         // optimizations targeted at register pressure

BOOL CG_create_unique_tn_defs = TRUE; // create unique defs of TNs
BOOL CG_merge_tn_defs = TRUE;    // merge multiple defs of same TN
...
/* NB: The '-ftz -no-prec-div -no-prec-sqrt' is consistent with what Tesla provides,
       and also with the nvcc '-use_fast_math' option.  The default values for Fermi
       could be different.  The initialization is also performed in cgdriver.cxx. */
INT32 CG_use_ftz = 1;          // flush-to-zero
INT32 CG_use_prec_div = 0;     // precise division and reciprocal
INT32 CG_use_prec_sqrt = 0;    // precise square root

BOOL CG_dump_maxlivein = FALSE;
BOOL CG_dump_fatpoint = FALSE;

BOOL  CG_oldstyle_params = FALSE; // use old-style param syntax
#endif
