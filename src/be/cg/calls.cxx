...
#endif
{
  ...
  FOR_ALL_BB_OPs_REV(bb, op) {
	 ...
	 */
#ifdef TARG_NVISA
	new_tn = rtn_tns[CREATE_CLASS_N_REG(rc, reg)];
#else	 
	 ...
	 */
#ifdef TARG_NVISA
	if ( rtn_tns[CREATE_CLASS_N_REG(rc, reg)] == NULL ) {
#else	 
	if ( rtn_tns[rc][reg] == NULL ) {
	  ...
	  OPS ops = OPS_EMPTY;
#ifdef TARG_NVISA
	  rtn_tns[CREATE_CLASS_N_REG(rc, reg)] = new_tn;
#else	  
	...
	if ( Is_Function_Value(tn) ) {
	  ...
	  ISA_REGISTER_CLASS rc = TN_register_class(tn);
#ifdef TARG_NVISA
	  TN *new_tn = rtn_tns[CREATE_CLASS_N_REG(rc, reg)];
#else	  
...
Generate_Unique_Exit(void)
{
   ...
   */
#ifdef TARG_NVISA
  /* NVISA uses vast numbers of registers, and so allocating rtn_tns on
     the stack can cause stack overflows.  Given that rtn_tns is actually
     extremely sparse, we will use a hash map instead.  */
  hash_map<mUINT32, TN *> rtn_tns;
#else   
  TN *rtn_tns[ISA_REGISTER_CLASS_MAX+1][REGISTER_MAX+1];
#endif
