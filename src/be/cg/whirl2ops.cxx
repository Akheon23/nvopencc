...
static PREG_NUM last_actual_param_used[ISA_REGISTER_CLASS_MAX+1];
static PREG_NUM last_actual_retval_used[ISA_REGISTER_CLASS_MAX+1];
#endif //TARG_NVISA
...
#endif
#ifdef TARG_NVISA
        if (Is_Formal_Preg(preg_num)) {
          last_actual_param_used[rclass] = 
            MAX (last_actual_param_used[rclass], preg_num);
        } 
        else { // return value preg
          last_actual_retval_used[rclass] = 
            MAX (last_actual_retval_used[rclass], preg_num);
        }
#endif
      	tn = Build_Dedicated_TN(rclass, reg, TY_size(preg_ty));

#ifdef TARG_X8664
	if( preg_num == First_Int_Preg_Return_Offset &&
	    Is_Target_32bit() ){
...
PREG_To_TN_Clear (void)
{
  ...
  // track which actual regs are used; first clear the array
  FOR_ALL_ISA_REGISTER_CLASS(rc) {
    last_actual_param_used[rc] = 0;
    last_actual_retval_used[rc] = 0;
  }
#endif
}
...
#ifdef TARG_NVISA
  // return value uses may be optimized away, but need to know about them to
  // track registers that are used, so call PREG_To_TN for all return values.
  TY_IDX call_ty = call_st ? ST_type(call_st) : WN_ty(call);
...
static VARIANT Memop_Variant(WN *memop)
{
  ...
  if (required_alignment > 1) {
	...
	offset = WN_load_offset(memop);
#ifdef TARG_NVISA
        // want to use alignment of base object
        WN *lda = Find_Lda (WN_kid0(memop));
        if (lda) {
          INT lda_align = ty_align; 
          if (WN_operator(lda) == OPR_LDID
            && TY_kind(ST_type(WN_st(lda))) == KIND_POINTER)
          {
            // Find_Lda can return LDID because that may still help
            // to determine memory space.  But for our use here,
            // we want to look at the pointed to type.
            // The ty on the ldid may not be same as ty on st,
            // if has cast; cast'ed type is final type to use,
            // but st type is real alignment of object.
            ty = TY_pointed(ST_type(WN_st(lda)));
            if (ty != Void_Type) { // ignore if void
              lda_align = TY_align(ty);
            }
          }
          else if (WN_operator(lda) == OPR_LDID
            && Is_Structure_Type(WN_ty(lda)) 
            && (WN_field_id(lda) != 0)) 
          {
            // If we are reading field of structure, use field type
            UINT cur_field_id = 0;
            FLD_HANDLE fld = FLD_get_to_field(WN_ty(lda),
                                          WN_field_id(lda),
                                          cur_field_id);
            if (!fld.Is_Null() && TY_kind(FLD_type(fld)) == KIND_POINTER) {
              ty = TY_pointed(FLD_type(fld));
              if (ty != Void_Type) { // ignore if void
                lda_align = TY_align(ty);
              }
            }
            ...
            lda_align = ST_alignment(WN_st(lda));
          }
          if (lda_align < ty_align) {
            DevWarn("use alignment from lda under iload %d->%d", ty_align, lda_align);
            // such pointer casting is sometimes legal,
            // so emit warning only if user asks for it
            if (DEBUG_Unalign_Check) {
	      ErrMsgSrcpos (EC_Unaligned_Memory_Warn, current_srcpos);
            }
            // ty_align = lda_align;
          }
        }
	...
	offset = WN_store_offset(memop);
        ...
        if (lda) {
          ...
          if (lda_align < ty_align) {
            DevWarn("use alignment from lda under istore %d->%d", ty_align, lda_align);
            // such pointer casting is sometimes legal,
            // so emit warning only if user asks for it
            if (DEBUG_Unalign_Check) {
	      ErrMsgSrcpos (EC_Unaligned_Memory_Warn, current_srcpos);
...
#ifdef TARG_NVISA
    else if (WN_Load_Uniform(memop)) Set_V_ld_uniform(variant);
#endif    
#endif
  return variant;
}
...
#ifdef TARG_NVISA
    // Note in above that WN_ty may have more exact info (U1 instead of U4), 
    // but that may not match actual preg kind (pregs are reg-sized),
    // so still use WN_st for creating preg; later 16bit optimization 
    // can convert these if safe to smaller reg.

    // check if need to add convert
    TYPE_ID rtype = OPCODE_rtype(opcode);
    if (result && TN_size(result) != TN_size(ldid_result)) {
        // asms may have result be different size from rtype
        Expand_Convert (result, Mtype_Of_TN(result), 
          ldid_result, Mtype_Of_TN(ldid_result), &New_OPs);
        return result;
    }
    if (result) 
...
#ifdef TARG_NVISA
static void
Copy_Memory_Space_To_Variant (const ST_MEMORY m, VARIANT *v)
{
	switch (m) {
	case MEMORY_GLOBAL:
	    Set_V_global_mem(*v);
	...
	default:
	    ...
	    DevWarn("unknown memory for variant"); 
...
Handle_ILOAD (WN *iload, TN *result, OPCODE opcode)
{
  ...
  if (WN_operator_is(kid0, OPR_LDA)) {
    ...
    if (OPT_Old_Mem_Ptr_Analysis) {
      ...
      WN *pldid = Find_Pointer_Ldid (addr_wn);
      if (lda) {
        if (WN_operator(lda) == OPR_LDID 
	  ...
	  ST_MEMORY m = ST_memory_space(WN_st(lda));
	  if (ST_sclass(WN_st(lda)) == SCLASS_FORMAL)
	    m = MEMORY_PARAM;
          else if (WN_operator(lda) == OPR_LDA && m != MEMORY_UNKNOWN)
            ; // use memory_space of lda
          else if ((ST_sclass(WN_st(lda)) == SCLASS_AUTO
                 || ST_sclass(WN_st(lda)) == SCLASS_PSTATIC)
            && TY_kind(WN_ty(lda)) == KIND_POINTER
            && BE_ST_memory_pointed_to(WN_st(lda)) != MEMORY_UNKNOWN)
          {
	    m = BE_ST_memory_pointed_to(WN_st(lda));
          }
	  Copy_Memory_Space_To_Variant (m, &variant);
        }
      } 
      else if (pldid 
        && BE_ST_memory_pointed_to(WN_st(pldid)) != MEMORY_UNKNOWN) 
      {
	  Copy_Memory_Space_To_Variant (
            BE_ST_memory_pointed_to(WN_st(pldid)), 
	    &variant);
      }
      else if (pldid && ST_class(WN_st(pldid)) == CLASS_PREG
        && Preg_memory_pointed_to(WN_load_offset(pldid)) != MEMORY_UNKNOWN)
      {
	  Copy_Memory_Space_To_Variant (
            Preg_memory_pointed_to(WN_load_offset(pldid)), 
	    &variant);
      } 
      else {
        if (Target_ISA < TARGET_ISA_compute_20) {
      ...
      enum PtrClasses mspace = ILSC->Query(iload);
      if (mspace != Global && mspace != Bottom) {
        WN_Reset_Load_Uniform(iload);
        Reset_V_ld_uniform (variant);
      }
      switch(mspace) {
        case Global:
          Set_V_global_mem(variant);
        ...
        case Top:
        default:
          FmtAssert(0,("Invalid memory space"));
      }
    }
#endif // NVISA
    Last_Mem_OP = OPS_last(&New_OPs);
...
Handle_STID (WN *stid, OPCODE opcode)
{
  ...
  if (WN_class(stid) == CLASS_PREG) {
	...
	Expand_Immediate( result_hi, Gen_Literal_TN(0,4), MTYPE_U4, &New_OPs );
      }
#endif // TARG_X8664
...
#ifdef TARG_NVISA
  if(V_const_mem(variant)) {
    ErrMsgSrcpos(EC_Store_Const_Err, WN_Get_Linenum(stid));      
  }
#endif
  }
...
#ifdef TARG_NVISA
  if(V_const_mem(variant)) {
    ErrMsgSrcpos(EC_Store_Const_Err, WN_Get_Linenum(stbits));      
  }
#endif
}
...
Handle_ISTORE (WN *istore, OPCODE opcode)
{
  ...
  if (WN_operator_is(kid1, OPR_LDA)) {
    ...
    if (OPT_Old_Mem_Ptr_Analysis) {
      ...
      enum PtrClasses mspace = ILSC->Query(istore);
      switch(mspace) {
        case Global:
          Set_V_global_mem(variant);
          break;
...
#ifdef TARG_NVISA
  if(V_const_mem(variant)) {
    ErrMsgSrcpos(EC_Store_Const_Err, WN_Get_Linenum(istore));      
  }
#endif
...
Handle_ISTBITS (WN *istbits)
{
  ...
  if (WN_operator_is(kid1, OPR_LDA)) {
    Last_Mem_OP = OPS_last(&New_OPs);
    Exp_Load(rtype, desc, field_tn, WN_st(kid1), 
    ...
    Last_Mem_OP = OPS_last(&New_OPs);
    Exp_Store(desc, result, WN_st(kid1), WN_store_offset(istbits) + WN_lda_offset(kid1),
	      &New_OPs, variant);
    Set_OP_To_WN_Map(istbits);
  }
  else {
...
#ifdef TARG_NVISA
  if(V_const_mem(variant)) {
    ErrMsgSrcpos(EC_Store_Const_Err, WN_Get_Linenum(istbits));      
  }
#endif
}
...
#ifdef TARG_NVISA
/* Optimize away the reciprocal for certain types of
   denumerator and rounding modes. */
static TN *
Handle_RECIP (WN *recip, TN *result)
{
  WN *denum = WN_kid0 (recip);

  if (!result) {
    result = Allocate_Result_TN (recip, NULL);
  }
  
  if (WN_opcode (recip) == OPC_F4RECIP && WN_opcode (denum) == OPC_F4SQRT
      && !CG_use_prec_div && !CG_use_prec_sqrt) {
    /* 1/sqrtf(x) -> rsqrtf(x)  [.approx rounding mode only] */
    TN *op = Expand_Expr (WN_kid0 (denum), denum, NULL);
    Exp_OP1 (OPC_F4RSQRT, result, op, &New_OPs);
  } else {
    TN *op = Expand_Expr (denum, recip, NULL);
    Exp_OP1 (WN_opcode (recip), result, op, &New_OPs);
  }  
  return result;
}
#endif  /* TARG_NVISA */
...
#ifdef TARG_IA64
  // if have pairs of true/false predicates that follow each other:
...
#endif
{
  ...
  if (OPCODE_has_sym(opcode) && WN_st(expr) != NULL) {
    ...
    Set_BE_ST_referenced (WN_st(expr));
  }
...
#ifdef TARG_NVISA
  case OPR_RECIP:
    return Handle_RECIP (expr, result);
#endif
...
// num_exits is still 0, so we need the extra check.
	    ...
	    // may be bug in machine description, so warn
	    DevWarn("no br_op for conditional branch?");
...
Handle_CONDBR (WN *branch)
{
  ...
  if (variant != V_BR_NONE) {
    ...
    */
    ...
    Is_True(   (WN_operator_is(condition, OPR_LDID) 
	        && WN_class(condition) == CLASS_PREG)
               || WN_operator_is(condition, OPR_LAND)
               || WN_operator_is(condition, OPR_LIOR),
	       ("MTYPE_B TRUEBR/FALSEBR condition must be preg or relop"));
#else
      Is_True(   WN_operator_is(condition, OPR_LDID) 
	      && WN_class(condition) == CLASS_PREG,
...
// This function handles ASM statements using the support
// for OPs with variable numbers of results and operands. 
...
Handle_ASM (const WN* asm_wn)
{
  ...
  CGTARG_Init_Asm_Constraints(asm_wn);
...
#ifdef TARG_NVISA
  VARIANT variant = V_NONE;
  if (INTRN_is_atomic(id) && WN_num_actuals(intrncall) > 0) {
    // Determine the memory space of the first argument of the atomic
    // intrinsic. This memory space is used by Exp_Intrinsic_Call to
    // determine the memory space of the atomic instruction.
    WN *kid0 = WN_kid0(intrncall);
    Is_True(WN_operator(kid0) == OPR_PARM, ("Unexpected intrinsic argument"));
    kid0 = WN_kid0(kid0);
    // Query ptrclass analysis.
    enum PtrClasses mspace = ILSC->Query(kid0);
    // Check for global and shared memory spaces.
    if (mspace == Global)
      Set_V_global_mem(variant);
    else if (mspace == Shared)
      Set_V_shared_mem(variant);
  }
#endif
...
#ifdef TARG_NVISA
  result = Exp_Intrinsic_Call (id, variant,
...
#ifdef TARG_NVISA
            // If is return-value tn, replace with result.
            // This is more general than the v0/f0 hack below,
            // and could be done for all targets, but will let them decide
            // if they want to switch to this style.
            // Another alternative is to use the dedicated return-value tn 
            // in the intrinsic call, but then later code that changes 
            // dedicated param tns into param operations needs to avoid this.
            && REGISTER_SET_MemberP(
              REGISTER_CLASS_function_value(TN_register_class(otn)),
              TN_register(otn))
#else
...
void Whirl2ops_Finalize(void)
{
  ...
  if (last_loop_pragma && 
      ...
      CG_opt_level > 1) {
#ifdef TARG_NVISA
    if (WN_pragma(last_loop_pragma) == WN_PRAGMA_UNROLL)
      DevWarn("Pragma UNROLL not followed by a loop, ignored");
    else
#endif
...
#ifdef TARG_NVISA
PREG_NUM
Last_Actual_Param_Used (ISA_REGISTER_CLASS rc)
{
  return last_actual_param_used[rc];
}

PREG_NUM
Last_Actual_Retval_Used (ISA_REGISTER_CLASS rc)
{
  return last_actual_retval_used[rc];
}
#endif
