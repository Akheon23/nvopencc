...
#ifdef TARG_NVISA
  if (OP_like_barrier(pred_op) || OP_like_barrier(succ_op)) {
    if (definite) *definite = FALSE;
    return TRUE;
  }

  // don't alias if from different memory spaces
  if (OP_memory(pred_op) && OP_memory(succ_op)
    && TN_enum(OP_Space_TN(pred_op)) != TN_enum(OP_Space_TN(succ_op))) 
  {
    if (definite) *definite = FALSE;
    return FALSE;
  }
...
#ifdef TARG_NVISA
  // memory operations do not alias if different memory spaces
  if (TN_enum(OP_Space_TN(memop1)) != TN_enum(OP_Space_TN(memop2))) {
    return FALSE;
  }
#endif // NVISA
