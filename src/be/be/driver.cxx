...
extern struct ARRAY_DIRECTED_GRAPH16 *Current_Dep_Graph;
FILE *DFile = stderr;
...
Do_WOPT_and_CG_with_Regions (PU_Info *current_pu, WN *pu)
{
	...
	if (Get_Trace(TP_REGION,TT_REGION_ALL)) {
	  ...
	  RID_WN_Tree_Print(TFile,rwn);
        ...
        PU_adjust_addr_flags(Get_Current_PU_ST(), rwn, TRUE);
        ilsc = new PTR_CLASS();
        ilsc->Compute(rwn);
#endif
...
#ifdef TARG_NVISA
          compiled_block = CG_Generate_Code(rwn, alias_mgr, ilsc, tmp_idx, TRUE);
#else //TARG_NVISA
...
#ifdef TARG_NVISA
    CG_Generate_Code(pu, alias_mgr, ilsc, PU_Info_pu_dst(current_pu), FALSE);
#else //TARG_NVISA
...
#ifdef TARG_NVISA
    // handle synchronous directive
    Process_Synchronous(pu);
#endif
...
#ifdef KEY // bug 7741
    if ((Run_lno || Run_preopt) && Run_wopt) {
