...
void
STMT_ITER::Print_src( FILE *fp )
{
  for (First(); !Is_Empty(); Next()) {
    if (OPCODE_is_scf(WN_opcode(Cur())))
      fdump_wn_src_no_st( fp, Cur() );
    else
      fdump_tree_src_no_st( fp, Cur() );
  }
}
...
#ifdef TARG_NVISA
// Check if the current PU is a __global__ CUDA function.
// For separate compilation, we could see __device__ routine,
// for such routine, need to fall-into conservative
// for some analysis and transformations.
BOOL
Current_PU_is_Global(void)
{
  ST *pu_st = Get_Current_PU_ST();
  if (ST_memory_space(pu_st) == MEMORY_GLOBAL) {
    return TRUE;
  } else {
    return FALSE;
  }
