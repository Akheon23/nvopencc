...
CODEREP::Print(INT indent, FILE *fp) const
{
  ...
  if (Kind() == CK_IVAR) {
    ...
    if (Kind () == CK_VAR) {
      ST* st = Get_current_comp_unit()->Opt_stab()->St (Aux_id());
      if (st && ST_class(st) != CLASS_PREG) {
        fprintf (fp, " #%s", ST_name(st));
      }
    } else if (Kind() == CK_LDA) {
      fprintf (fp, " #%s", Get_current_comp_unit()->Opt_stab()->St_name(Lda_aux_id()));
    }
    fprintf(fp, "\n");
  }
}
...
{ 
  ...
  if (exist_cr == NULL) {
    ...
    if (need_cvt == NEED_CVT) {
      // convert to input type of cvt
      exist_cr = exist_cr->Fixup_type(OPCODE_desc(opc), this);
    }
    else { // NEED_CVTL
      // convert to result type of cvtl
      exist_cr = exist_cr->Fixup_type(OPCODE_rtype(opc), this);
    }
#endif
    new_cr->Init_expr(opc, exist_cr);
...
#ifdef TARG_NVISA
    CODEREP *ret_cr = Hash_Op(new_cr)->Fixup_type(cr->Dtyp(), this);
    FOLD ftmp;
    // We may have generated several CVTs, see if these can be folded.
    CODEREP *folded = ftmp.Fold_Expr(ret_cr);
    return folded ? folded : ret_cr;
#else
    return Hash_Op(new_cr);
#endif
  }
...
#ifdef TARG_NVISA
      // Make type signed as a NEG is being generated.
      MTYPE rtype = Mtype_TransferSign(MTYPE_I4, OPCODE_rtype(op));
#else
      MTYPE rtype = OPCODE_rtype(op);
#endif
      ccr->Set_tree(Add_unary_node(
           OPCODE_make_op(OPR_NEG, rtype, MTYPE_V), kid1.Tree()));
    }
    return propagated;
  }
  if (kid1.Tree() == NULL) {
		 ...
		 UINT field_id, BOOL is_store)
{
  ...
  // when the version is -1, it is for clone an original CR, which
  // should not change the original dsctyp and dtyp
  if (version != -1 && 
     is_store && dtyp!=dsctyp && dsctyp != MTYPE_BS) {
			     ...
			     MTYPE to_dsctyp, TY_IDX to_ty, UINT field_id)
{
  ...
  if ( Is_flag_set(CF_MADEUP_TYPE) || Dsctyp() == MTYPE_UNKNOWN ) {
	...
	Need_load_type_conversion( is_sign_extd, to_sign_extd, 
      ...
      CODEREP *new_cr = Alloc_stack_cr(0);
#ifdef TARG_NVISA
      // Need to add necessary type conversions before any cvtl is applied
      // to cover cases like when a {I4,U4}{dsctyp}LDID is propagated to a 
      // {I8,U8}{to_dsctyp}LDID, so that the dtyp of the cvtl matches the
      // dtyp of its kid in size. The type fixup at the bottom of this
      // function is too late for the case when a cvtl is applied.
      if (need_cvt == NEED_CVTL)
        retval = retval->Fixup_type(to_dtyp, htable);
      IncUsecnt();
      // use the CR after type fix up above
      new_cr->Init_expr( opc, retval );
#else
      IncUsecnt();
  ...
  if ( retv == NULL ) {
	...
	MTYPE_size_min(dsctyp) > MTYPE_size_min(dtyp)) {
      ...
      if (dsctyp == MTYPE_BS) {
        ...
        if (v->Kind() == CK_OP) {
#ifdef TARG_NVISA
          Is_True((v->Opr()==OPR_CVT || v->Opr()==OPR_CVTL),
                  ("CODEMAP::Cur_def: unexpected op"));
#endif
          v = v->Opnd(0);  // skip the CVTL inserted earlier
        }
#ifdef TARG_NVISA
        // Var_type_conversion can introduce a CVT below the CVTL, so skip
        // this CVT if inserted.
        if (v->Kind() == CK_OP) {
          Is_True((v->Opr()==OPR_CVT || v->Opr()==OPR_CVTL),
                  ("CODEMAP::Cur_def: unexpected op"));
          v = v->Opnd(0);
            ...
            if (WOPT_Enable_CRSIMP &&
                ...
                retv_op->Get_opnd(1)->Kind() == CK_CONST) {
                  ...
                  if (retv->Opr() == OPR_CVT) {
                    ...
                    // create new node rather than reuse old CVT node,
                    // else later use of coderep will refer to wrong value
                    // (cvt no longer includes the add).
	            cr = Add_unary_node(
                          OPCODE_make_op(OPR_CVT, retv->Dtyp(), retv->Dsctyp()),
                          retv_op->Get_opnd(0));
                  }
                  else
#endif
...
#ifdef TARG_NVISA
    CODEREP *retv_ivar = retv;
    while (retv_ivar->Kind() != CK_IVAR)
      retv_ivar = retv_ivar->Opnd(0);
#else
...
CODEREP::Check_if_result_is_address(OPT_STAB *opt_stab) const
{
  ...
  case CK_VAR: {
    ...
    TY_IDX ty = Lod_ty();
    // Need to take the type of the field if a struct
...
// is FALSE
//
static void (*print_cr_var_info)(FILE *, const CODEREP *);

void
set_print_cr_var_info (void (*new_print_cr_var_info)(FILE *, const CODEREP *)) 
{
  print_cr_var_info = new_print_cr_var_info;
}

void
CODEREP::Print_src(INT indent, FILE *fp) const
{
  INT i;
  CODEREP *base;
  ...
  case CK_LDA:
    fprintf (fp, "&%s_%d", 
             Get_current_comp_unit()->Opt_stab()->St_name(Lda_aux_id()),
             Lda_aux_id());
    return;

  case CK_CONST:
    fprintf (fp, "%lld", Const_val());
    return;
	
  case CK_RCONST:
    fprintf (fp, "const_%d", ST_st_idx(Const_id()));
    return;

  case CK_VAR:
	if (print_cr_var_info != NULL) {
      (*print_cr_var_info)(fp, this);
	} else {
      fprintf (fp, "cr%d", Coderep_id());
	}
	return;
  ...
  case CK_IVAR:
    ...
    case OPR_PARM:
      if (Ilod_base()) {
        Ilod_base()->Print_src(indent, fp);
      } else {
        Istr_base()->Print_src(indent, fp);
      }
      if (Offset()) {
        fprintf(fp, " + %d", Offset());
      }
    ...
    default:
      fprintf(fp, "un-supported IVAR operator");
      break;
    }

    if (Ivar_mu_node() && Ivar_mu_node()->OPND()) {
      fprintf (fp, "mu_%d/cr%d ", Ivar_mu_node()->OPND()->Aux_id(), 
               Ivar_mu_node()->OPND()->Coderep_id());
    } else {
      fprintf (fp, "mu_cr%d ", Coderep_id());
    }
    return;
  }
  ...
  op_name = get_operator_info (Opr(), &opnds, &is_prefix);
  ...
  if (op_name != NULL) {
    ...
    if (opnds == 2) {
      if (is_prefix) {
        ...
        Get_opnd(0)->Print_src(indent+1, fp);
        fprintf(fp, ", ");
        Get_opnd(1)->Print_src(indent+1, fp);
        fprintf(fp, ")");
      } else {
        extern BOOL Need_extra_brace(OPERATOR opr, OPERATOR opr2);

        BOOL need_extra_brace = Get_opnd(0)->Kind() == CK_OP && 
                                Need_extra_brace(Opr(), Get_opnd(0)->Opr());
        ...
        need_extra_brace = Get_opnd(1)->Kind() == CK_OP && 
                           Need_extra_brace(Opr(), Get_opnd(1)->Opr());
        if (need_extra_brace) {
          fprintf(fp, "(");
        }
        Get_opnd(1)->Print_src(indent+1, fp);
  ...
  switch (Opr()) {
    ...
    case OPR_CSELECT:
      ...
      Get_opnd(1)->Print_src(indent+1, fp);
      fprintf(fp, " : ");
      Get_opnd(2)->Print_src(indent+1, fp);
      return;
    ...
    case OPR_ARRAY:
    {
      int kid_count = Kid_count();
      int ndim = kid_count/2;
      int i;
      Get_opnd(0)->Print_src(indent+1, fp);
      for (i = 1; i <= ndim; i++) {
        fprintf(fp, "[");
        Get_opnd(ndim+i)->Print_src(indent+1, fp);
        fprintf(fp, "]");
      }
      return;
    }
    ...
    {
      int kid_count = Kid_count();
      fprintf(fp, "%s(", INTRINSIC_name((INTRINSIC) Intrinsic()));
      for (int i = 0; i < kid_count; i++) {
        Get_opnd(i)->Print_src(indent+1, fp);
        fprintf(fp, ", ");
      }
      fprintf(fp, ")");
      return;
    ...
    case OPR_ASM_INPUT:
      fprintf(fp, "      ASM_INPUT: ");
      Get_opnd(0)->Print_src(indent+1, fp);
      fprintf(fp, " : %s\n", ST_name(Asm_constraint()));
      return;
    ...
    default:
      fprintf(fp, "un-supported operator %s", OPERATOR_name(Opr()));
      break;
  }

  Print_node_src(indent, fp);
}


void
CODEREP::Print_node_src(INT32 indent, FILE *fp) const
{
  INT i;
  switch (Kind()) {
  case CK_OP:
...
void
STMT_LIST::Print_src(FILE *fp) const
{
  STMTREP_CONST_ITER  stmt_iter(this);
  const    STMTREP   *tmp;
  INT32               last_line = 0;
...
//
void
STMTREP::Print_src(FILE *fp) const
{
  if (_opr == OPR_PRAGMA) {
    WN *wn = Orig_wn();
    fprintf(fp, "       ");
    extern void print_pragma(WN *, FILE *);
    print_pragma (Orig_wn(), fp);
    return;
  }
...
//
void
STMTREP::Print_node_src(FILE *fp) const
{
  int bno;
  ...
  case OPR_TRUEBR:
    fprintf(fp, "    ");
    fprintf(fp, "if (IS_TRUE(");
    Rhs()->Print_src (1, fp);
    fprintf(fp, ")) ");
    bno = get_block_number_on_label(Label_number());
    fprintf(fp, "goto BLOCK %d (L%d); \n", bno, Label_number());
    fflush(fp);
    break;

  case OPR_FALSEBR:
    fprintf(fp, "    ");
    fprintf(fp, "if (IS_FALSE(");
    Rhs()->Print_src (1, fp);
    fprintf(fp, ")) ");
    bno = get_block_number_on_label(Label_number());
    fprintf(fp, "goto BLOCK %d (L%d); \n", bno, Label_number());
  ...
  {
    fprintf(fp, "    ");
    fprintf(fp, "ASM_STMT %s\n", ST_name(Asm_string_idx()));
    vector<CLOBBER_PRAGMA_INFO,
           mempool_allocator<CLOBBER_PRAGMA_INFO> >::const_iterator p;
    for (p = Asm_pragma()->clobber_info.begin();
         p != Asm_pragma()->clobber_info.end();
         ++p) {
      fprintf(fp, "      ASM_CLOBBER, %s, preg:%d\n",    
              ST_name((ST_IDX)p->clobber_string_idx),
              p->preg_st_idx);
    }
    vector<CONSTRAINT_PRAGMA_INFO, 
	  mempool_allocator<CONSTRAINT_PRAGMA_INFO> >::const_iterator q;
    for (q = Asm_pragma()->constraint_info.begin();
         q != Asm_pragma()->constraint_info.end();
         ++q) {
      fprintf(fp, "      ASM_CONSTRAINT, %s, opnd %d, preg:%d\n",    
              ST_name((ST_IDX)q->constraint_st_idx),
              q->asm_opnd_num,
              q->asm_neg_preg);
    }
    Rhs()->Print_src(1, fp);
