...
#ifdef TARG_NVISA
    INT32 _visited;           // used for traversing control flow
#endif
  } _u11;
  union {
...
public:
  ...
  // interface for retrieve the visited field
  INT32 Get_visited() { return _u11._visited; }

  // interface for setting the visited field
  void Set_visited(INT32 visited) { _u11._visited = visited; }

  // Check if a BB_NODE belongs to a loop
  BOOL In_loop (BB_LOOP *loop) 
  {
    BB_LOOP *parent = Innermost();
    while (parent != NULL) {
      if (parent == loop) {
        return TRUE;
      }
  ...
  void         Print_aux(FILE *fp=stderr) const;// print the bb structure where AUX_TABLE is available
                                                // but CODEREP is not available
  void         Print_cr(FILE *fp=stderr) const;// print the bb structure where CODEREP is available
  void         Print_src(void) const;         // print the bb structure and the body in C style
  void         PrintVis(void) const;            // print the bb structure
  void         Print_wn(FILE *fp=stderr) const;  // print the bb structure
  void         Print_wn_src (FILE *fp=stderr) const;  // print the bb body in C style
  void         Print_ssa(FILE *fp=stderr) const; // print the bb structure
  void         Print_ssa(void);                  // for workaround VC8 problem
  void         Print(void);                      // for workaround VC8 problem
  void         Print_head(FILE *fp=stderr) // print the cfg info of the bb
               const;
  void         Print_head_src (FILE *fp) const;  // print brief head information
