...
public:
  ...
  // Same as above, but operates on the CFG without the STMTREPs.
  // Directly modifies the underlying WHIRL representation.
  void Remove_critical_edges_wn(void);
  // Adds an empty bb on the edge v->w. So v->w becomes
  // v -> empty_bb -> w.
  void Insert_empty_bb_wn(BB_NODE *v, BB_NODE *w);
