...
// $Source: /scratch/mee/Patch0002-taketwo/kpro64-pending/be/opt/SCCS/s.opt_dse.cxx $
//
// Revision history:
...
DSE::Same_memloc( WN* store1, WN* store2) const
{
  ...
  if (Opt_stab()->Rule()->Same_location(store1, store2, pt1, pt2)) {
    ...
    if (TY_is_volatile(ST_type(pt1->Base())) 
        || TY_is_volatile(ST_type(pt2->Base()))) {
      // if the base symbol is volatile, need to fall into conservative.
      // The main reason is that the volatile varibale is not placed 
      // into MU or CHI, and load/iload do not hold any information
      // about aliased variables, which leave a problem for the following
      // case,
      //           volatile res;
      //           ...
      //           x = &res;
      ...
      //           res = res + 1;
      //
      // The points_to information tells that *x and res have same
      // memory location, but since res is volatile, and is not in 
      // the chi list of ISTORE *x, there is no chance for 
      // the istore to be marked live.
      // 
      // Similarly, it would not work for cases like,
      //
      //           volatile res;
      //           ...
      //           x = &res;
