...
#ifdef TARG_NVISA
#include "opt_sync.h"			/* Removing redundant syncrhonization */
#endif
...
extern void
Perform_Procedure_Summary_Phase (WN* w, DU_MANAGER *du_mgr,
...
private:
  ...
  BOOL  _limited_preopt_copy_prop;
  BOOL  _lego_opt;
  BOOL  _load_pre;
  BOOL  _local_rvi;
  BOOL  _ocopy;
  ...
  void Adjust_Optimization(void) {
...
#ifdef TARG_NVISA
    case PREOPT_CMC_PHASE2:
      Allow_wrap_around_opt = FALSE;
      WOPT_Enable_Zero_Version = FALSE;
      WOPT_Enable_Call_Zero_Version = FALSE;
      WOPT_Enable_Dead_Sync_Elim = 0;
      break;
    case PREOPT_CMC_PHASE:
      ...
      WOPT_Enable_Copy_Propagate = TRUE;
      WOPT_Enable_Limited_Preopt_Copy_Prop = FALSE;
      WOPT_Enable_Dead_Sync_Elim = 0;     // temporarily disable this
      WOPT_Enable_IVR = FALSE;            // disable loop normalization
      break;
#endif
    ...
    if ((_phase != MAINOPT_PHASE 
         || !WOPT_Enable_New_Phase_Ordering) 
#ifdef TARG_NVISA
        && _phase != PREOPT_CMC_PHASE2
#endif
       )
    {
  ...
  void Unadjust_Optimization(void) {
    ...
    case PREOPT_CMC_PHASE:
      ...
      WOPT_Enable_DU_Full = _du_full;
      WOPT_Enable_Copy_Propagate = _copy_propagate;
      WOPT_Enable_Limited_Preopt_Copy_Prop = _limited_preopt_copy_prop;
      WOPT_Enable_Dead_Sync_Elim = _dead_sync_elim;
      WOPT_Enable_IVR = _ivr;
      break;
#endif
    case PREOPT_LNO_PHASE:
...
public:
  WOPT_SWITCHES(OPT_PHASE phase, INT32 pragma_flags, BOOL disable_parm_alias) {
    ...
    _limited_preopt_copy_prop = WOPT_Enable_Limited_Preopt_Copy_Prop;
...
/************************************************************
 * Inline functions to enhance readability of caller
 ***********************************************************/

static inline void 
Live_Range_Verify(COMP_UNIT *comp_unit)
{
#ifdef Is_True_On  
  if (Get_Trace(TKIND_INFO, TINFO_TIME)) {
    SET_OPT_PHASE("Skip verify Live-Range because timing trace is on");
...
static inline void 
Codemap_Verify(COMP_UNIT *comp_unit) 
{
#ifdef Is_True_On  
  SET_OPT_PHASE("Verify CODEMAP");
  if (Get_Trace(TKIND_INFO, TINFO_TIME)) 
...
static inline void 
IR_Verify(COMP_UNIT *comp_unit, int a, const char* b)
{
#ifdef Is_True_On
  Is_True(comp_unit->Verify_IR(comp_unit->Cfg(), comp_unit->Htable(), a),
	  (b));
#endif
}

static inline void 
Feedback_Verify(COMP_UNIT *comp_unit, const char *a)
{
  if ( comp_unit->Cfg()->Feedback() )
    comp_unit->Cfg()->Feedback()->Verify( comp_unit->Cfg(), a);
}

static void 
Do_Pre_Before_Ivr(COMP_UNIT *comp_unit, OPT_PHASE phase)
{
  ...
  {
#ifdef TARG_NVISA
    if (phase == PREOPT_CMC_PHASE2) {
      BOOL temp_WOPT_Enable_LFTR2 = WOPT_Enable_LFTR2;
      BOOL temp_WOPT_Enable_New_SR = WOPT_Enable_New_SR;
      WOPT_Enable_LFTR2 = FALSE;
      WOPT_Enable_New_SR = FALSE;
      {
        SET_OPT_PHASE("SSA PRE before RVI");
        comp_unit->Do_comic_pre();
        if (Get_Trace(TP_GLOBOPT, CR_DUMP_FLAG)) {
          comp_unit->Htable()->Print(TFile);
        }
        Verify_Codemap(comp_unit);
      }
      WOPT_Enable_LFTR2 = temp_WOPT_Enable_LFTR2;
      WOPT_Enable_New_SR = temp_WOPT_Enable_New_SR;
    } else {
#endif
    SET_OPT_PHASE("SSA PRE before RVI");
...
Pre_Optimizer(INT32 phase, WN *wn_tree, DU_MANAGER *du_mgr,
	      ALIAS_MANAGER *alias_mgr)
{
  if (WOPT_Enable_Dump_WN_IR_Before_Preopt) {
    dump_tree_src(wn_tree);
  }
  ...
  // NOTE: this is done like this to satisfy gcc checking which prohibits
  // an ifdef inside a macro (i.e. Is_True)
#ifndef TARG_NVISA
...
#ifdef TARG_NVISA
    LabelOpt::Remove_Unreferenced_Labels(wn_tree);
#endif
    WN_unroll(wn_tree);
#endif
...
#ifdef TARG_NVISA
    // in CMC_PHASE2 lower arrays to get PRE on High-Whirl
    if (phase == PREOPT_CMC_PHASE2) {
      lower_flags = LOWER_ARRAY;
    }
#endif
...
#ifdef TARG_NVISA
  // control the propagation of restrict pointers based on
  // the phases 
  Set_propagate_restrict_pointer (phase == MAINOPT_PHASE);
#endif
...
// create aux symbol table
  ...
  /* NB: We use CXX_OPT_POOL (see be/opt/opt_defs.h) instead of an ordinary MEM_POOL so that
     the deletion of the memory pool is delayed until function exit, _after_ the 'ac' object
     itself is destroyed.  We no longer call OPT_POOL_Initialize() or OPT_POOL_Delete(). */
  CXX_OPT_POOL alias_class_pool("Alias classification pool", FALSE, MEM_DUMP_FLAG+20);
  ...
  if (Cur_PU_Feedback) {
    ...
    Feedback_Verify(comp_unit, "after CFG Annotation");

    // TODO: Perhaps clear out Cur_PU_Feedback now?
  }

// Opt_sync mark
// Has to be done before the symbol table mangling happens.  
 
#ifdef TARG_NVISA
  if (WOPT_Enable_Dead_Sync_Elim)
  {
    SET_OPT_PHASE("Dead Sync Elimination");
    DSyncE sync_elim(comp_unit->Cfg(), comp_unit->Opt_stab());
    if(WOPT_Enable_Dead_Sync_Elim == 1)
      sync_elim.Do_dead_sync_elim_new();
    else if(WOPT_Enable_Dead_Sync_Elim == 2)
      sync_elim.Do_dead_sync_elim();
  }
#endif
  ...
  //  Copy_Restricted_Map(comp_unit->Input_tree(), alias_mgr);
  ...
  if (WOPT_Enable_Dump_SSA_IR_Before_Create_SSA) {
    print_cfg_aux ();
  }

  SET_OPT_PHASE("Create CODEMAP Representation");
  comp_unit->Ssa()->Create_CODEMAP();
  Live_Range_Verify(comp_unit);

  if (WOPT_Enable_Dump_SSA_IR_After_Create_SSA) {
    print_cfg_cr ();
  }
  ...
  IR_Verify(comp_unit, 1, "Verify CFG wrong after Htable");

#ifdef TARG_NVISA
  // on CMC_PHASE2, for multicore, we have transformed CUDA
  // program to have thread-loops etc. so as for the IR to
  // now represent a regular C program. The following runs
  // PRE on this code to hoist invariants and potentially
  // allow vectorization by host compiler
  if (phase == PREOPT_CMC_PHASE2) {
    Do_Pre_Before_Ivr(comp_unit, (OPT_PHASE) phase);
  } 

  // on gpu path want to do partial preopt for unrolling, 
  // but not full preopt (need to create cfg for unroller,
  // and codemap for emitter).

  else if (phase == MAINOPT_PHASE || phase == PREOPT_CMC_PHASE) {
#endif
  ...
  //
  // 1. IV recognition
  // 2. copy-prop/forward-sub-1
  // 3. bool-simplification
  // 4. DCE-1
  ///////////////////////////////////////////////////
  ...
  IR_Verify(comp_unit, 2, ("Verify CFG wrong after copy prop"));
  ...
  if (WOPT_Enable_DCE) {
    ...
    Feedback_Verify(comp_unit, "after Dead Code Elimination");
  }

  if (WOPT_Enable_Dump_SSA_IR_After_OPT_1) {
    print_cfg_cr ();
  }
...
#ifdef KEY 
  // moved here because renaming causes bad code when there are
  ...
  if (WOPT_Enable_Dump_SSA_IR_After_OPT_2) {
    print_cfg_cr ();
  }
  ...
  // 1. CFG-Opt
  // 2. Rename_CODEMAP
  // 3. copy-prop/forward-sub - 2
  // 4. DCE-2 (break if this removes no paths)
  // 5. Fold_Lda_ILoad -2
  ///////////////////////////////////////////////////
  ...
  for (INT i = 0; i < WOPT_Enable_Extra_Rename_Pass; ++i) {
    ...
    if (WOPT_Enable_CFG_Opt && phase == MAINOPT_PHASE) {
      ...
      Feedback_Verify(comp_unit, "after CFG Optimization");
    }
    ...
    if (WOPT_Enable_DCE) {
      ...
      Feedback_Verify(comp_unit, "Dead Code Elimination");
  ...
  IR_Verify(comp_unit, 3, ("Verify CFG wrong after extra passes"));

  if (WOPT_Enable_Dump_SSA_IR_After_OPT_3) {
    print_cfg_cr ();
  }
  ...
  // 2. Write-Once-Variable-Promotion
  ///////////////////////////////////////////////////
  ...
  if (WOPT_Enable_Edge_Placement && phase == MAINOPT_PHASE) {
    ...
    Feedback_Verify(comp_unit, "after Edge Placement" );
  }

  IR_Verify(comp_unit, 4, ("Verify CFG wrong after dce"));
  Codemap_Verify(comp_unit);
  Live_Range_Verify(comp_unit);

  if (WOPT_Enable_Dump_SSA_IR_After_OPT_4) {
    print_cfg_cr ();
  }
  ...
  if(WOPT_Enable_WOVP){
  ...
  else if (WOPT_Enable_Limited_Preopt_Copy_Prop) {
    // under this option we want to do the following 3 opts
    // the copy_prop is changed to propagate only address
    // loads in pre_opt. The dead code can then remove 
    // certain address saves, iload-lda changes lda's to ldid
    // and before the main-opt phase, address save flags
    // can be cleared, allowing register promotion
    ...
    IR_Verify(comp_unit, 3, ("Verify CFG wrong after dce"));
  ...
  if ( phase == MAINOPT_PHASE ) {
	...
	if ( phase == MAINOPT_PHASE ) {
	  comp_unit->Do_vnfre(TRUE/*before_epre*/);
        ...
        if (WOPT_Enable_Expression_Reshaping) {
          SET_OPT_PHASE("Expression Reshaping");
          comp_unit->Do_expression_reshaping();
        }
        
        IR_Verify(comp_unit, 5, ("Verify CFG wrong after reshape"));
        Codemap_Verify(comp_unit);
        Live_Range_Verify(comp_unit);    
	...
	comp_unit->Do_vnfre(FALSE/*before_epre*/);
        ...
        IR_Verify(comp_unit, 6, ("Verify CFG wrong after pre/vnfre"));
        Codemap_Verify(comp_unit);
        Live_Range_Verify(comp_unit);
      }
	...
	Feedback_Verify(comp_unit, "after Dead Code Elimination 2");
      }

      if (WOPT_Enable_Dump_SSA_IR_After_OPT_5) {
        print_cfg_cr ();
      }
      ...
      if (WOPT_Enable_Dump_SSA_IR_After_LPRE) {
        print_cfg_cr ();
      }
	...
	comp_unit->Htable()->Verify_var_phi_hash();
	  ...
	  if (WOPT_Enable_Dump_SSA_IR_After_SPRE) {
       print_cfg_cr ();
      }
      ...
      if (WOPT_Enable_Dump_SSA_IR_After_BIT_DCE) {
        print_cfg_cr ();
      }
    ...
    if (WOPT_Enable_RVI) {
      // Hacky invocation of new PRE RVI hooks for testing. 
    ...
    IR_Verify(comp_unit, 7, "Verify CFG wrong after creating RVI instance");

    ////////////////////////////////////////////////////
    // 
    // Statistics and Emission
    ///////////////////////////////////////////////////
    ...
    Feedback_Verify(comp_unit, "before MainOpt emitter");

    if (comp_unit->Cfg()->Feedback() &&
        comp_unit->Cfg()->Feedback()->Trace())
      comp_unit->Cfg()->Feedback()->Print( TFile );

    if (WOPT_Enable_Dump_SSA_IR_Before_Emit) {
      print_cfg_cr ();
    }

#ifdef TARG_NVISA
    if (WOPT_Enable_Memory_Space_Based_Alias_Analysis) {
      // For MAINOPT phase, the WN_memory_space_MAP is no longer
      // needed
      Delete_Current_WN_Memory_Space_Map ();
    }

    if (WOPT_Enable_LDU_Analysis) {
      CR_VarianceMap *cr_var_map = new CR_VarianceMap (comp_unit->Htable());
      cr_var_map->Setup_coderep_tid_var_info(); 

      CR_Region_LDU_INFO *cr_region_info = NULL;
      if (WOPT_Enable_Region_LDU_Analysis) {
        cr_region_info = new CR_Region_LDU_INFO(comp_unit->Htable());
      }   
      opt_wn = comp_unit->Emit_ML_WHIRL(&rvi);
      delete cr_var_map;
      if (cr_region_info != NULL) {
        delete cr_region_info;
      }
    } else {
      opt_wn = comp_unit->Emit_ML_WHIRL(&rvi);
    }

    if (Target_ISA >= TARGET_ISA_compute_20 && WOPT_Enable_Cons2glb > 0) {
      // promote __constant__ to __global__. this should not be moved before Emit_ML_WHIRL
      ST *st;
      int i;
    ...
    // we are free to do non-ssa based optimizations on
    // opt_wn here.
    if(WOPT_Enable_PDE) {
      SET_OPT_PHASE("PDE");

      if (Get_Trace(TP_WOPT3, PDE_FLAG)) {
        fprintf(TFile, "%sBefore PDE%s\n", DBar, DBar);
        fdump_tree(TFile, opt_wn);
      }

      opt_wn = Perform_PDE(opt_wn, alias_mgr);

      if (Get_Trace(TP_WOPT3, PDE_FLAG)) {
        fprintf(TFile, "%sAfter PDE%s\n", DBar, DBar);
        fdump_tree(TFile, opt_wn);
      }
    }
#endif
...
#ifdef TARG_NVISA
    // Create memory space map before emitting, and
    // the information will be saved in current_wn_memory_space_map
    // during emitting, and the information will be used in the next
    // round of alias analysis
    if (WOPT_Enable_Memory_Space_Based_Alias_Analysis) {
      Create_Current_WN_Memory_Space_Map (comp_unit->Htable(), MEM_pu_pool_ptr);
    }
#endif
...
#ifdef TARG_NVISA
    if (WOPT_Enable_Memory_Space_Based_Alias_Analysis) {
      // The information has reflected in the current_wn_memory_space_map,
      // so, the CR_MemoryMap is no longer needed.
      // Delete current CR_MemoryMap
      Delete_Current_CR_Memory_Map ();
    }
#endif
	...
	!REGION_has_black_regions(comp_unit->Rid())) {
      ...
      /* The ALIAS_CLASSIFICATION constructor will call OPT_POOL_Push();
         the Relase_resources() method will call OPT_POOL_Pop().  */
      ALIAS_CLASSIFICATION ac(comp_unit->Opt_stab(),
  ...
  if (WOPT_Enable_Dump_WN_IR_After_Preopt) {
    dump_tree_src(opt_wn);
  }
