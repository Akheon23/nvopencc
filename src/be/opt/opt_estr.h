...
#ifdef TARG_NVISA
  BOOL in_ivar;            // for information about if the occurance within an IVAR  
#endif

} EXPRESSION_SR_OCCURS;
...
private:
  ...
  DYN_ARRAY<EXPRESSION_SR_OCCURS> _iv_occurs;
  DYN_ARRAY<IV_SR_RECORD> _iv_occurs_record;
  ...
  // in BB.
  BOOL Is_const_or_loop_invar(const CODEREP *expr, BB_NODE *bb) const;
  ...
  // management of iv expression array
  void     Clear_iv_expression_occurs();
#ifdef TARG_NVISA
  void     Add_iv_expression_occurs(const CODEREP *cr, BB_NODE *bb, BOOL in_ivar);
  BOOL Collect_expression(const CODEREP *cr, const CODEREP *iv, BB_NODE *bb, BOOL in_ivar);
#else
  void     Add_iv_expression_occurs(const CODEREP *cr, BB_NODE *bb);
  BOOL Collect_expression(const CODEREP *cr, const CODEREP *iv, BB_NODE *bb);
#endif
  EXPRESSION_SR_OCCURS Get_iv_expression_occurs(INT i)  { return _iv_occurs[i]; }
  mINT32   Iv_expression_occurs_count(void) { return _iv_occurs.Lastidx()+1; }

  // management of iv record
  void         Add_iv_record(const CODEREP *iv, IV_SR_STATUS);
  IV_SR_RECORD Get_iv_record(INT i)  { return _iv_occurs_record[i]; }
  mINT32       Iv_record_count(void) { return _iv_occurs_record.Lastidx()+1; }
  IV_SR_STATUS Get_recorded_status(const CODEREP *iv);

  BOOL Collect_iv_occurs(const CODEREP *iv, BB_NODE *bb);
  BOOL Iv_can_disappear(const CODEREP *iv, BB_NODE *use_bb, const CODEREP *cr);
  BOOL Is_profitable(const CODEREP *iv, BB_NODE *use_bb, const CODEREP *cr);
...
public:
  ...
  STR_RED(CFG *cfg, CODEMAP *htable, MEM_POOL *pool, BOOL tracing) :
    ...
    _repaired_statements(pool), _iv_occurs(pool),
    _iv_occurs_record(pool), _lpool(pool)
      { };
...
#ifdef TARG_NVISA
  BOOL Check_expr_occs_post_dom_loop_header(BB_LOOP *loop, const CODEREP *expr_cr);
#endif
