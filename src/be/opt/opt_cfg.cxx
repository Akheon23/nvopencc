...
CFG::Is_simple_expr(WN *wn) {
  ...
  if (opr == OPR_CVT) {
      ...
      if (kidans == 0) return 0;
      return kidans;
  }
  if (opr == OPR_RECIP) {
      TYPE_ID ty = WN_rtype(wn);
      // do not predicate on any FP size greater than 32 bits
      if ((ty == MTYPE_F8) || (ty == MTYPE_F10) || (ty == MTYPE_F16))
          return 0;
      INT kidans = Is_simple_expr(WN_kid0(wn));
      return kidans;
  }
...
BOOL CFG::If_conv_criteria_met(WN* wn, WN* else_wn, WN* then_wn, BOOL empty_else, BOOL empty_then)
{
  ...
  if (
      ...
      (!empty_then &&
	   ...
	   (!empty_else || WN_rtype(WN_kid0(WN_first(then_wn))) == WN_desc(WN_first(then_wn)))) || 
...
void
CFG::Insert_empty_bb_wn(BB_NODE *v, BB_NODE *w)
{
  BB_NODE *new_bb;
  ...
  if(v->Next() == w) {
    ...
    WN *bb_branch = v->Branch_wn();
    Is_True(bb_branch != NULL, ("BB has no branch statement."));

    OPERATOR opr = WN_operator(bb_branch);
    // assocate the new_bb with a label and update the TRUEBR label 
    Is_True(w->Labnam() != 0, ("BB has no label."));
    LABEL_IDX new_label = Alloc_label();
    ...
    WN *new_labstmt = WN_CreateLabel(new_label, 0, 0);
    new_bb->Insert_wn_before(new_labstmt, 0);

    if(opr == OPR_TRUEBR || opr == OPR_FALSEBR || opr == OPR_GOTO)
      WN_label_number(bb_branch) = new_label;
    else {
      Is_True( opr == OPR_COMPGOTO,
	       ("don't know how to handle this branch kind.") );
      for (INT32 num_entry = 0; num_entry < v->Switchentries(); num_entry++) {
    ...
    BB_NODE *u = new_bb->Prev();
    if(w->Pred()->Contains(u)) {
      WN *u_branch = u->Branch_wn();
      if (u_branch == NULL && u->Kind() != BB_REGIONSTART &&
	  u->Kind() != BB_ENTRY) {
        WN *new_goto = WN_CreateGoto(w->Labnam());
        u->Insert_wn_after(new_goto, u->Laststmt());
      }
      else {
	if (u->Kind() == BB_REGIONSTART || u->Kind() == BB_ENTRY ||
	    WN_operator(u_branch) == OPR_TRUEBR ||
	    WN_operator(u_branch) == OPR_FALSEBR) {
          BB_NODE *new_u = Create_and_allocate_bb(BB_GOTO);
          u->Insert_After(new_u);
	  ...
	  new_u->Append_succ( w, Mem_pool() );
          ...
          WN *new_goto = WN_CreateGoto(w->Labnam());
          new_u->Insert_wn_after(new_goto, new_u->Laststmt());
        }
      }
    }
  }
  new_bb->Set_rvi_anns(0);
}

void 
CFG::Remove_critical_edges_wn(void)
{
  CFG_ITER cfg_iter(this);
  BB_NODE *bb;
  BOOL added = FALSE;
...
void CFG::PrintCDVis(FILE *file)
{
  CFG_ITER cfg_iter(this);
  BB_NODE * bb;
  fprintf(file, "digraph CD { \n");
  FOR_ALL_NODE(bb, cfg_iter, Init()) {
    BB_NODE * dep_node;
