...
// $Source: /scratch/mee/Patch0002-taketwo/kpro64-pending/be/opt/SCCS/s.opt_lftr2.cxx $
//
// ====================================================================
//
...
//Bug# 1164
...
#ifdef TARG_NVISA
  // If unsigned address comparisons are being done, should not do LFTR on
  // non-loop conditions because they are much more vulnerable to underflow.
  // Issue on NVISA because shared memory addresses are close to zero.
  if (WOPT_Enable_LFTR2_Unsigned_Address_Compare &&
      addressable == ADDRESSABILITY_IS_ADDRESS) {
    BB_KIND comp_kind = comp->Bb()->Kind();
    if (comp_kind != BB_DOEND && comp_kind != BB_WHILEEND) {
      Is_Trace(Trace(), (TFile, "LFTR::Replace_comparison: comparison "
			 "involves memory addresses but is not loop cond (is "
			 "%d): risky to perform", comp_kind));
      return;
    }
  }
#endif
  ...
  // create new rhs using tos expression as a template
  {
    ...
    if (WOPT_Enable_LFTR2_Unsigned_Address_Compare &&
	addressable == ADDRESSABILITY_IS_ADDRESS) {
      if (MTYPE_is_signed(new_compare_type))
