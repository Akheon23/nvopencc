...
//         hi = (( hi > ofst + size ) ?  hi : ofst + size ) ;
           ...
           hi = (( hi > (INT64)(ofst + size) ) ?  hi : (INT64)(ofst + size) ) ;
#endif /* linux */
...
#ifdef TARG_NVISA
      pt->Set_mem_kind(ST_get_memory_kind(st));
#endif
    }
...
#ifdef TARG_NVISA
  // for global routine, the shared/local memory space should not have effect at the end
  // of the routine
  if (Current_PU_is_Global()) {
    pt->Set_mem_kind ((MEMORY_KIND)(MEMORY_KIND_GLOBAL | MEMORY_KIND_CONST | MEMORY_KIND_GLB_CONST | MEMORY_KIND_PARAM));
  } else {
    // for device routine, fall-into conservative
    pt->Set_mem_kind (MEMORY_KIND_UNKNOWN);
  }
#endif
