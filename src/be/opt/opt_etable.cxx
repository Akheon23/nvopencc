...
// ===================================================================
// Skiplist is a list of codereps that we want to skip for EPRE for
// multicore target on NVIDIA.
// 
// For the loop conditional like:
//    EXPR-1
//    EXPR-2
//   OPR_LE
//  TRUEBR
//
//  Where EXPR-1,2 involves the loop index
//
// We note those expressions (codereps) in EXPR-1,2 which have both 
// children as leaves. Then we skip these expressions in the main
// PRE algorithm. This also means these don't get added from
// some other expression. Also note that only expressions with all
// children as leaves are operated on and thus PRE will never get to
// any of the higher expressions. This also means minimal storage
//
// These are then stored as a linked list off ETABLE class. This
// list is expected to be very very small in most cases
//
// ===================================================================

BOOL
ETABLE::Is_skip_candidate(STMTREP *stmt)
{
  BB_NODE * bb = stmt->Bb();

  BOOL skip = TRUE;
  
  skip &= (stmt->Op() == OPC_TRUEBR || 
           stmt->Op() == OPC_FALSEBR);
  skip &=  ((bb != NULL) &&
            (bb->Loop() != NULL) &&
            (bb->Loop()->Index() != NULL));
  ...
  if ((rhs == NULL) || !rhs->Non_leaf())
    return FALSE;

  const OPERATOR cond_opr = rhs->Opr();
  skip &=  (cond_opr == OPR_LE || 
            cond_opr == OPR_GE ||
            cond_opr == OPR_LT ||
            cond_opr == OPR_GT);
  ...
  // We have WN for index, try to find coderep for it in phi's
  WN *index = bb->Loop()->Index();
  ...
  // now we have to see if this coderep exists in our tree
  skip = FALSE;
...
// ===================================================================
// Add to list of exprs to skip in EPRE
// see main skiplist comment above
// ===================================================================

void
ETABLE::Add_to_skiplist(CODEREP * cr)
{
  Is_Trace(Tracing(),
	   (TFile, "Add coderep to skiplist: <cr%d>\n", cr->Coderep_id()));
  
  // create a new label node and add to list head
  COMIC_SKIP_LIST * new_node = (COMIC_SKIP_LIST*)CXX_NEW(COMIC_SKIP_LIST, Etable_pool());
  new_node->Set_coderep_id(cr->Coderep_id());
  new_node->Set_next(Comic_skip_list());
  Set_comic_skip_list(new_node);
}
 
// ===================================================================
// Check if this coderep is marked to be skipped.
// see main skiplist comment above
// ===================================================================

BOOL
ETABLE::In_skiplist(CODEREP * cr)
{
   COMIC_SKIP_LIST * node = Comic_skip_list();
   while (node != NULL) {
     if (node->Coderep_id() == cr->Coderep_id())
       return TRUE;
     node = node->Next();
   }
   return FALSE;
...
// ===================================================================
// Collect subexpressions with all children as leaves in this 
// expression
// see main skiplist comment above
// ===================================================================
 
void
ETABLE::Bottom_up_skiplist(CODEREP *cr)
{
  switch (cr->Kind()) {
  case CK_CONST:  
  case CK_RCONST: 
  ...
  case CK_IVAR:
    if ((cr->Opr() == OPR_ILOAD) ||
      ...
      CODEREP * base =  (cr->Ilod_base() == NULL) ? 
        cr->Istr_base() : cr->Ilod_base();
      
      if (base->Non_leaf()) 
        Bottom_up_skiplist(base);
      else
        Add_to_skiplist(cr);
    }
    break;
  ...
  case CK_OP:
    {
      ...
      for (INT32 i=0; i<cr->Kid_count(); i++) { 
        if (cr->Opnd(i)->Non_leaf()) {
          Bottom_up_skiplist(cr->Opnd(i));
          all_leaves = FALSE;
        }
      }
      if (all_leaves)
...
#ifdef TARG_NVISA
  // If this statement needs to be skipped for retaining loop
  // structure, don't process these codereps. Also store them
  // since the same codereps might be added from another path
  if (Comic() && Is_skip_candidate(stmt)) {
    Bottom_up_skiplist(stmt->Rhs());
    return;
  }
#endif
                               ...
                               UINT depth, BOOL is_istore)
{
  ...
  if (cr->Kind() == CK_VAR && 
      ...
      ST_is_value_parm(Opt_stab()->Aux_stab_entry(cr->Aux_id())->St())) {
    worklist->Set_has_addr_saved_param();
  }
#endif
...
ETABLE::Perform_PRE_optimization(void)
{
  ...
  FOR_ALL_NODE(cur_worklst, worklst_iter, Init()) {
...
#ifdef TARG_NVISA
    if (Comic()) {
      skip = In_skiplist(cur_worklst->Exp());
      if (skip) {
        Is_Trace(Tracing(), (TFile,
                             "ETABLE::Perform_PRE_optimization_comic: "
    ...
    if (!skip && cur_worklst->Insert_exp_phi(this)) {
...
// If block bb has such control relation to loop
// that any expression in bb will be downsafe in the loop. This means
// that hoisting/strength reduction etc. will not be hampered due
// to control relation of this block to its enclosing loop 
BOOL 
Control_dependence_implies_down_safety(BB_NODE* bb, 
                                       BB_LOOP* loop,
                                       MEM_POOL *lpool)
{
  // we don't want to fail, just return false
  if ((loop == NULL) || (bb == NULL))
    return FALSE;

  // in aggressive code motion, down-safety is fudged for loops
  if (WOPT_Enable_Aggressive_Code_Motion)
    return TRUE;

#ifdef TARG_NVISA
  // Early exit loops might be undesirable under certain flags
  ...
  // check 1: if body block is post-dominated by this block
  BB_NODE * body = loop->Body();
  if ((body != NULL) &&
      bb->Postdominates(loop->Body())) {
    return TRUE;
  }

  // check 2: if block and loop header have same control dependence
  BB_NODE * header = loop->Header();
  if ((lpool != NULL) && (header != NULL)) {
    BB_NODE_SET * bb_ctrl = bb->Rcfg_dom_frontier();
    BB_NODE_SET * loop_ctrl = header->Rcfg_dom_frontier();
    
    if ((bb_ctrl != NULL) && (loop_ctrl != NULL)) {
      BB_NODE_SET * diff = bb_ctrl->Difference(loop_ctrl, lpool);
