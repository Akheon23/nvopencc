  ...
  case CK_OP:
	...
	for (INT i = 0; i < exp->Kid_count(); ++i) {
	  ...
	  WN_kid(wn, i) = Gen_exp_wn(opnd, emitter);
          ...
          // Also an issue if EXTRACT_BITS under ASM and need to preserve sign.
          // But make sure rtypes are compatible.
          if (Mtype_TransferSign(WN_rtype(WN_kid(wn,i)), exp->Asm_input_rtype())
...
#ifdef TARG_NVISA
      if (Target_ISA >= TARGET_ISA_compute_20 && exp->Opr() == OPR_ILOAD && CR_is_uniform (exp)) {
        WN_Set_Load_Uniform(wn);
      }
      WN_set_memory_kind(wn, CR_get_memory_kind(exp));
#endif
...
#ifdef TARG_NVISA
      if (Target_ISA >= TARGET_ISA_compute_20 && WOPT_Enable_Cons2glb > 0 && ST_in_constant_mem(st)) {
        WN_Set_Load_Uniform(wn);
      }
#endif
...
#ifdef TARG_NVISA
      if (WOPT_Enable_Cons2glb > 0) {
        // fix inline assembly ld.const -> ld.global for __constant__ to __global__
        char *s = ST_name(WN_st_idx(rwn));
        const char *ldc = "ld.const";
        char *p1, *p2;
        UINT32 cnt = 0;
        p1 = s;
        while (p2 = strstr(p1, ldc)) {
          cnt++;
          p1 = p2 + strlen(ldc);
        }

        if (cnt > 0) {
          char *d = (char *)alloca(strlen(s) + cnt + 1);
          const char *ldg = "ld.global";
          char *d2 = d;
          p1 = s;
          while (p2 = strstr(p1, ldc)) {
            strncpy(d2, p1, (size_t)(p2-p1));
            d2 += (size_t)(p2-p1);
            strncpy(d2, ldg, strlen(ldg));
            d2 += strlen(ldg);
            p1 = p2 + strlen(ldc);
          }
          strcpy(d2, p1);

          ST *st = &St_Table[WN_st_idx(rwn)];
          Set_ST_name_idx(st, Save_Str(d));
        }
      }
#endif
...
#ifdef TARG_NVISA
      WN_set_memory_kind(rwn, CR_get_memory_kind(lhs));
#endif
      break;
    }
