...
// ====================================================================
// Contains_lda - see if the expr contains lda
// ====================================================================
BOOL
CODEREP::Contains_lda() const
{
  switch (Kind()) {
  case CK_LDA:
    return TRUE;
  ...
  case CK_OP:
    {
      BOOL has_lda = FALSE;
      for (INT32 i = 0; i < Kid_count(); i++)
        has_lda = has_lda | Opnd(i)->Contains_lda(); 
      return has_lda;
    }
  default:
    break;
...
BOOL
COPYPROP::Is_allowed_preopt_case(CODEREP *src_cr, 
                                 BB_NODE *src_bb, 
                                 BB_NODE *dest_bb)
{
  // be aggressive with lda's
  if (!src_cr->Contains_lda()) 
    return FALSE;

  // don't get stuck into unhoistable spot

  // if dest is not loopnest, we're fine
  if ((dest_bb->Loopdepth() == 0) ||
      (dest_bb->Loopdepth() <= src_bb->Loopdepth()))
    return TRUE;
  
  BB_LOOP *loop = dest_bb->Innermost();
  
  // get into the same level of loop as src or
  // the outermost loop if src is in different
  // loop nest
  while ( (loop->Parent() != NULL) &&
          (loop->Parent() != src_bb->Innermost()) ) 
    loop = loop->Parent();

  // we should be able to hoist it out from this loop
  // now
  if (Control_dependence_implies_down_safety(dest_bb,
                                             loop,
                                             Loc_pool()))
    return TRUE;
  
  // while loops not normalized yet
  if (dest_bb == loop->End())
    return TRUE;
...
#ifdef TARG_NVISA
      // If we are propagating a U4U4LDID to replace a U8U4LDID, we do
      // need the cvt, so the cvt can be avoided only if the dtyp matches.
      && expr->Dtyp() == Dtyp()
#endif // TARG_NVISA
     )
    return expr;
...
#ifdef TARG_NVISA
//
// Copy propagation of restrict pointers may lead to 
// more conservative alias information, so
// if alias analysis is to be re-applied, it is better
// not to propagate the restrict pointers.
//
// The following interfaces are to allow
// controlling the restrict pointer copy propagation.
//
static BOOL propagate_restrict_pointer = TRUE;

void
Set_propagate_restrict_pointer (BOOL v)
{
  propagate_restrict_pointer = v;
}

inline BOOL
Get_propagate_restrict_pointer (void)
{
  return propagate_restrict_pointer;
}

// For a store statement, check if the right-side is 
// more expensive if it is propagated.
//
BOOL
Rhs_is_heavier(CODEREP *lhs, CODEREP *rhs, OPT_STAB *opt_stab)
{
  if (rhs->Non_leaf()) {
    return TRUE;
  }
  ...
  if (rhs->Kind() != CK_VAR) {
    // CK_CONST, CK_RCONST. CK_LDA
    return FALSE;
  }

  ST *rhs_st = opt_stab->Aux_stab_entry(rhs->Aux_id())->St();
  if (rhs_st == NULL) {
    return FALSE;
  }

  if (ST_sclass(rhs_st) == SCLASS_REG) {
    // it should be always good to propagate reg
    return FALSE;
  }
  ...
  if (ST_sclass(rhs_st) == SCLASS_AUTO) {
    return FALSE;
  }

  if (ST_sclass(rhs_st) == SCLASS_EXTERN) {
    if (strcmp(ST_name(rhs_st),"threadIdx") == 0
	|| strcmp(ST_name(rhs_st),"gridDim") == 0
	|| strcmp(ST_name(rhs_st),"blockIdx") == 0
	|| strcmp(ST_name(rhs_st),"blockDim") == 0) {
      // special globals which are from special registers
      return FALSE;
    }
  }

  if (ST_sclass(lhs_st) == SCLASS_AUTO) {
    return TRUE;
  }

  if (ST_sclass(lhs_st) == ST_sclass(rhs_st)) {
    return FALSE;
  }
...
// Check if it is safe to do partial propagation of statement in the form of
//     left_id = ...cr...
//
static BOOL
check_partial_prop_cr(IDTYPE left_id, CODEREP *cr_var)
{
  IDTYPE id = cr_var->Aux_id();

  if (id != left_id || cr_var->Is_var_volatile()) {
    return TRUE;
  } else {
    return FALSE;
  }
...
//
static BOOL 
check_partial_prop_expr(IDTYPE left_id, CODEREP *expr)
{
  switch (expr->Kind()) {
  case CK_LDA:
  case CK_CONST:
  case CK_RCONST: return TRUE;
  ...
  case CK_VAR:
	...
	return TRUE;
      return check_partial_prop_cr(left_id, expr);
    }

  case CK_IVAR:
    if (expr->Ilod_base()) {
      if (check_partial_prop_expr(left_id, expr->Ilod_base()) == FALSE) {
        return FALSE;
      }
    } 

    if (expr->Istr_base()) {
      if (check_partial_prop_expr(left_id, expr->Istr_base()) == FALSE) {
        return FALSE;
      }
    }

    if (expr->Opr() == OPR_MLOAD) {
      if (check_partial_prop_expr(left_id, expr->Mload_size()) == FALSE) {
        return FALSE;
      }
    }
 
    if (expr->Opr() == OPR_ILOADX) {
      if (check_partial_prop_expr(left_id, expr->Index()) == FALSE) {
        return FALSE;
      }
    }
  ...
  case CK_OP:
    for (INT32 i = 0; i < expr->Kid_count(); i++) {
      if (check_partial_prop_expr(left_id, expr->Opnd(i)) == FALSE) {
        return FALSE;
      }
    }
    return TRUE;
...
#ifdef TARG_NVISA
  if (Get_propagate_restrict_pointer () == FALSE
      && TY_is_restrict(x->Lod_ty())) {
    // do not propagate restrict pointer, otherwise, the alias will become worse
    // where the restrict pointer is used.
    return NULL;
  }
#endif
...
#ifdef TARG_NVISA
  {
    // less aggressive in propagating assignments like,
    //       x = *(y + offset)
    // help improve overall performance
    //
    ST *st = Htable()->Opt_stab()->Aux_stab_entry(x->Aux_id())->St();
    if (st != NULL && (ST_sclass(st) == SCLASS_REG || ST_sclass(st) == SCLASS_AUTO)) {
      if (expr->Kind() == CK_IVAR && x->Usecnt() > 1) {
        return NULL;
      } 
    }
    // The Open64 SSA-based IR place some limitations on the live ranges of 
    // CR_VAR which comes from same Aux-id, which affects if it is safe 
    // to do partial copy propagation.
    // I.e., for statement in the following format,
    //
    //         x_version_2 = x_version_1 + 1
    //
    // if x_version_2 has multiple uses, and some are replaced, 
    // and some are not, the above assignment cannot be deleted, 
    // this can lead to overlapping of live ranges of
    // CRs comes from the same Aux_id.
    //
    // The function check_partial_prop_expr() is to find out if it is safe 
    // to do partial propagation on the statement.
    //
    if (WOPT_Check_Partial_Prop 
       && (stmt->Lhs()->Is_var_volatile() 
        || check_partial_prop_expr(stmt->Lhs()->Aux_id(), expr))) 
    { 
      if ((expr->Kind() != CK_VAR 
         && expr->Kind() != CK_CONST 
         && expr->Kind() != CK_RCONST
         ...
         && stmt->Bb() != curbb) 
      {
        // make sure propagation of 
        //         left_id = expr
        // won't happen on conditions.
        // This might be too restricted, the propagation to single condition 
        // branch might do some good.
        // The complete check requires def-use chains, so that all uses 
        // can be checked together.
        // For example, if a definition is only used in a single 
        // condition-block, it should be good to propagate and delete the
        // original assignment. 
        // But PDE or PRE later might take care of such cases.
        BOOL pdom_stmt = FALSE;
        BB_NODE     *pdom_bb;
        BB_LIST_ITER pdom_bb_iter;
        FOR_ALL_ELEM (pdom_bb, pdom_bb_iter, Init(curbb->Pdom_bbs()) ) {
          if (pdom_bb == stmt->Bb()) {
            pdom_stmt = TRUE;
            break;
          }
...
#ifdef TARG_NVISA
  if (WOPT_Enable_Limited_Preopt_Copy_Prop &&
      (Htable()->Phase() == PREOPT_PHASE) && 
      Rhs_is_heavier(stmt->Lhs(), expr, Opt_stab()) &&
      !Is_allowed_preopt_case(expr, stmt->Bb(), curbb))
    return NULL;
#endif
...
COPYPROP::Copy_propagate(BB_NODE *bb)
{
 ...
 //   bug 469540:We should always update new version if we limite some kind of copy_propagate in PREOPT.  

      if (Is_function_of_itself(stmt, Opt_stab()) &&
	  Opt_stab()->Stack_elements(lhs->Aux_id()) != 0 &&
      !(Htable()->Phase() == PREOPT_PHASE &&
      WOPT_Enable_Limited_Preopt_Copy_Prop))
