...
// $Source: /scratch/mee/Patch0002-taketwo/kpro64-pending/be/opt/SCCS/s.opt_etable.h $
//
// ====================================================================
//
...
#ifdef TARG_NVISA
    EWF_PARAM_ADDR_SAVED = 0x1000 // this CK_VAR is a value PARAM and the address is taken
#endif
  };
...
#ifdef TARG_NVISA
  // variable has address saved 
  BOOL            Has_addr_saved_param(void) const {
    return _flags & EWF_PARAM_ADDR_SAVED;}
  void            Set_has_addr_saved_param(void) {
    _flags |= EWF_PARAM_ADDR_SAVED; }
#endif  
...
// A linked-list to keep codereps to skip for comic target
// for maintaining loop structure

class COMIC_SKIP_LIST
{
   COMIC_SKIP_LIST *_next;
   INT32      _coderep_id;
  public:
   COMIC_SKIP_LIST() { _next = NULL; _coderep_id = -1; }
 
   COMIC_SKIP_LIST* Next()                          { return  _next; }
   INT32            Coderep_id()                    { return _coderep_id; }
   void             Set_next(COMIC_SKIP_LIST *list) { _next = list; }
   void             Set_coderep_id(INT32 n)         { _coderep_id = n; }
};
#endif
...
#ifdef TARG_NVISA
  BOOL                    _comic;               // Doing pre for comic (cuda on multi-core)
  COMIC_SKIP_LIST        *_comic_skip_list; 
#endif
...
#ifdef TARG_NVISA
  BOOL            Is_skip_candidate(STMTREP *stmt);
  void            Add_to_skiplist(CODEREP * cr);
  BOOL            In_skiplist(CODEREP * cr);
  void            Bottom_up_skiplist(CODEREP *stmt);
  COMIC_SKIP_LIST * Comic_skip_list()  { return _comic_skip_list; }
  void            Set_comic_skip_list(COMIC_SKIP_LIST* l) { _comic_skip_list = l; }
#endif
...
#ifdef TARG_NVISA
  void            Set_comic(BOOL b) { _comic = b; }
  BOOL            Comic() { return _comic; }
#endif
...
BOOL   Control_dependence_implies_down_safety(BB_NODE* bb, 
                                              BB_LOOP* loop,
                                              MEM_POOL *lpool);
