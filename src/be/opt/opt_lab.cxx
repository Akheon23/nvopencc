...
LabelOpt:: Mark_Referenced_Labels(WN * wn)
{
  ...
  case OPR_BLOCK:
    ...
    break;
  case OPR_SWITCH:
    // kid0 is an expression, so can be ignored.
    for (kidno = 1; kidno < WN_kid_count(wn); kidno++) 
      Mark_Referenced_Labels(WN_kid(wn, kidno));
    break;
  case OPR_COMPGOTO:
    Mark_Referenced_Labels(WN_kid(wn, 1));
    Set_Mark(WN_last_label(wn));
    break;
  ...
  case OPR_XGOTO:
  case OPR_AGOTO:
    // If AGOTO/XGOTOs are present, then this pass is broken (because labels
    // may actually be loaded/stored, and we never look at load/store expression
    // nodes here).
