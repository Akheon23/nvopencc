...
#define RESHAPE_FLAG           0x80 /* trace n-level expression reshaping*/
/* TP_WOPT3 (phase-number) */
#define PTRCLASS_FLAG           0x1 /* trace NVISA pointer classification */
#define PDE_FLAG                0x2 /* trace partial dead-code elimination */
...
/* The CXX_OPT_POOL class is analogous to CXX_MEM_POOL found in common/util/mempool.h; it will clean up
   after itself as it goes out of scope, (hopefully) after objects that use it are destroyed.  */

class CXX_OPT_POOL: public CXX_MEM_POOL
{
   MEM_POOL *_mp;
   int _debug;
 public:
   CXX_OPT_POOL(const char *name, BOOL do_bzero, int debug): CXX_MEM_POOL(name, do_bzero), _debug(debug) {
     _mp = CXX_MEM_POOL::operator()();
#ifdef Is_True_On   
    if (Get_Trace(TP_GLOBOPT, MEM_DUMP_FLAG)) {
      fprintf(TFile,"0x%x, 0x%p, OPT_POOL_Initialize(%s)\n", _debug, _mp, name);
      fprintf(TFile,"0x%x, 0x%p, OPT_POOL_Push()\n",_debug, _mp);
