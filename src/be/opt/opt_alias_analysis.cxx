...
void OPT_STAB::Simplify_Pointer_Arith(WN *wn_expr, POINTS_TO *ai)
{
...
#ifdef TARG_NVISA
  case OPR_CVT:
    // Handle a cvt as long as the converted-to type is compatible with
    // the pointer type.
    if (MTYPE_byte_size(WN_rtype(wn_expr)) < Pointer_Size)
    {
      ai->Set_expr_kind(EXPR_IS_INT);
      ai->Set_base_kind(BASE_IS_UNKNOWN);
      ai->Set_ofst_kind(OFST_IS_UNKNOWN);
  ...
  case OPR_BIOR:
    {
      // We need to handle bitwise OR involving pointers (like BIOR with zero),
      // treat it similar to add. Only handle OR of pointers with volatile
      // scalars.
      WN *kid0 = WN_kid0(wn_expr);
      if (WN_operator(kid0) != OPR_LDID ||
          TY_kind(WN_ty(kid0)) != KIND_SCALAR ||
          !TY_is_volatile(WN_ty(kid0)))
      {
        ai->Set_expr_kind(EXPR_IS_INT);
        ai->Set_base_kind(BASE_IS_UNKNOWN);
        ai->Set_ofst_kind(OFST_IS_UNKNOWN);
...
#ifdef TARG_NVISA
    occ->Points_to()->Set_mem_kind(WN_get_memory_kind(wn));
    // Accumulate MEMORY_KIND for virtual variables accessed in different site
    aux_stab[vp_idx].Points_to()->Set_mem_kind((MEMORY_KIND)(occ->Points_to()->Get_mem_kind() |aux_stab[vp_idx].Points_to()->Get_mem_kind()));
#endif
...
#ifdef TARG_NVISA
  BOOL volatile_call_asm = (strstr(ST_name(WN_st_idx(wn)), "call") != NULL) && WN_Asm_Volatile(wn);
#endif
...
#ifdef TARG_NVISA
      if (volatile_call_asm) {
        if (Aux_stab_entry(idx)->Is_virtual() || Aux_stab_entry(idx)->Lda_vsym()) {
	  how = READ_AND_WRITE;
	  goto label_how;
        }
      }
