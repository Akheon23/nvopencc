...
// $Source: /scratch/mee/Patch0002-taketwo/kpro64-pending/be/opt/SCCS/s.opt_rename.cxx $
//
// ====================================================================
//
	...
	Is_True(tmp != NULL, ("Second rename: cannot find non-zero version."));
      ...
      if (tmp->Is_flag_set((CR_FLAG)(CF_DEF_BY_CHI|CF_DEF_BY_PHI))) {
        ...
        if (MTYPE_is_integral(cr->Dtyp())
          && MTYPE_byte_size(cr->Dtyp()) != MTYPE_byte_size(tmp->Dtyp()))
        {
          DevWarn("insert cvt above cur-version");
          CODEREP cvt_cr;
          cvt_cr.Init_expr(OPCODE_make_op(OPR_CVT, cr->Dtyp(), tmp->Dtyp()), 
                           tmp);
          tmp = htable->Rehash(&cvt_cr);
	  tmp->Opnd(0)->Set_sign_extension_flag(); 
        }
        else {
#endif // NVISA
#ifdef KEY
