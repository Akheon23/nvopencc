...
// $Source: /scratch/mee/Patch0002-taketwo/kpro64-pending/be/opt/SCCS/s.opt_canon.cxx $
//
// Revision history:
...
CANON_CR::Convert2cr(WN *wn, CODEMAP *htable, BOOL foldit) const
{
  ...
  if (typ == MTYPE_V) 
   ...
   const_mtype = typ;
#else
   const_mtype = MTYPE_I8;
#endif
#ifdef TARG_NVISA
   // usually want to preserve the type, 
   // but if a negative value and the type is unsigned, 
   // then use signed type.
   if (Scale() < 0 && MTYPE_is_unsigned(const_mtype)) {
     DevWarn("negative scale so change mtype to signed");
     const_mtype = MTYPE_complement(const_mtype);
   }
#endif
  ...
  if (Tree() && Scale() != 0) {
    return htable->Add_bin_node_and_fold
      (OPCODE_make_op(OPR_ADD, typ, MTYPE_V),
       Tree(), htable->Add_const(const_mtype, Scale()));
  }
  if (Tree()) 
    return Tree();

  // return a CK_CONST node
  CODEREP *cr =  htable->Add_const(const_mtype, Scale());
  return cr;
}
