...
void
BB_NODE::Print_head_src (FILE *fp) const
{
  // print the bb structure
  fprintf(fp, "---- BB%d (RPO %d)", Id(),Rpo_id());
  if (Labnam())
...
void
BB_NODE::Print_wn_src (FILE *fp) const
{
  STMT_ITER wn_iter(_firststmt, _laststmt);
  wn_iter.Print_src(fp);
}
...
void
BB_NODE::Print_src () const
{
  Print_head(stdout);
  if (Phi_list())
    Phi_list()->Print(stdout);  
  Print_wn_src(stdout);
  _stmtlist.Print_src(stdout);
  fflush(stdout);
}


void
BB_NODE::Print_aux (FILE *fp) const
{
  Print_head_src(fp);
  fflush(stdout);
  if (Phi_list())
...
void
BB_NODE::Print_cr (FILE *fp) const
{
  Print_head_src(fp);
  fflush(stdout);
  if (Phi_list()) {
