...
public:
...
#ifdef TARG_NVISA
  ST_IDX Get_Cloned_Label (ST_IDX idx);
#endif
  ...
  ST* Get_ST(ST* old_st) {	// Get the copy of the orig st
        ...
        if ((ST_level(old_st) != _orig_level))
           return old_st;
        if ((_cloned_scope_tab[_cloned_level].st_tab)->Size() <= (ST_index(old_st)+_cloned_st_last_idx))
           return NULL;
	return (&_cloned_scope_tab[_cloned_level].st_tab->Entry(ST_index(old_st)+_cloned_st_last_idx));
  }
