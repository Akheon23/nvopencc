...
#ifdef TARG_NVISA
  void Set_mem_kind(MEMORY_KIND kind) { _mem_kind = kind; } 
  MEMORY_KIND Get_mem_kind() const { return _mem_kind; } 
#endif
...
WN *Find_addr_recur(WN *wn, const SYMTAB &stab)
{
  ...
  case OPR_ARRAY:
    return Find_addr_recur(WN_kid0(wn), stab);
#ifdef TARG_NVISA
  // Need to analyze further for bitwise OR expression involving addresses
  // as long as it is or-ed with a volatile scalar
  case OPR_BIOR:
    {
      WN *kid0 = WN_kid0(wn);
  ...
  case OPR_CVT:
    if (WN_operator(wn) == OPR_CVT &&
        MTYPE_byte_size(WN_rtype(wn)) < Pointer_Size)
      return NULL; // converted-to type incompatible with pointer type
    // fall through
#endif
  case OPR_ADD:
    { 
