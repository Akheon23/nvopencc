...
public:
#ifdef TARG_NVISA
  BE_ST(void) : flags(0),memory_pointed_to(MEMORY_UNKNOWN),io_auxst(NULL) { }
#else
...
#ifdef TARG_NVISA
  ST_MEMORY Memory_pointed_to(void) const { return memory_pointed_to; }
  void Set_memory_pointed_to(ST_MEMORY m) { memory_pointed_to = m; }
#endif
...
#ifdef TARG_NVISA
inline ST_MEMORY
BE_ST_memory_pointed_to (const ST *const st)
{
  return Be_symbol_table[ST_st_idx(st)].Memory_pointed_to();
}
inline void
Set_BE_ST_memory_pointed_to (const ST *st, ST_MEMORY m)
{
  Be_symbol_table[ST_st_idx(st)].Set_memory_pointed_to(m);
}
#endif
...
#ifdef TARG_NVISA
  WN *lda;
  ST_MEMORY memory_pointed_to;
#endif
public:
#ifdef TARG_NVISA
  BE_PREG(void) : home_location(NULL), lda(NULL), memory_pointed_to(MEMORY_UNKNOWN)  { }
#else
...
inline ST_MEMORY
Preg_memory_pointed_to (const PREG_NUM preg)
{
  const UINT idx = preg - Last_Dedicated_Preg_Offset;
  if (idx < Be_preg_tab.Size())
    return Be_preg_tab[idx].Memory_pointed_to();
  else
    return MEMORY_UNKNOWN;
}
inline void
Set_Preg_memory_pointed_to (PREG_NUM preg, ST_MEMORY m)
{
  Be_preg_tab[preg - Last_Dedicated_Preg_Offset].Set_memory_pointed_to(m);
}
...
// Search tree for a LDID of pointer type, returning NULL if not found.
extern WN* Find_Pointer_Ldid (WN *tree);
#endif
