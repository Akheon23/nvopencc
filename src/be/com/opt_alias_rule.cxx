...
#ifdef TARG_NVISA
  // formal parameter's restrict attribute is propagated, and
  // the code here is to enforce the scoping of the restrict pointers
  // propagated from inliner.
  //
  // Since the original restrict pointers are treated the same as
  // unique pointers, the implementation here is to treat the
  // restricted pointers unique pointers within the same group
  // of pointers from the same inline instance, restricted or not.
  //
  // The existing behaviour of other restricted pointers 
  // which are not from formal parameters are kept the same.
  //
  ST *st_base1 = mem1->Based_sym();
  ST *st_base2 = mem2->Based_sym();
  if (st_base1 == NULL || st_base2 == NULL) {
    return TRUE;
  }
  if (st_base1 == st_base2) {
    return TRUE;
  }

  INT32 gid1 = ST_get_restrict_group_id (st_base1);
  INT32 gid2 = ST_get_restrict_group_id (st_base2);

  BOOL is_restrict_1 = mem1->Restricted();
  BOOL is_restrict_2 = mem2->Restricted();
  
  if (is_restrict_1 && gid1 == 0) {
    // This restrict pointer is not from inliner, so
    // keep the existing behaviour
    if (!mem2->Default_vsym()) {
      return FALSE;
    }
  }

  if (is_restrict_2 && gid2 == 0) {
    // This restrict pointer is not from inliner, so 
    // keep the existing behaviour
    if (!mem1->Default_vsym()) {
      return FALSE;
    }
  }

  if ((is_restrict_1 || is_restrict_2) && gid1 == gid2) {
    // printf("New restrict rules takes effect: %d\n", gid1);
    return FALSE;
  } else {
    return TRUE;
  }
...
#ifdef TARG_NVISA
  if (WOPT_Enable_Memory_Space_Based_Alias_Analysis) {
    // Use Memory space information
    MEMORY_KIND kind1 = p1->Get_mem_kind();
    MEMORY_KIND kind2 = p2->Get_mem_kind();

    if (kind1 != kind2 && kind1 != MEMORY_KIND_INIT && kind2 != MEMORY_KIND_INIT
        && kind1 != MEMORY_KIND_UNKNOWN && kind2 != MEMORY_KIND_UNKNOWN) {
      if (kind1 == MEMORY_KIND_GLOBAL || kind1 == MEMORY_KIND_PARAM
          || kind1 == (MEMORY_KIND_GLOBAL | MEMORY_KIND_PARAM)) {
        if (!(kind2 & (MEMORY_KIND_GLOBAL | MEMORY_KIND_PARAM))) {
          if (WOPT_Enable_Dump_Memory_Space_Based_Alias_Analysis) {
            printf("Global or param do not alias with non-global\n");
          }
          return ALIAS_KIND (AR_NOT_ALIAS);
        }
      }
      if (kind2 == MEMORY_KIND_GLOBAL || kind2 == MEMORY_KIND_PARAM
          || kind2 == (MEMORY_KIND_GLOBAL | MEMORY_KIND_PARAM)) {
        if (!(kind1 & (MEMORY_KIND_GLOBAL | MEMORY_KIND_PARAM))) {
          if (WOPT_Enable_Dump_Memory_Space_Based_Alias_Analysis) {
            printf("Global or param do not alias with non-global\n");
          }
          return ALIAS_KIND (AR_NOT_ALIAS);
      ...
      if (kind1 == MEMORY_KIND_SHARED && !(kind2 & MEMORY_KIND_SHARED)) {
        if (WOPT_Enable_Dump_Memory_Space_Based_Alias_Analysis) {
          printf ("Shared do not alias with non-shared\n");
        }
        return ALIAS_KIND (AR_NOT_ALIAS);
      }
      if (kind1 == MEMORY_KIND_CONST || kind2 == MEMORY_KIND_CONST
          || kind1 == (MEMORY_KIND_CONST | MEMORY_KIND_GLB_CONST)
          || kind1 == MEMORY_KIND_GLB_CONST || kind2 == MEMORY_KIND_GLB_CONST
          || kind2 == (MEMORY_KIND_CONST | MEMORY_KIND_GLB_CONST)) {
        if (WOPT_Enable_Dump_Memory_Space_Based_Alias_Analysis) {
          printf("constants do not alias with anything else\n");
        }
        return ALIAS_KIND (AR_NOT_ALIAS);
      }
    }
