...
Allocate_Entry_Formal(ST *formal, BOOL on_stack, BOOL in_formal_reg)
{
  if (Has_No_Base_Block(formal))
  {
    ...
    */
    ...
    else if (ST_in_shared_mem(formal) || ST_in_param_mem(formal)) {
	Clear_ST_is_value_parm(formal);
...
#endif
#ifdef TARG_NVISA
          // even virtual registers take space, so count them
          Set_PU_arg_area_size(pu_ty, 
            Get_PU_arg_area_size(pu_ty) + PLOC_size(ploc));
#endif // NVISA
	}
...
Initialize_Stack_Frame (WN *PU_tree)
{
  ...
  if (Target_ISA < TARGET_ISA_compute_20) {
    // we are limited to 256 bytes of formal space
