				 ...
				 BOOL mode, TY_IDX rty_idx, WN * addr, INT64 roffset)
{
  ...
  do {
	...
	if (mode) {
	  ...
	  if (offset <= INT32_MAX)
	    wn1 = WN_CreateLda ( opc_lda, offset, TY_pointer(ty_idx),
				 WN_st(addr) );
	  else
	    wn1 = WN_Add ( Pointer_type,
			   WN_CreateLda ( opc_lda, 0, TY_pointer(ty_idx),
					  WN_st(addr) ),
	...
	switch (etype) {
	  ...
	  case FIT_COMPLEX16:
	    if (mode) {
	      ...
	      if (offset <= INT32_MAX)
		wn1 = WN_CreateLda ( opc_lda, offset, TY_pointer(ety_idx),
				     WN_st(addr) );
	      else
		wn1 = WN_Add ( Pointer_type,
			       WN_CreateLda ( opc_lda, 0, TY_pointer(ety_idx),
					      WN_st(addr) ),
			       WN_CreateIntconst ( opc_const, offset ) );
	    } else
	      wn1 = WN_Add ( Pointer_type, WN_COPY_Tree(addr),
	  ...
	  case FIT_RECORD:
	    if (mode) {
	      (void) Make_Pointer_Type ( ety_idx, FALSE );
	      if (offset <= INT32_MAX)
		wn = WN_CreateLda ( opc_lda, offset, TY_pointer(ety_idx),
				    WN_st(addr) );
	      else
