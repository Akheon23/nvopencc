...
#ifdef _MSC_VER
/* Should push back to Commieland */
extern const char *INTR_intrinsic_name(WN *);
#else
				  ...
				  SRCPOS srcpos, LOWER_ACTIONS actions)
{
  ...
  // But don't reset it if already had a def.
  WN *lda = Find_Lda (tree);
  if (lda && !Preg_Lda(pregNo)) {
    Set_Preg_Lda (pregNo, lda);
  }
#endif
...
/* ====================================================================
 *
 * WN *lower_u8u2_load(WN *block, WN *tree, LOWER_ACTIONS actions)
 *
 * Perform lowering to add a U8U4 convert since U8U2 form confuses
 * the optimizer
 *
 * ==================================================================== */

static WN *lower_u8u2_load(WN *block, WN *tree, LOWER_ACTIONS actions)
{
  TYPE_ID  mtype   = WN_rtype(tree);

  Is_True((WN_operator(tree) == OPR_LDID || WN_operator(tree) == OPR_ILOAD),
          ("expected LDID/ILOAD node, not %s", OPCODE_name(WN_opcode(tree))));

  switch (mtype) {
    case MTYPE_I8:
      WN_set_rtype(tree, MTYPE_I4);
      tree = lower_expr(block, tree, actions);
      tree = WN_Cvt(MTYPE_I4, MTYPE_I8, tree);
      return lower_expr(block, tree, actions);

    case MTYPE_U8:
      WN_set_rtype(tree, MTYPE_U4);
      tree = lower_expr(block, tree, actions);
      tree = WN_Cvt(MTYPE_U4, MTYPE_U8, tree);
      return lower_expr(block, tree, actions);
 ...
 * ==================================================================== */
#ifdef TARG_NVISA
static WN* lower_bit_field_id(WN *wn)
#else
...
#endif
{
  ...
  if (OPERATOR_is_load(new_opr) && 
      rtype != WN_rtype(wn)) {
    // For a bit-field load, the front-end can generate a U4BS load for
    // a data with U8 base type (bytes_accessed == 8) but whose bit-field
    // size is within U4 size. This may have been lowered to a U8U8 load,
    // in that case add a cvt because we could be doing a 32-bit operation
    // on the load.
    return WN_Cvt(WN_rtype(wn), rtype, wn);
  }
  else
    return wn;
#endif
...
static WN *lower_expr(WN *block, WN *tree, LOWER_ACTIONS actions)
{
  ...
  */
  ...
  case OPR_ILOAD:
#ifdef TARG_NVISA
    if (Action(LOWER_U8U2_LOAD) &&
        (WN_opcode(tree) == OPC_U8U2ILOAD ||
         WN_opcode(tree) == OPC_I8I2ILOAD))
      return lower_u8u2_load(block, tree, actions);
#endif
...
#ifdef TARG_NVISA
    if (Action(LOWER_U8U2_LOAD) &&
        (WN_opcode(tree) == OPC_U8U2LDID ||
         WN_opcode(tree) == OPC_I8I2LDID))
      return lower_u8u2_load(block, tree, actions);
#endif
...
#ifndef TARG_NVISA // NVISA needs moves to different register sets
...
static WN *lower_return_mstid(WN *block, WN *tree, LOWER_ACTIONS actions)
{
  ...
  if (RETURN_INFO_return_via_first_arg(return_info)) { // fake first parm
	...
	while (MTYPE_byte_size(desc) < TY_size(WN_ty(tree)))
      ...
      n_rhs = WN_CreateLdid(OPR_LDID, Mtype_comparison(mtype), mtype, 
	...
	DevWarn("return_info struct alignment is smaller than register size, may produce wrong results");
      ...
      wn = WN_CreateStid(OPR_STID, MTYPE_V, desc, 
		         WN_store_offset(tree)
#ifdef TARG_NVISA
                         + RETURN_INFO_offset(return_info,i),
#else
...
#ifdef TARG_NVISA
// lower struct copy of asm output, which goes downward from negative preg,
// into sequence of ldid/stid.  This is useful for texture and surface 
// intrinsics that return vectors, and we want to expand into field copies.
static WN *lower_asm_mistore (WN *block, WN *tree, LOWER_ACTIONS actions)
{
  WN *wn;
  WN *ldid;
  INT field_id = 0;
  TY_IDX vty = TY_pointed(WN_ty(tree));
  FmtAssert(TY_can_be_vector(vty), ("not a vector type"));
  TY_IDX ety = TY_vector_elem_ty(vty);
  PREG_NUM pnum = WN_load_offset(WN_kid0(tree)); // asm out preg
  for (INT i = 0; i < TY_vector_count(vty); ++i) {
      ...
      wn = WN_CreateIstore(OPR_ISTORE, MTYPE_V, TY_mtype(ety), 
		/* increment by size of element */
		i * MTYPE_byte_size(TY_mtype(ety)), 
		WN_ty(tree), ldid, WN_COPY_Tree(WN_kid1(tree)), field_id);
      wn  = lower_store (block, wn, actions);
      WN_Set_Linenum (wn, WN_Get_Linenum(tree));
      WN_INSERT_BlockLast (block, wn); // insert the last STID created 
  }
...
static WN *lower_store(WN *block, WN *tree, LOWER_ACTIONS actions)
{
  ...
  case OPR_ISTORE:
    ...
    if (Action(LOWER_RETURN_VAL) && WN_desc(tree) == MTYPE_M &&
       ...
       // asm pregs start at -2, go downward
       WN_load_offset(WN_kid0(tree)) <= -2)
    {
       return lower_asm_mistore(block, tree, actions);
    }
#endif
  ...
  case OPR_STID:
    ...
    &&   (! Preg_Is_Dedicated (WN_store_offset (tree))))
    {
      ...
      if (lda && !Preg_Lda(WN_store_offset(tree))) {
        Set_Preg_Lda (WN_store_offset(tree), lda);
      }
    }
#endif
...
extern TYPE_ID compute_copy_quantum(INT32 alignment)
{
  ...
  if (Target_ISA >= TARGET_ISA_compute_20) {
    if (alignment > 8) {
      DevWarn("for quantum change alignment from %d to 8", alignment);
      alignment = 8;
    }
  } else if (alignment > 4) {
...
/* ====================================================================
 * void copy_aggregate_simple(WN *block, WN *origStore, LOWER_ACTIONS actions)
 * 
 * Creates a sequence of STID instructions for a MSTORE, but only when
 * destination was a pointer to structure, and the structure has
 * scalar fields.
 * 
 * ==================================================================== */
static void
copy_aggregate_simple(WN *block, WN *origStore, LOWER_ACTIONS actions)
{
  TY_IDX  dstTY = TY_pointed(Ty_Table[WN_ty(origStore)]);

  Is_True((TY_kind(dstTY) == KIND_STRUCT),
	  ("expected KIND_STRUCT"));

  FLD_ITER fld_iter = Make_fld_iter(TY_fld(Ty_Table[dstTY]));
  WN_OFFSET cur_offset = 0;
  UINT fid = 1;
  ...
  while(TRUE) {
    ...
    Is_True((TY_kind(Ty_Table[fty_idx]) == KIND_SCALAR),
            ("expecting only scalar fields"));

    cur_offset = FLD_ofst(fld);

    WN *stid = WN_CreateStid(OPR_STID, 
                             MTYPE_V, 
                             TY_mtype(fty_idx), 
                             cur_offset, 
                             WN_st(WN_kid1(origStore)), 
                             dstTY,
                             WN_COPY_Tree(WN_kid0(origStore)), 
                             fid);

    lower_copy_maps(origStore, stid, actions);

    fid++;
#ifdef KEY
    // Bug 1268 - copy linenumber when creating WNs.
    WN_copy_linenum(origStore, stid);
#endif
    WN_INSERT_BlockLast(block, stid);
    ...
    */
	...
	break;
	  ...
	  if (diff < MTYPE_byte_size(quantum) && diff != 0)
        ...
        /* Struct_Init_Memset : If the mstore is for a struct with simple scalar fields,
         * AND they are getting initialized to zero, turn them into a sequence of
         * STIDs. 
         * Note : The fact that the number of fields is few enough is checked by
         * GenerateMstoreAction above, so here only the other pre-requisites are checked.
         */
        if(OPT_Enable_Struct_Init_Memset_Opt &&
           (dstTY == KIND_STRUCT) &&
           (WN_operator(WN_kid0(mstore)) == OPR_INTCONST) &&
           (WN_operator(addr) == OPR_LDA) &&
           WN_const_val(WN_kid0(mstore)) == 0) {
          copy_aggregate_simple(newblock,
                                mstore,
                                actions);
        }
        else {
				...
				LOWER_ACTIONS actions)
{
  ...
  */
...
#ifdef TARG_NVISA
  // we don't want LDA in code, but LDID is okay (will later ILOAD it)
  if (WN_operator(WN_kid0(mload)) != OPR_LDA)
#endif
  {
  ...
  {
#ifdef TARG_NVISA
    // offsets may not be adjacent, so use explicit offset rather than add size
    // (this should actually be okay for any target)
    mloadOffset = PLOC_offset(ploc);
#endif
...
#ifdef TARG_NVISA
      // handle smaller field sizes
      if (PLOC_size(ploc) == 1) 
        type = MTYPE_I1;
      else if (PLOC_size(ploc) == 2)
        type = MTYPE_I2;
#endif
	...
	*/
	...
	else
	{
	  ...
	  load = WN_IloadLdid(type, mloadOffset,
            ...
            // want to transform MLOAD(LDA x,INTCONST) into direct LDID x
            // to avoid penalty of LDA.
            WN *lda = WN_kid0(mload);
            load = WN_Ldid(type, mloadOffset + WN_offset(lda), 
               WN_st(lda), MTYPE_To_TY(type));
          }
#endif // NVISA
	  lower_copy_maps(mload, load, actions);
	}
...
#ifndef TARG_NVISA // see above comment about why not needed
			       ...
			       LOWER_ACTIONS actions)
{
  ...
  while (PLOC_is_nonempty(ploc))
  {
	...
	reg = Int32_Preg;
      // we allow gaps in struct params (cause pass fields),
      // so set offset explicitly rather than adding previous size.
      offset = PLOC_offset(ploc);
#else
      ST	*reg = Preg_Offset_Is_Float(regNo) ? Float_Preg : Int_Preg;
#endif
...
#else
    FmtAssert(FALSE, ("cis_intrinsic not supported"));
#endif
  }
#endif
...
#ifdef TARG_NVISA
  case LOWER_U8U2_LOAD:                 return "LOWER_U8U2_LOAD";
#endif
