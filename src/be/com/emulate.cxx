...
#ifndef TARG_NVISA /* does anyone really need values.h? */
#include <values.h>
#endif
...
static BOOL check_size(WN *size, WN *src, WN *dst)
{
  if (Is_Integer_Constant(size))
  {
    ...
    {
      /* The else condition under TARG_NVISA used to do just exit with
       * ErrMsg. But now TRUE is returned because n > CG_memmove_inst_count
       * is handled correctly, i.e., converted to a loop later.
       */
      return TRUE;
...
#ifdef TARG_NVISA
static void *create_emuloop(WN *block, ST *index_st, WN *dst, WN *src, WN *size, WN* init_index, TYPE_ID rtype, int stepsize)
{
  WN *loop_test, *loop_body, *index_incr;
  WN *index_wn, *init_wn;
  WN *tmp_wn0, *tmp_wn1, *tmp_wn2;
  // Use the same type as the size to simplify the loop_test node
  TYPE_ID index_type = WN_rtype(size);

  index_wn = WN_CreateIdname(0, index_st);

  //loop_test
  tmp_wn0 = WN_CreateLdid(OPR_LDID, index_type, index_type, 0, index_st, ST_type(index_st), 0);
  loop_test = WN_GT(index_type, size, tmp_wn0);


  //index increment
  tmp_wn1 = WN_CreateLdid(OPR_LDID, index_type, index_type, 0, index_st, ST_type(index_st), 0);
  tmp_wn1 = WN_Add(index_type, tmp_wn1, WN_Intconst(index_type, stepsize));
  index_incr = WN_CreateStid(OPR_STID, MTYPE_V, index_type, 0, index_st, ST_type(index_st), tmp_wn1, 0);

  //loop_body
  tmp_wn2 = WN_CreateLdid(OPR_LDID, index_type, index_type, 0, index_st, ST_type(index_st), 0);
  tmp_wn2 = WN_Add(index_type, tmp_wn2, dst);

  WN * loop_block;
  loop_block = WN_CreateBlock();

  loop_body = WN_Istore(rtype, 0, Make_Pointer_Type(MTYPE_To_TY(rtype)),
              tmp_wn2, src);
  
  WN_INSERT_BlockAfter(loop_block, WN_last(loop_block), loop_body);
  loop_body = loop_block;

  WN_INSERT_BlockAfter(block, WN_last(block), WN_CreateDO(index_wn, init_index, loop_test, index_incr, loop_body, NULL));
}
#endif
...
#ifdef TARG_NVISA
    // We use a loop to emulate the memset without constant size
  else {
       // no memset required for 0 size
       if (Is_Integer_Constant(size) && WN_const_val(size) == 0)
         return WN_COPY_Tree(var);
       TYPE_ID index_type = WN_rtype(size);
       ST *index_st = Gen_Temp_Symbol(MTYPE_TO_TY_array[index_type],
                                      "_memset_loop_index");

       // Get the alignment of var which decides the step length of the copy.
       INT32 copy_align = compute_copy_alignment(aux_compute_alignment(var), (TY_IDX)0, 0); 
       TYPE_ID quantum = compute_copy_quantum(copy_align);
       INT32 stepsize = MTYPE_RegisterSize(quantum);
         
       // index initializationn
       WN *init_index = WN_CreateStid(OPR_STID, MTYPE_V, index_type, 0,
                                      index_st, ST_type(index_st),
                                      WN_Intconst(index_type, 0), 0);

       WN *main_loop_size = size;
       if (stepsize > 1) {
         // main loop size = (size / stepsize) * stepsize
         main_loop_size = WN_Mpy(WN_rtype(size),
                                 WN_Div(WN_rtype(size), size,
                                        WN_Intconst(WN_rtype(size), stepsize)),
                                 WN_Intconst(WN_rtype(size), stepsize));
       }
       // Do multiple bytes mem operation according to alignment.
       create_emuloop(block, index_st, var, con, main_loop_size, init_index,
                      quantum, stepsize); 

       //Currently the con would not be constant, add this cond in case we change check_size() in the future and handle large size mem operation. 
       //accomplish leftover size bytes by bytes
       if (stepsize > 1 &&
           (!Is_Integer_Constant(size) ||
            (WN_const_val(size) % stepsize != 0))) {

          TYPE_ID desc = Mtype_TransferSign(quantum, MTYPE_I1);
          // init index = main loop size;
          WN *tmp_wn = WN_CreateStid(OPR_STID, MTYPE_V, index_type, 0,
                                     index_st, ST_type(index_st),
                                     WN_COPY_Tree(main_loop_size));

          create_emuloop(block, index_st, WN_COPY_Tree(var),
                         WN_COPY_Tree(con), WN_COPY_Tree(size),
                         tmp_wn, desc, 1);        
       }  
...
static WN *em_memcpy(WN *block, WN *tree, WN *dst, WN *src, WN *size)
{
  ...
  else 
  {
       ...
       ST *index_st = Gen_Temp_Symbol(MTYPE_TO_TY_array[index_type],
                                      "_memcpy_loop_index");
       TY_IDX srcTY = aux_compute_alignment(src);
       TY_IDX dstTY = aux_compute_alignment(dst);

       // Get the alignment of var which decides the step length of the copy.
       INT32 copy_align = compute_copy_alignment(srcTY, dstTY, 0); 
       TYPE_ID quantum = compute_copy_quantum(copy_align);
       INT32 stepsize = MTYPE_RegisterSize(quantum);
       ...
       WN *tmp_wn = WN_CreateLdid(OPR_LDID, index_type, index_type, 0,
                                  index_st, ST_type(index_st), 0);
       tmp_wn = WN_Add(index_type, src, tmp_wn);

       // src wn = src + index
       WN *src_wn = WN_Iload(quantum, 0, MTYPE_TO_TY_array[quantum], tmp_wn); 
       ...
       // Do multiple bytes mem operation according to alignment.
       create_emuloop(block, index_st, dst, src_wn, main_loop_size,
                      init_index, quantum, stepsize); 
       ...
       if (stepsize > 1 &&
          ...
          WN *tmp_wn = WN_CreateLdid(OPR_LDID, index_type, index_type, 0,
                                     index_st, ST_type(index_st), 0);
          tmp_wn = WN_Add(index_type, WN_COPY_Tree(src), tmp_wn);
           
          WN *src_wn = WN_Iload(desc, 0, MTYPE_TO_TY_array[desc], tmp_wn);

          create_emuloop(block, index_st, WN_COPY_Tree(dst), src_wn,
                         WN_COPY_Tree(size), init_index,
                         desc, 1);        
       }
