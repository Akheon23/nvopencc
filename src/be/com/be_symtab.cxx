...
ST_is_const_initialized (const ST* st)
{
	...
	return FALSE;
    if (ST_in_param_mem(st)) 
        // again may be readonly but don't know the initial value
        return FALSE;
    ...
    if (Target_ISA >= TARGET_ISA_compute_20
      && ST_in_global_mem(st) && ST_addr_taken(st))
    {
      // if was in constant but had addr taken, will be put in global
      if (ST_is_initialized(st) && strncmp(ST_name(st),"__cuda",6) == 0)
	return TRUE;
      else
	return FALSE;
...
ST_is_const_initialized_scalar(const ST *st, INT64 offset, TCON &tcon_copy)
{
    ...
    // if scalar then offset should be 0
...
Find_Lda (WN *tree)
{
  ...
  case OPR_LDID:
	if (WN_class(tree) == CLASS_PREG) {
	  ...
	  && ( ST_in_shared_mem(WN_st(tree)) || ST_in_param_mem(WN_st(tree)) ) )
	{
	  TY_IDX ty = WN_ty(tree);
	  if (TY_kind(ty) == KIND_STRUCT) {
              ty = Get_Field_Type(ty, WN_field_id(tree));
	  }
	  ...
	  if (TY_kind(ty) == KIND_POINTER) {
        ...
        else if ((ST_sclass(WN_st(tree)) == SCLASS_AUTO
               || ST_sclass(WN_st(tree)) == SCLASS_PSTATIC)
          && TY_kind(WN_ty(tree)) == KIND_POINTER
          && BE_ST_memory_pointed_to(WN_st(tree)) != MEMORY_UNKNOWN)
        {
...
// Search tree for a LDID of pointer, returning NULL if not found.
WN*
Find_Pointer_Ldid (WN *tree)
{
  if (tree == NULL) 
	return NULL;
  ...
  switch (WN_operator(tree)) {
  case OPR_LDID:
        ldid_ty = ST_type(WN_st(tree));
        // If we are reading field of structure, use field type
        if (Is_Structure_Type(ldid_ty)) {
          ldid_ty = Get_Field_Type(ldid_ty, WN_field_id(tree));
        }
        if (TY_kind(ldid_ty) == KIND_POINTER) {
          return tree;
        }
        else if (ST_class(WN_st(tree)) == CLASS_PREG
          && Preg_memory_pointed_to(WN_load_offset(tree)) != MEMORY_UNKNOWN)
        {
          // is preg holding pointer symbol
          return tree;
        }
        return NULL;
  case OPR_ILOAD:
        // ILOAD(LDA) is really just a LDID
        ldid = Find_Lda (WN_kid0(tree));
	if (ldid) {
          ldid_ty = ST_type(WN_st(ldid));
          if (Is_Structure_Type(ldid_ty)) {
            ldid_ty = Get_Field_Type(ldid_ty, WN_field_id(ldid));
          }
          if (TY_kind(ldid_ty) == KIND_POINTER) {
            return ldid;
          }
  ...
  case OPR_ADD:
  case OPR_SUB:
	ldid = Find_Pointer_Ldid (WN_kid0(tree));
	if (ldid) return ldid;
	ldid = Find_Pointer_Ldid (WN_kid1(tree));
	if (ldid) return ldid;
	return NULL;
  case OPR_SELECT:
	ldid = Find_Pointer_Ldid (WN_kid1(tree));
	if (ldid) return ldid;
	ldid = Find_Pointer_Ldid (WN_kid2(tree));
	if (ldid) return ldid;
	return NULL;
  default:
	return NULL;
