...
#ifdef TARG_NVISA
BOOL inline_has_debug_flag = FALSE;
#endif
                       ...
                       )
{
	...
	switch (DST_INFO_tag(DST_INFO_IDX_TO_PTR(child_idx))) {
...
#ifdef TARG_NVISA
         /* The following modified code is used to generate debugging information for
            formal parameters of inlined subroutines.

            In order to find the cloned variable for the formal parameter, 
            we need to go through two steps of indirection, from the cloned
            symbol table and/or map.

            Please note that Get_Cloned_ST() may not succeed as it looks for the
            ST from the _hash_map while not every ST has an entry in it. Only
            newly cloned symbols will have entry there. Variables without conflicts can
            only be accessed through Get_ST(). 
         */
#endif
 		...
 		DST_ASSOC_INFO_st_idx(DST_FORMAL_PARAMETER_st (attr))]);
            ...
            param_st = symtab->Get_Cloned_ST(&St_Table[DST_ASSOC_INFO_st_idx(DST_FORMAL_PARAMETER_st (attr))]); 
       
            if (inline_has_debug_flag && !param_st) {
               ST *tmp_st;
               param_st = symtab->Get_ST(&St_Table[DST_ASSOC_INFO_st_idx(DST_FORMAL_PARAMETER_st (attr))]);
               tmp_st = symtab->Get_Cloned_ST(param_st);
               if (!tmp_st) 
                   tmp_st = symtab->Get_ST(param_st);

              if (tmp_st && strcmp(ST_name(param_st), ST_name(tmp_st))==0)
                 param_st = tmp_st;
            }
#endif
        ...
        case DW_TAG_variable:
	    {
              ...
              dst_info= (DST_INFO *)DST_INFO_IDX_TO_PTR(child_idx);

#ifdef TARG_NVISA
              if (inline_has_debug_flag || (!(DST_IS_declaration(DST_INFO_flag(dst_info))) &&
                DST_IS_automatic(DST_INFO_flag(dst_info)))) {
...
#ifdef TARG_NVISA
         /* This modification is used to generate debugging information for local 
            variables of inlined subroutines. Similar to formal arguments, 
            Get_Cloned_ST() may not be able to find the cloned symbol from _hash_map,
            and Get_ST() should be used therefore.
          */

                local_st = symtab->Get_Cloned_ST(&St_Table[
			DST_ASSOC_INFO_st_idx(DST_VARIABLE_def_st (attr))]);
                if (inline_has_debug_flag && !local_st) 
                    local_st = symtab->Get_ST(&St_Table[
		   	    DST_ASSOC_INFO_st_idx(DST_VARIABLE_def_st (attr))]);
#else
                local_st = symtab->Get_Cloned_ST(&St_Table[
                        DST_ASSOC_INFO_st_idx(DST_VARIABLE_def_st (attr))]);
        ...
        case DW_TAG_inlined_subroutine:
	    {
            ...
            LABEL_IDX low_lbl = (LABEL_IDX) DST_ASSOC_INFO_st_index(
                                 DST_INLINED_SUBROUTINE_low_pc (attr));
            LABEL_IDX b_lbl = make_LABEL_IDX (
              LABEL_IDX_index(low_lbl) + symtab->Get_cloned_label_last_idx(),
              symtab->Get_cloned_level() );
            LABEL_IDX high_lbl = (LABEL_IDX) DST_ASSOC_INFO_st_index(
                                 DST_INLINED_SUBROUTINE_high_pc (attr));
            LABEL_IDX e_lbl = make_LABEL_IDX (
              LABEL_IDX_index(high_lbl) + symtab->Get_cloned_label_last_idx(),
              symtab->Get_cloned_level() );
	...
	case DW_TAG_lexical_block:
	    ...
	    ST_IDX high_pc;
#ifdef TARG_NVISA
            ST_IDX low_pc_label_idx;
            ST_IDX  cloned_low_pc_label_idx;
            ST_IDX high_pc_label_idx;
            ST_IDX cloned_high_pc_label_idx;
#endif
...
#ifdef TARG_NVISA
            low_pc_label_idx = DST_ASSOC_INFO_st_index(DST_LEXICAL_BLOCK_low_pc (attr));
            cloned_low_pc_label_idx = symtab->Get_Cloned_Label(low_pc_label_idx);

            high_pc_label_idx = DST_ASSOC_INFO_st_index(DST_LEXICAL_BLOCK_high_pc (attr));            
            cloned_high_pc_label_idx = symtab->Get_Cloned_Label(high_pc_label_idx);

            low_pc = make_ST_IDX(cloned_low_pc_label_idx,
                                 symtab->Get_cloned_level() );

            high_pc = make_ST_IDX(cloned_high_pc_label_idx,
                                  symtab->Get_cloned_level() );
			     ...
			     mUINT16 cross_file_id)
{
    ...
    // WARNING:  this is evil hack: they are putting label index&level
    // into index part of st, which cgdwarf then extracts st_index
    // to get whole label_idx which use for label_name.
    // Should instead pass label_idx, then cgdwarf extract whole idx,
    // but that causes more changes so wait till later. (mmurphy)
    low_pc = make_ST_IDX(
        begin_label,
        symtab->Get_cloned_level() );
