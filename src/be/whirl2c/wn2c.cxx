...
#ifdef TARG_NVISA
// if any identifier or anything starting with _, assume is call
#define WN2C_IS_INFIX_OP(opc) \
   ((WN2C_Opc2cname[opc]!=NULL)? (((WN2C_Opc2cname[opc][0])!='_') && !isalpha(WN2C_Opc2cname[opc][0])) : FALSE)
...
#ifdef TARG_NVISA
  {OPC_U8F4TRUNC, "(unsigned long long int)"},
#else
...
#endif
#ifdef TARG_NVISA
  // hack:  not really a call, but translates to 1.0f/(a)
  {OPC_F4RECIP, "1.0f/"},
#else
		...
		CONTEXT      context)
{
   ...
   TOKEN_BUFFER opnd_tokens1, opnd_tokens2;
   ...
   /* special case for COMIC on multi-core platform.
      _F8MAX(v1, v2) => ((v1)>(v2)?(v1):(v2))
      _F8MIN(v1, v2) => ((v2)<(v2)?(v1):(v2))
      _I8F8TRUNC(v) => ((long long int)(v))
      _I4F8TRUNC(v) => ((int )(v))
      May need to add more cases here.
   */

   if (opcode == OPC_F8MAX || opcode == OPC_F8MIN)
     {
       Append_Token_String(tokens, "((");
       opnd_tokens1 =
               WN2C_Translate_Arithmetic_Operand(wn0, descriptor_ty, context);
       Append_And_Copy_Token_List(tokens, opnd_tokens1);
       ...
       Append_And_Copy_Token_List(tokens, opnd_tokens2);
       Append_Token_String(tokens, ")?(");
       Append_And_Copy_Token_List(tokens, opnd_tokens1);
       Append_Token_String(tokens, "):(");
       Append_And_Copy_Token_List(tokens, opnd_tokens2);
       ...
       Reclaim_Token_Buffer(&opnd_tokens1);
       Reclaim_Token_Buffer(&opnd_tokens2);
       return EMPTY_STATUS;
     }

   if (opcode == OPC_I8F8TRUNC ||opcode== OPC_I4F8TRUNC)
     {

       switch (opcode) {
       case OPC_I8F8TRUNC:
#ifdef WIN32
            Append_Token_String(tokens, "((__int64)(");
#else
            Append_Token_String(tokens, "((long long int)(");
#endif
         break;
       case OPC_I4F8TRUNC:
       ...
       opnd_tokens1 =
               WN2C_Translate_Arithmetic_Operand(wn1, descriptor_ty, context);
       Append_And_Reclaim_Token_List(tokens, &opnd_tokens1);
       Append_Token_String(tokens, "))");
...
#ifdef TARG_NVISA
               /*
                 Do not allow size-based matching if base_ty is a structure.
                 This is to avoid generating illegal C code, e.g.:
                 "(unsigned long long)(res)". here,  type of res (i.e., base_ty)
                 is a union/structure and load_mtype is MTYPE_U8. By preventing
                 the below, we correctly generate:
...
#ifdef TARG_NVISA
    /* ignore anonymous structures */
    if (TY_anonymous(FLD_type(fld_stack.ar[i])) && TY_kind(FLD_type(fld_stack.ar[i])) == KIND_STRUCT)
       continue;
#endif
    Append_Token_Special(expr_tokens, '.');
    Append_Token_String(expr_tokens, FLD_name(fld_stack.ar[i]));
#ifndef TARG_NVISA
    /* for NVISA target, we have seen cases where the offset is folded even for the last field.
    */
...
#ifdef TARG_NVISA
     // It should be just an array acess with constant index. 
     if (TY_Is_Scalar(base_obj_ty) && !cur_offset) {
         /*If the object_ty is equal to base_obj_ty, it implies that we want to acess the element of array. 
           Then we do not have to anything. Otherwise, it implies that we want to access the address of the array element.
           Then we need to add the '&'*/
         if (object_ty != base_obj_ty) {
            Prepend_Token_Special(expr_tokens, '&');
         }
         goto ret_point;
     }
...
#ifndef TARG_NVISA
     /* For TARG_NVISA, the lda offset for the array field of a structure has already been
        expressed as a constant index into that array (e.g. s.arr[0]). So the resulting expression
        is an lvalue (which it would not have been, if we had generated "s.arr" for offset=0)
     */
		       ...
		       CONTEXT       context)
{
   ...
   if (!WN2C_compatible_lvalues(assign_ty, rhs_ty))
   {
...
#ifdef TARG_NVISA
   const char * intrinsic_name = WN_intrinsic_name((INTRINSIC)WN_intrinsic(wn));
   if (strcmp(intrinsic_name, "__cudaGetSharedMem") == 0) {
     /*
      * we special case the cudaGetSharedMem intrinsics by specifying
      * the parameter type as "void *", since this information is not
      * available otherwise.
      */
     TY_IDX       parm_ty = Stab_Pointer_To(MTYPE_To_TY(MTYPE_V));
     CONTEXT_set_given_lvalue_ty(context, parm_ty);
   }
#endif
...
WN2C_cvt(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
...
#ifdef TARG_NVISA
     /* we don't alway have intptr_t in scope. size_t is alway there */
     Prepend_Token_String(expr_tokens, "(size_t)");
#else
   ...
   {
     /*
        Optimizations like copy-propagation may cause the kid's mtype
        to be different than the desc mtype of the CVT. If this
        happens for scalar types, and the signedness of the desc mtype
        and tree_expression_mtype are different, we need an additional cast 
        to convert the tree_expression_mtype to the desc_mtype.

        E.g. consider this code snippet:
            unsigned long long j;
            unsigned long long result;

            unsigned temp = (-(signed int)(x));
            result = (unsigned long long)temp * j;

        Without copy-prop, the IR for the result stmt is:
            STID result <-- U8U4CVT(temp) * j
        
        After copy-prop, the IR becomes:
            STID result <-- U8U4CVT (NEGI4 (x)) * j

        Without this fix, whirl2c will generate:
            result = (unsigned long long)(-(signed int)(x)) * j;
        
        but the correct expression is:
            result = (unsigned long long) ((unsigned)(-(signed int)(x))) * j;
     */
     TY_IDX ty = WN_Tree_Type(WN_kid0(wn));
     if (TY_kind(ty) == KIND_SCALAR && 
         MTYPE_is_signed(WN_opc_dtype(wn)) ^ MTYPE_is_signed(TY_mtype(ty)))
     {
        // cast to desc mtype
        WN2C_prepend_cast(expr_tokens, Stab_Mtype_To_Ty(WN_opc_dtype(wn)), FALSE);
        WHIRL2C_parenthesize(expr_tokens);
     }
   }
#endif
...
WN2C_ldid(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
   ...
   if (ST_sym_class(WN_st(wn)) == CLASS_PREG)
   {
	 ...
	 case MTYPE_C10:
	 case MTYPE_CQ:
            sprintf(buffer, "_w2c_reg%d",
              RETURN_INFO_preg(Get_Return_Info(WN_ty(wn), Use_Simulated), 0));
...
#ifdef TARG_NVISA
  Append_Token_String(tokens,"__cuda_alloc");
#else
