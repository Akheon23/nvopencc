...
#ifdef TARG_NVISA
#define USE_REAL_NAMES  /* emit real type names rather than whirl2c.h names */
#endif
...
TY2C_prepend_filler_field(TOKEN_BUFFER decl_tokens, INT64 byte_size)
{
   ...
   Append_Token_String(field_tokens,
                       ...
                       Scalar_C_Names[MTYPE_U1].real_name);
#else
...
TY2C_scalar(TOKEN_BUFFER decl_tokens, TY_IDX ty, CONTEXT context)
{
...
#ifdef TARG_NVISA
   // TY_is_character was designed just for fortran,
   // but we also use it to say that this was a plain char,
   // as opposed to a signed or unsigned char
   // (which are separate types in C).
   else if (TY_is_character(ty)) {
       Prepend_Token_String(decl_tokens, "char");
   }
   // front end creates a separate type for long; rather than use a ty flag,
   // just check for name being "long {unsigned }int"
   else if (strncmp(TY_name(ty), "long ", 5) == 0) {
     INT32 mtype = TY_mtype(ty);
     if (MTYPE_is_unsigned(mtype))
       Prepend_Token_String(decl_tokens, "unsigned long");
     else
   ...
   else
   {
	 Prepend_Token_String(decl_tokens, 
#ifdef USE_REAL_NAMES
                              Scalar_C_Names[TY_mtype(ty)].real_name);
#else
...
#ifdef TARG_NVISA
   if (TY_is_volatile(ty) && TY_is_volatile(TY_AR_etype(ty))) {
     CONTEXT_set_volatile(context);
   }
#endif //TARG_NVISA
   TY2C_prepend_qualifiers(decl_tokens, ty, context);
#ifdef TARG_NVISA
   if (TY_is_volatile(ty) && TY_is_volatile(TY_AR_etype(ty))) {
     CONTEXT_reset_volatile(context);
   }
#endif //TARG_NVISA
} /* TY2C_array */
...
#ifdef TARG_NVISA
static void TY2C_Output_Struct_Type(TOKEN_BUFFER typedef_tokens, TY_IDX ty,
                             INT lines_between_decls,
                             CONTEXT context) {

    // don't need hash set, just check flag
    ...
    // usually we output typedef to .h file,
    // but for anonymous local structs we want them in the .c file
    // since we won't have the same typedef in the real .h file.
    if (TY_anonymous(ty))
      Prepend_And_Reclaim_Token_List(typedef_tokens, &tmp_tokens);
    else
...
{
  TOKEN_BUFFER typedef_tokens = New_Token_Buffer();
  if (!TY_is_translated_to_c(ty)) {
    //Add this struct type to the global w2c.h
    CONTEXT_reset_incomplete_ty2c (context);
#ifdef TARG_NVISA
    TY2C_Output_Struct_Type(typedef_tokens, ty, 1, context);
#else
  ...
  TY2C_prepend_qualifiers(decl_tokens, ty, context);
#ifdef TARG_NVISA
  Prepend_And_Reclaim_Token_List(decl_tokens, &typedef_tokens);
#endif
} /* TY2C_struct */
