...
void
dump_tokens(FILE *f, TOKEN_BUFFER buf, BOOL with_token_name)
{
   const char *str;
   STRING_IDX  c;
   TOKEN_IDX   t;
   ...
   for (t = buf->token_list.first; t != NO_TOKEN_IDX; t = TOKEN_next(a_token))
   {
	 ...
	 break;
      ...
      case SPECIAL_TOKEN:
	 if (with_token_name)
	    fputs("SPECIAL_TOKEN(", f);
	 fputc(TOKEN_char(a_token), f);
	 break;
...
{
  dump_tokens(DBGOUT, buf, with_token_name);
}
