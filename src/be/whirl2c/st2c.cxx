...
#ifdef TARG_NVISA
   // on Linux64, we initialize all pointer and scalar variables
   // since the gcc is too picky.
   if ((ST_sym_class(st) == CLASS_VAR) && (ST_sclass(st) == SCLASS_AUTO)) {
     if (TY_kind(ty) == KIND_POINTER ||
         TY_kind(ty) == KIND_SCALAR) {
       Append_Token_String(decl_tokens, "= 0");
     }
   }
#endif
