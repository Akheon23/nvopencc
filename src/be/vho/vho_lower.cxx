...
static WN * vho_lower_stid ( WN * wn, WN * block );
 ...
 *
 * Return last field_id in case call this recursively (nested structs).
 * ============================================================================
 */
...
Find_Ldid_Under_Iload (WN *iload_kid)
{
    if (Is_Ldid_Or_Lda(iload_kid)) {
    ...
    else if (WN_operator(iload_kid) == OPR_ILOAD) {
      ...
      else if (WN_operator(WN_kid0(iload_kid)) == OPR_ILOAD) {
        return Find_Ldid_Under_Iload (WN_kid0(iload_kid));
      }
      else if (WN_operator(WN_kid1(iload_kid)) == OPR_ILOAD) {
        return Find_Ldid_Under_Iload (WN_kid1(iload_kid));
      }
    }
    ...
    if (TY_kind(src_ty_idx) == KIND_STRUCT
      ...
      if ( VHO_Struct_Can_Be_Lowered && VHO_Struct_Nfields) {
        ...
        for (i = 0; i < VHO_Struct_Nfields; i++) {
          ...
          INT src_wn_field_id = VHO_Struct_Field_Id_Table[i];    
          INT dst_wn_field_id = dst_field_id - src_field_id + VHO_Struct_Field_Id_Table[i];

              // If the initial field id is zero and 
              // The WN type does not agree with the symbol type, 
              // it is a nested structure field access and the computed field id is 
              // not reliable. We just keep it as zero. 

          if (!src_field_id && !src_is_pointer) {
              TY_IDX real_sym_ty_idx; 
              real_sym_ty_idx = ST_type(src_st);

              while (TY_kind(real_sym_ty_idx) == KIND_ARRAY) {
                  real_sym_ty_idx = TY_etype(real_sym_ty_idx);
              }

              if (!Equivalent_Types(src_ty_idx, real_sym_ty_idx, QUAL_IGNORE)) {
                   src_wn_field_id = 0; 
              }
          }
          ...
          if (!dst_field_id) {
              TY_IDX real_sym_ty_idx; 
              real_sym_ty_idx = ST_type(dst_st);

              while (TY_kind(real_sym_ty_idx) == KIND_ARRAY) {
                  real_sym_ty_idx = TY_etype(real_sym_ty_idx);
              }

              if(!Equivalent_Types(dst_ty_idx, real_sym_ty_idx, QUAL_IGNORE)) {
                  dst_wn_field_id = 0; 
              }
          }
          ...
          if (src_is_pointer) {
              ...
              opc = OPCODE_make_op ( OPR_LDID,
                                     Promoted_Mtype [TY_mtype(fty_idx)],
                                     TY_mtype(fty_idx) );
              if (VHO_Struct_Field_Is_Array_Table[i] || !src_wn_field_id) {
          ...
          opc = OPCODE_make_op ( OPR_STID, MTYPE_V, TY_mtype(fty_idx) );
          if (VHO_Struct_Field_Is_Array_Table[i] || !dst_wn_field_id) {
              // each element uses same field_id,
              // so rather than create array refs, just do element copies
...
VHO_Lower_Mistore ( WN * wn )
{
  ...
  if ( VHO_Struct_Opt
    ...
    if (WN_opcode(src_value) == OPC_MMLDID) {
      ...
      TY_IDX src_st_idx= ST_type(WN_st(src_value));

      if ( ST_class(WN_st(src_value)) != CLASS_PREG ) {
          src_ldid = src_value;
      } else {
#if 1
          if (TY_kind(src_ty_idx) == KIND_STRUCT) {

            FLD_ITER iter = Make_fld_iter (TY_fld (src_ty_idx));
            if (FLD_last_field (iter)) {
               /*If the symbol in MMLDID is a PREG,the structure only has one field, it is just a scalar load. */
               WN *src; 
               FLD_HANDLE fld (iter);
  
               OPCODE src_opc = OPCODE_make_op ( OPR_LDID,
                                     Promoted_Mtype [TY_mtype(src_st_idx)],
                                     TY_mtype(src_st_idx) );
  
               src = WN_CreateLdid ( src_opc, 
                                  WN_offset(src_value),
                                  WN_st(src_value), src_st_idx );
               WN_kid0(wn)= src; 
               WN_Delete(src_value);
               src_value = src; 
           }
        ...
        for ( i = 0; i < VHO_Struct_Nfields; i++ ) {
          ...
          if (src_is_pointer) {
            ...
            if (VHO_Struct_Field_Is_Array_Table[i] || !src_wn_field_id) {
              ...
              src = WN_CreateLdid ( opc, 
                                    src_offset + VHO_Struct_Offset_Table[i],
                                    src_st, src_ty_idx, src_wn_field_id);
            }
          }
          opc = OPCODE_make_op ( OPR_ISTORE, MTYPE_V, TY_mtype(fty_idx));
          if (VHO_Struct_Field_Is_Array_Table[i] || !dst_wn_field_id) {
...
*/
  ...
  if ( comma_block ) {
	...
	{ // Use a temporary symbol instead of a preg
	  ST * comma_st = Gen_Temp_Symbol (MTYPE_TO_TY_array[rtype], "comma");
...
vho_lower_cselect ( WN * wn_cselect, WN * block, BOOL_INFO * bool_info )
{
  ...
  if (    WHIRL_Mldid_Mstid_On && opcode == OPC_MCSELECT && test != NULL ) {
    ...
    WN_Set_Linenum (wn, VHO_Srcpos);
    wn = vho_lower_stid (wn, cflow_block1);
    WN_INSERT_BlockLast (cflow_block1, wn);

    wn = WN_CreateStid (OPR_STID, MTYPE_V, MTYPE_M,
                        0, ST_st_idx (temp), ty_idx, rwn);
    WN_Set_Linenum (wn, VHO_Srcpos);
    wn = vho_lower_stid (wn, cflow_block2);
...
#endif
#ifdef TARG_NVISA
        // add more efficient cause OCG can fold into shift
        wn  = WN_Add (rtype, lshift, rshift);
#else
...
static const char * syscalls[NUM_SYSCALLS] = {"vprintf", "malloc", "free"};

static BOOL
func_is_syscall (ST *st)
{
  for (INT i = 0; i < NUM_SYSCALLS; ++i) {
    if (strcmp(ST_name(st), syscalls[i]) == 0) 
      return TRUE;
  }
  return FALSE;
}
...
vho_lower_call ( WN * wn, WN * block )
{
  ...
  if (WN_operator(wn) != OPR_ICALL 
    && ST_sclass(WN_st(wn)) == SCLASS_EXTERN
    && !CLIST_enabled  // okay if producing C
    && !Run_ipl) // okay if runing IPL
  {
    if (Target_ISA >= TARGET_ISA_compute_20
      && TY_has_prototype(ST_type(WN_st(wn)))
      && func_is_syscall(WN_st(wn)))
    {
      ; // okay if sm_2x and syscall
    } else {
