...
#if defined(__GNUC__) && defined(SHARED_BUILD)
...
#ifdef KEY
extern void Set_addr_saved_expr(WN *, BOOL, BOOL for_cg = FALSE);
#endif
...
static void F90_Lower_Init_Dep_Info(DEP_INFO *d, INT ndim) 
{
   ...
   /* allocate the main structure */
   SET_DEP_NDIM(d,ndim);
   SET_DEP_SUMMARY(d,DEP_INDEPENDENT_F90);
...
#define SHOW_DEP_RES  if (trace_depinfo) {fprintf(TFile,"M :");  print_dep_info(in1);}
   ...
   if (sum2 == DEP_INDEPENDENT_F90) {
      SHOW_DEP_RES;
      return (TRUE);
   }
   if (sum2 == DEP_IDENTICAL) {
      if (sum1 == DEP_INDEPENDENT_F90) {
   ...
   /* sum2 is REMOVABLE */
   if (sum1 == DEP_IDENTICAL || sum1 == DEP_INDEPENDENT_F90) {
      *in1 = *in2;
      SHOW_DEP_RES;
      return (TRUE);
   }
...
static const char * f90_depsum_name(DEP_SUMMARY d)
{
   switch (d) {
    ...
    case DEP_INDEPENDENT_F90: return ("INDEPENDENT");
...
static DEP_SUMMARY Analyze_all_indices(WN *lhs, WN *rhs, DEP_INFO *kid_dep)
{
   ...
   for (i=0; i < ndim; i++) {
      ...
      if (dir == DIR_DONTCARE) {
	 SET_DEP_SUMMARY(kid_dep,DEP_INDEPENDENT_F90);
	 allzero = FALSE;
 ...
 *                DEP_INDEPENDENT_F90 if the two do not overlap
...
static DEP_SUMMARY analyze_structure_bases(INT lhs_start, INT rhs_start, INT *lhs_end1, INT *rhs_end1)
{
    ...
    */
   if (lhs_ty == rhs_ty) {
      if (lc != rc) return (DEP_INDEPENDENT_F90);
   } else {
      return (DEP_UNKNOWN);
   }
   ...
   while (ri > rhs_end && li > lhs_end) {
	 ...
	 if (t == DEP_UNKNOWN || t == DEP_INDEPENDENT_F90) {
	    return (t);
...
* DEP_INDEPENDENT_F90 - the bases are different and non-overlapping
...
static DEP_SUMMARY Analyze_bases(WN * addr, BOOL is_expr, BOOL is_char)
{
   ...
   if (!((lhs_flag | rhs_flag) & F90_INDIRECTION)) {
	 ...
	 return (DEP_INDEPENDENT_F90);
      ...
      if (r == DEP_INDEPENDENT_F90 || r == DEP_UNKNOWN) {
			    ...
			    WN *parent, INT kidno, INT lhsdim, BOOL is_char_load)
{
   ...
   if (opr == OPR_LDID) {
	 ...
	 if (rhs_sym != NULL && !(rhs_flag & F90_ARRSECTION) && parent) {
	    ...
	    SET_DEP_SUMMARY(dep,DEP_INDEPENDENT_F90);
	 } else {
	    SET_DEP_SUMMARY(dep,DEP_UNKNOWN);
	 }
	 return;
      } else if (base_dep == DEP_INDEPENDENT_F90) {
...
static void f90_analyze_assignment(WN *stmt, WN *block, BOOL is_call)
{
   ...
   if (rhs) {
	 ...
	 Dependence_Walk(WN_kid(lhs_arrsection,i),adata,&lhs_dep,FALSE,lhs_arrsection,i,lhsdim,FALSE);
      ...
      if (DEP_SUMMARY(&lhs_dep) != DEP_INDEPENDENT_F90) {
