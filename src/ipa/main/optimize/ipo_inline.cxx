...
#if defined(TARG_NVISA) || (!defined(_STANDALONE_INLINER) && !defined(_LIGHTWEIGHT_INLINER))
...
#ifdef TARG_NVISA
extern BOOL inline_has_debug_flag;  // generate debugging info for inlined subroutines
#endif
...
IPO_INLINE::Pre_Process_Caller (LABEL_IDX& return_label)
{
    ...
    if (
#ifdef TARG_NVISA
          inline_has_debug_flag ||
#endif
          Callee_Summary_Proc()->Has_early_returns()
        )
...
#else
    // by default, the restrict attribute is propagated, the
    // flag could be used to change the behaviour
    if (IPA_Enable_Inliner_prop_restrict == FALSE) {
      Clear_TY_is_restrict (copy_st->u2.type);
    }
    ST_set_restrict_group_id (copy_st, Caller_node()->Get_current_restrict_gid());
#endif
#endif
...
#ifdef TARG_NVISA
    if (WN_num_formals(callee) > 0) {
      INT32 gid = Caller_node()->Get_current_restrict_gid() + 1;
      Caller_node()->Set_current_restrict_gid (gid);
    }
#endif
