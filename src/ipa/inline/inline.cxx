  ...
  if (&caller == &callee)
    {
      ...
      if (Target_ISA < TARGET_ISA_compute_20) {
        ErrMsgSrcpos(EC_Ipa_Recursive_Call,
          WN_Get_Linenum(edge.Whirl_Node()), cplus_demangle(callee.Name(), 0));
      }
      else if (callee.Has_Must_Inline_Attrib()) {
        // must inline, but is recursive
        ErrMsgSrcpos(EC_Ipa_Inline_Recursive_Call, 
          WN_Get_Linenum(edge.Whirl_Node()), cplus_demangle(callee.Name(), 0));
      }
#endif
  ...
  if (IPA_Call_Graph->Graph()->Is_Recursive_Edge (edge.Edge_Index ()))
    {
      ...
      else if (edge.Has_Must_Inline_Attrib()) {
        // must inline, but is recursive
        ErrMsgSrcpos(EC_Ipa_Inline_Recursive_Call, 
          WN_Get_Linenum(edge.Whirl_Node()), cplus_demangle(callee.Name(), 0));
      }
