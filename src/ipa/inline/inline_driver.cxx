...
#ifdef TARG_NVISA
/* Debugging flag to control code generation */
extern BOOL inline_has_debug_flag ;
#endif
...
Process_Command_Line (INT argc, char **argv)
{
    ...
    for ( i = 1; i < argc; i++ ) {
	  ...
	  continue;
		...
		case '3':
#ifdef TARG_NVISA
                    inline_has_debug_flag = TRUE;
#endif

		    if (argv[i][3] != 0)
			break;
		    /* else, fall through */
#if 0           /* This was ifndef linux, but not what we want for windows,
                 * was probably only for irix.
                 * Also not clear what was meant here;
                 * if want only -g to turn off DFE, then above should be == 0;
                 * as it is, it always falls through to this.
                 */
