...
SUMMARIZE<program>::Summarize (WN *w)
{
  ...
  if (!Has_alt_entry ()) {
    ...
    Chi_To_Idx_Map =
	CXX_NEW (CHI_CR_TO_INT_MAP (113, ipl::ptr_hash<CODEREP> (),
				    std::equal_to<CODEREP*> (), &Temp_pool),
		 &Temp_pool);
    Phi_To_Idx_Map =
	CXX_NEW (PHI_NODE_TO_INT_MAP (113, ipl::ptr_hash<PHI_NODE> (),
...
hash_of_full_path_name (int index)
{
  ...
  char *fullname = new char[10 + strlen(cwd) + strlen(Src_File_Name) + 1 + 1];
  sprintf (fullname, "%d%s/%s", index, cwd, Src_File_Name);
  __gnu_cxx::hash<char*> hfn;

  if (malloced) free (cwd);

  unsigned int h = (unsigned int) hfn (fullname);
  delete fullname;
