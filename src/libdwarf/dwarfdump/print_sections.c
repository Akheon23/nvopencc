...
print_line_numbers_this_cu(Dwarf_Debug dbg, Dwarf_Die cu_die)
{
	...
	for (i = 0; i < linecount; i++) {
	    ...
	    printf("%s:\t[%3" LL_FORMAT "u,%2" LL_FORMAT "d]\t%#" LL_FORMAT "x",
                   filename, lineno, column, pc);
	    if (sres == DW_DLV_OK)
		       ...
		       int code_alignment_factor, Dwarf_Half addr_size)
{
	...
	case DW_CFA_offset:
	    ...
	    printreg(reg);
	    printf(" %" LL_FORMAT "d", (signed long long)
		   (((Dwarf_Signed) uval) * data_alignment_factor));
	    if (verbose) {
		printf("  (%" LL_FORMAT "u * %d)", (unsigned long long) uval,
	...
	default:
		...
		off += addr_size;
		printf("\t%2u DW_CFA_set_loc %" LL_FORMAT "u\n",
		...
		len -= 1;
		off += 1;
		printf("\t%2u DW_CFA_advance_loc1 %" LL_FORMAT "u\n",
		...
		len -= 2;
		off += 2;
		printf("\t%2u DW_CFA_advance_loc2 %" LL_FORMAT "u\n",
		       loff, (unsigned long long) uval2);
		break;
		...
		len -= 4;
		off += 4;
		printf("\t%2u DW_CFA_advance_loc4 %" LL_FORMAT "u\n",
		       loff, (unsigned long long) uval2);
		break;
		...
		len -= 8;
		off += 8;
		printf("\t%2u DW_CFA_MIPS_advance_loc8 %" LL_FORMAT "u\n",
		       loff, (unsigned long long) uval2);
		break;
		...
		if (verbose) {
		    printf("  (%" LL_FORMAT "u * %d)", (unsigned long long) uval2,
			   (int) data_alignment_factor);
		}
		printf("\n");
		break;
		...
		printreg(uval);
		printf(" %" LL_FORMAT "u", (unsigned long long) uval2);
		printf("\n");
		break;
		...
		len -= uleblen;
		off += uleblen;
		printf("\t%2u DW_CFA_def_cfa_offset %" LL_FORMAT "u\n",
		       loff, (unsigned long long) uval);
		break;
	    ...
	    case DW_CFA_GNU_args_size:{
			...
			local_dwarf_decode_u_leb128(instp + 1,
						    &uleblen);
		    printf
			("\t%2u DW_CFA_GNU_args_size arg size: %" LL_FORMAT "u\n",
...
print_frames(Dwarf_Debug dbg)
{
	...
	if (fres == DW_DLV_ERROR) {
		...
		printf
		    ("<%3" LL_FORMAT "d><%#" LL_FORMAT "x:%#" LL_FORMAT "x><%s><fde offset 0x%" LL_FORMAT "x length: 0x%" LL_FORMAT "x>",
		...
		if (eh_table_offset == DW_DLX_NO_EH_OFFSET) {
		    ...
		    printf("<eh offset 0x%" LL_FORMAT "x>\n", eh_table_offset);
		}
		...
		if (verbose > 1) {
			...
			_dwarf_fde_section_offset(dbg, fde_data[i],
						  ...
						  &err);
		    if (offres == DW_DLV_OK) {
			printf("\tfde sec. offset %" LL_FORMAT "u 0x%" LL_FORMAT "x"
			       " cie offset for fde: %" LL_FORMAT "u 0x%" LL_FORMAT "x\n",
			...
			if (cires == DW_DLV_ERROR) {
			    printf
				("Bad cie index %" LL_FORMAT "d with fde index %" LL_FORMAT "d!\n",
		...
		for (i = 0; i < cie_element_count; i++) {
		    ...
		    if (verbose) {
			printf("<%3" LL_FORMAT "d>\tversion\t\t\t\t%d\n", i,
			       version);
			cires =
			    _dwarf_cie_section_offset(dbg, cie_data[i],
						      &cie_off, &err);
			if (cires == DW_DLV_OK) {
			    printf("\tcie sec. offset %" LL_FORMAT "u 0x%" LL_FORMAT "x\n",
				   (unsigned long long) cie_off,
				   (unsigned long long) cie_off);
			...
			printf("\tcode_alignment_factor\t\t%" LL_FORMAT "u\n",
			       code_alignment_factor);
			printf("\tdata_alignment_factor\t\t%" LL_FORMAT "d\n",
			       data_alignment_factor);
			printf("\treturn_address_register\t\t%d\n",
			       return_address_register_rule);
			printf
			    ("\tbytes of initial instructions:\t%" LL_FORMAT "d\n",
			     (long long) initial_instructions_length);
			printf("\tcie length :\t\t\t%" LL_FORMAT "d\n",
...
print_pubnames(Dwarf_Debug dbg)
{
	...
	for (i = 0; i < count; i++) {
	    ...
	    printf("global %-15s die %" LL_FORMAT "d, cu-die %" LL_FORMAT "d,"
		   " off-in-cu %" LL_FORMAT "d, cu %" LL_FORMAT "d",
		...
		if ((die_off - die_CU_off) != off) {
		    printf(" error: real cuhdr %" LL_FORMAT "u", off);
		    exit(1);
		}
		...
		struct Dwarf_Macro_Details_s *mdp)
{
    ...
    if(mdp->dmd_macro) {
        printf( "%3ld %s: %6" LL_FORMAT "u %2" LL_FORMAT "d [%4" LL_FORMAT "d] \"%s\" \n",
                        i,
                        type,
                        mdp->dmd_offset,
                        mdp->dmd_fileindex,
        ...
        printf( "%3ld %s: %6" LL_FORMAT "u %2" LL_FORMAT "d [%4" LL_FORMAT "d] 0\n",
                        i,
                        type,
                        mdp->dmd_offset,
                        mdp->dmd_fileindex,
...
print_locs(Dwarf_Debug dbg)
{
    ...
    while ((lres = dwarf_get_loclist_entry(dbg, offset,
					   ...
					   &next_entry,
					   &err)) == DW_DLV_OK) {
	printf("\t <obel> 0x%08" LL_FORMAT "x 0x%09" LL_FORMAT "x " "0x%08" LL_FORMAT "x " "%8" LL_FORMAT "d\n",
...
print_abbrevs(Dwarf_Debug dbg)
{
	...
	if (attr_count == 0) {
	    ...
	    if (dense) {
		printf("<%" LL_FORMAT "d><%" LL_FORMAT "d><%" LL_FORMAT "d><%s>\n",
		       abbrev_num,
		       offset, (signed long long) /* abbrev_code */ 0,
		       "null .debug_abbrev entry");
	    } else {
		printf("<%4" LL_FORMAT "d><%5" LL_FORMAT "d><code: %2" LL_FORMAT "d> %-20s\n",
		       abbrev_num,
		       offset, (signed long long) /* abbrev_code */ 0,
		       "null .debug_abbrev entry");
	    }
	...
	if (dense)
	    printf("<%" LL_FORMAT "d><%" LL_FORMAT "d><%" LL_FORMAT "d><%s>", abbrev_num,
		   offset, abbrev_code, get_TAG_name(dbg, tag));
	else
	    printf("<%4" LL_FORMAT "d><%5" LL_FORMAT "d><code: %2" LL_FORMAT "d> %-20s", abbrev_num,
		   offset, abbrev_code, get_TAG_name(dbg, tag));
	++abbrev_num;
...
print_strings(Dwarf_Debug dbg)
{
    ...
    while ((sres = dwarf_get_str(dbg, offset, &name, &length, &err))
	   == DW_DLV_OK) {
	printf("name at offset %" LL_FORMAT "d, length %" LL_FORMAT "d is %s\n",
	       offset, length, name);
	offset += length + 1;
    }
    if (sres == DW_DLV_ERROR) {
...
print_aranges(Dwarf_Debug dbg)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (dres != DW_DLV_OK) {
		    ...
		    printf("\narange starts at %" LL_FORMAT "x, "
			   "length of %" LL_FORMAT "d, cu_die_offset = %" LL_FORMAT "d",
...
print_static_funcs(Dwarf_Debug dbg)
{
	...
	for (i = 0; i < count; i++) {
	    ...
	    printf("static-func %-15s die %" LL_FORMAT "d, cu-die %" LL_FORMAT "d,"
		   " off-in-cu %" LL_FORMAT "d, cu %" LL_FORMAT "d",
		   name, (long long) die_off, (long long) cu_off,
		   /* the cu die offset */
		   (long long) die_CU_off,
...
print_static_vars(Dwarf_Debug dbg)
{
	...
	for (i = 0; i < count; i++) {
	    ...
	    printf("static-var %-15s die %" LL_FORMAT "d, cu-die %" LL_FORMAT "d,"
		   " off-in-cu %" LL_FORMAT "d, cu %" LL_FORMAT "d",
		   name, (long long) die_off, (long long) cu_off,
		   /* the cu die offset */
		   (long long) die_CU_off,
...
print_types(Dwarf_Debug dbg)
{
	...
	for (i = 0; i < count; i++) {
	    ...
	    printf("type %-15s die %" LL_FORMAT "d, cu-die %" LL_FORMAT "d,"
		   " off-in-cu %" LL_FORMAT "d, cu %" LL_FORMAT "d",
		   name, (long long) die_off, (long long) cu_off,
		   /* the cu die offset */
		   (long long) die_CU_off,
...
print_weaknames(Dwarf_Debug dbg)
{
	...
	for (i = 0; i < count; i++) {
	    ...
	    printf("weakname %-15s die %" LL_FORMAT "d, cu-die %" LL_FORMAT "d,"
		   " off-in-cu %" LL_FORMAT "d, cu %" LL_FORMAT "d",
		   name, (long long) die_off, (long long) cu_off,
		   /* the cu die offset */
		   (long long) die_CU_off,
...
{
    if (reg > (sizeof(regnames)*sizeof(char*)))
      printf("%llx", reg);
    else
