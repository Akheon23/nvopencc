...
$Header: /var/tmp/pathscale-compiler-sources-2.2.1-2005.08.05/kpro64/libdwarf/dwarfdump/print_die.c 1.1 05/04/27 11:15:37-07:00 bos@eng-25.pathscale.com $ */
#include "globals.h"
	      ...
	      char **srcfiles, Dwarf_Signed cnt)
{
	...
	if (indent_level == 0) {
		...
		printf
		    ("\nCOMPILE_UNIT<header overall offset = %" LL_FORMAT "u>:\n",
	...
	if (dense) {
	    ...
	    if (indent_level == 0) {
		printf("<%d><%" LL_FORMAT "u+%" LL_FORMAT "u><%s>", 
                  indent_level, overall_offset - offset, offset, tagname);
	    } else {
		printf("<%d><%" LL_FORMAT "u><%s>", 
                  indent_level, offset, tagname);
	    }
	} else {
	    printf("<%d><%5" LL_FORMAT "u>\t%s\n", 
              indent_level, offset, tagname);
	}
    }
			 ...
			 char *start_buf, char **out_buf)
{
	...
	sprintf(p, "<lowpc=0x%" LL_FORMAT "x>",
		(unsigned long long) llbuf->ld_lopc);
	p += strlen(p);
	sprintf(p, "<highpc=0x%" LL_FORMAT "x>",
		(unsigned long long) llbuf->ld_hipc);
	p += strlen(p);
	if (verbose) {
	    sprintf(p, "<from %s offset 0x%" LL_FORMAT "x>",
		    llbuf->
	...
	if (op >= DW_OP_breg0 && op <= DW_OP_breg31) {
	    sprintf(p, "%+" LL_FORMAT "d", (Dwarf_Signed) opd1);
	    p += strlen(p);
	} else {
	    switch (op) {
	    case DW_OP_addr:
		sprintf(p, " %#" LL_FORMAT "x", opd1);
		p += strlen(p);
		break;
	    case DW_OP_const1s:
	    case DW_OP_const2s:
		...
		sprintf(p, "%+" LL_FORMAT "d", (Dwarf_Signed) opd2);
		p += strlen(p);
		break;
	    default:
		break;
	       ...
	       char **srcfiles, Dwarf_Signed cnt)
{
	...
	if (bres == DW_DLV_OK) {
	    sprintf(attrib_buf, "%#" LL_FORMAT "x", (unsigned long long) addr);
	} else {
	...
	if (bres == DW_DLV_OK) {
	    sprintf(attrib_buf, "<global die offset %" LL_FORMAT "u>",
	...
	sprintf(attrib_buf, "<%" LL_FORMAT "u>", off);
	...
	if (fres == DW_DLV_ERROR) {
		...
		if (wres == DW_DLV_OK) {
		    /* attrib_buf is large compared to %llu output, so
		       sprintf is safe */
		    sprintf(attrib_buf, "%" LL_FORMAT "u", tempud);
		...
		if (wres == DW_DLV_OK) {
			...
			tempsd = i;
			sprintf(attrib_buf, "%" LL_FORMAT "d", tempsd);
		    } else {
			sprintf(attrib_buf, "%" LL_FORMAT "u", tempud);
		    }
		} else if (wres == DW_DLV_NO_ENTRY) {
	...
	sprintf(attrib_buf, "%s", get_FORM_name(dbg, theform));
	break;
    default:
