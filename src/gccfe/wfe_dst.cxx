...
#ifdef TARG_NVISA
extern tree lookup_name (tree);                 // in c-decl.c
#endif
}
...
#define MAX_SCOPE_LEVEL  128
/* Support nested scopes for debugger 
   Basically, the compiler will generate DW_TAG_lexical_block whemever a nested block
   has local variable declarations. Right now, the DW_TAG_variable is attached to the
   DW_TAG_subprogram. In another word, all local variables in a subroutine are promoted 
   to the same level, such that debuggers cannot distinguish them by their original name
   (in the source code).

   gcc-3.4/open64 generates the DW_TAG_variable tags during the parsing phase, while 
   handles scope_stmt and generates WHIRL tree in a second phase. In order to generate 
   correct DW_TAG_lexical_block, a stack with two level link list are used here.
   
   At each scope level, there might be multiple blocks that have local variable declarations.
   They form a linked list. Under each block, multiple variable tags are linked together.

   Level 3 of scope statement corresponds to function body. Therefore, only level 4 and above
   scopes should be used to generate lexical block tags.

   Two labels are generated for each block. The start label will be inserted when the compiler
   handles the scope begin statement, while the second label will be added when the compiler
   handles the scope exit statement. The second label is saved in the linked list node too.

   Touched files:  
        gnu/c-decl.c  gnu/c-semantics.c 
          initialize counters when parsing or processing scope_stmt 
          initialize or reset some data structures
          call the corresponding function in wfe_dst.cxx 
       wfe_dst.cxx 
          save variable tags 
          build DW_TAG_lexical_block
          insert labels and attach variable tags 
*/

bool seen_scope_stmt = false;
int parser_scope_level = FUNCTION_SCOPE_LEVEL;

static MEM_POOL var_dst_mempool;
static bool var_dst_mempool_initialized = false;

/* this is used to hold variable tags for nested scopes */
struct kids_link { 
    DST_INFO_IDX var_dst; 
    struct kids_link * next; 
};

typedef struct kids_link DST_KIDS_LINK;

/* this is used to hold a block at each scope level */
struct level_link {
    DST_INFO_IDX level_dst;
    WN *         high_pc_label;
    DST_KIDS_LINK *kids;
    int count;
    bool processed;
...
typedef struct level_link DST_LEVEL_LINK;

/* this is the data structure for each level 
   active_dst and parent_dst are used to track the parent.
*/
typedef struct scope_dst_info_entry_t {
   DST_INFO_IDX active_dst;
   DST_INFO_IDX parent_dst;
   int count;
   DST_LEVEL_LINK * level;
} SCOPE_DST_INFO;

SCOPE_DST_INFO  scope_dst_stack[MAX_SCOPE_LEVEL];
int scope_dst_stack_index=FUNCTION_SCOPE_LEVEL;

static void WFE_append_to_scope (DST_INFO_IDX dst);
...
#ifdef TARG_NVISA
/* Obtain the source file and line information for the entity pointed to by 'node'
  (this entity may be a type or a declaration) and convert it to SRCPOS format.  */
  
static SRCPOS
Get_Tree_Srcpos (tree node)
{
  SRCPOS s;
  SRCPOS_clear(s);

  if (node && TREE_CODE (node) == IDENTIFIER_NODE) {
    node = lookup_name (node);
  }
  if (node && TYPE_P (node)) {
    node = TYPE_MAIN_VARIANT (node);
  }
  if (node && TYPE_P (node)) {
    node = TYPE_STUB_DECL (node);
  }
  if (node && DECL_P (node)) {
    UINT saved_current_dir = current_dir, saved_current_file = current_file;
    WFE_Set_Line_And_File (1, DECL_SOURCE_FILE (node));
    SRCPOS_filenum(s) = current_file;
    SRCPOS_linenum(s) = DECL_SOURCE_LINE (node);
    current_dir = saved_current_dir, current_file = saved_current_file;
  }
  return s;
}
#endif  /* TARG_NVISA */
...
//    int currentcontainer = -1 ;
	...
	tree ftype = TREE_TYPE(field);
        ...
        USRCPOS src;
#ifdef TARG_NVISA
        /* We want to know where the field was declared, not where it is being used.  */
        USRCPOS_srcpos(src) = Get_Tree_Srcpos(field);
#else        
...
{ 
	...
	//
        USRCPOS src;
#ifdef TARG_NVISA
        /* We want to know where the struct/union was declared, not where it is being used.  */
        USRCPOS_srcpos(src) = Get_Tree_Srcpos(type_tree);
#else        
        // For now, the source location appears bogus
        // (or at least odd) for files other than the base
        // file, so lets leave it out. Temporarily.
...
{ 
   ...
   if(DST_IS_NULL(dst_idx)) {
      ...
      USRCPOS src;
#ifdef TARG_NVISA
      /* We want to know where the enum type was declared, not where it is being used.  */
      USRCPOS_srcpos(src) = Get_Tree_Srcpos(type_tree);
#else        
      // For now, the source location appears bogus
      // (or at least odd) for files other than the base
      ...
      for( ; enum_entry; enum_entry = TREE_CHAIN(enum_entry) ) {
         USRCPOS src;
#ifdef TARG_NVISA
         /* We want to know where the enumerator (value) was declared, not
            where it is being used.  */
         USRCPOS_srcpos(src) = Get_Tree_Srcpos(TREE_PURPOSE (enum_entry));
#else        
         // For now, the source location appears bogus
         // (or at least odd) for files other than the base
         // file, so lets leave it out. Temporarily.
...
{ 
   ...
   if(DST_IS_NULL(dst_idx)) {
      ...
      USRCPOS src;
#ifdef TARG_NVISA
      /* We want to know where the array was declared, not where it is being used.  */
      USRCPOS_srcpos(src) = Get_Tree_Srcpos(type_tree);
#else        
      // For now, the source location appears bogus
      // (or at least odd) for files other than the base
        ...
        tree decl, ST* var_st)
{
  ...
  if (TREE_CODE(decl) == VAR_DECL && 
		 DECL_EXTERNAL(decl)
#ifdef TARG_NVISA
    /* for extern __shared__ variables, the debugger needs to know the type info,
       although such variable has no memory space allocated, and has no location info.
       Therefore, the DW_AT_location is not generated here. Bug# 494728 */
      &&
      !(ST_sclass(var_st) == SCLASS_COMMON
      && ST_export(var_st) == EXPORT_PREEMPTIBLE
      && ST_is_weak_symbol(var_st))
#endif
...
DST_Create_type(ST *typ_decl, tree decl)
{
    USRCPOS src;
#ifdef TARG_NVISA
    /* We want to know where the typedef was declared, not where it is being used.  */
    USRCPOS_srcpos(src) = Get_Tree_Srcpos(decl);
#else        
    // For now, the source location appears bogus
    // (or at least odd) for files other than the base
    // file, so lets leave it out. Temporarily.
...
DST_Create_Parmvar(ST *var_st, tree param)
{
    USRCPOS src;
#ifdef TARG_NVISA
    /* We want to know where the parameter was declared, not where it is being used.  */
    USRCPOS_srcpos(src) = Get_Tree_Srcpos(param);
#else        
    // For now, the source location appears bogus
    // (or at least odd) for files other than the base
    // file, so lets leave it out. Temporarily.
...
DST_Create_var(ST *var_st, tree decl)
{
    USRCPOS src;
#ifdef TARG_NVISA
    /* We want to know where the variable was declared, not where it is being used.  */
    USRCPOS_srcpos(src) = Get_Tree_Srcpos(decl);
#else        
    // For now, the source location appears bogus
    // (or at least odd) for files other than the base
...
#ifdef TARG_NVISA
    if (strncmp(ST_name(var_st), "__T", 3) == 0) 
      return DST_INVALID_IDX;
#endif
...
#endif
#ifdef TARG_NVISA
    char *origname = ST_name(var_st);
    if (strncmp(origname, "__cuda_local_var_", 17) == 0) {
      // cudafe mangled the name to avoid conflict, retrieve original name
      // must be of form __cuda_local_var_<num>_<num>_<origname>
      origname += 17;
      while (isdigit(*origname))
        ++origname;
      ++origname; // _
      while (isdigit(*origname))
        ++origname;
      FmtAssert(*origname == '_', ("unexpected name mangling"));
      ++origname; // _
      DevWarn("demangle %s to %s", ST_name(var_st), origname);
    }
#endif // NVISA
    dst = DST_mk_variable(
        src,                    // srcpos
...
#ifdef TARG_NVISA
    scope_dst_stack[FUNCTION_SCOPE_LEVEL].parent_dst = comp_unit_idx;
    scope_dst_stack[FUNCTION_SCOPE_LEVEL].active_dst = dst;
#endif
...
*/
...

   {
      comp_unit_idx = DST_mk_compile_unit((char*)dump_base_name,
					  ...
					  comp_info, 
#ifdef TARG_NVISA
					  DW_LANG_C_plus_plus, // input is C++
#else
					  DW_LANG_C89,
#endif
...
/* this function creates DW_TAG_lexical_block for a nested scope *
   it also tracks parent - child relationship.
*/

void WFE_Create_Lexical_Block()
{
   /* level 2 is subroutine level 
      the parent is comp_unit_idx

      we copy the information from above level first. We will not change it
      unless we generate DW_TAG_lexical_block at this level 
   */
   if ( scope_dst_stack_index > FUNCTION_SCOPE_LEVEL) {
       scope_dst_stack[scope_dst_stack_index].parent_dst = scope_dst_stack[scope_dst_stack_index-1].parent_dst;
       scope_dst_stack[scope_dst_stack_index].active_dst = scope_dst_stack[scope_dst_stack_index-1].active_dst;
   }

   /* may need to generate DW_TAG_lexical_block at this level */
   if ( scope_dst_stack_index >= NESTED_BLOCK_START_LEVEL && scope_dst_stack[scope_dst_stack_index].level != NULL )
     {
        LABEL_IDX low_pc_label_idx;
        LABEL_IDX high_pc_label_idx;
        WN * low_pc_wn;
        WN * high_pc_wn;
        DST_INFO_IDX block_idx;
        DST_INFO_IDX parent_idx;
        ST_IDX  low, high;
        DST_KIDS_LINK * kids;
        DST_LEVEL_LINK * level; 

        level = scope_dst_stack[scope_dst_stack_index].level;
        while (level) {
           if (level->count == scope_dst_stack[scope_dst_stack_index].count)
              break;
           level = level->next;
        }

        if (!level || !level->kids) return; /* nno kids, no need to do the work */

        kids = level->kids;

        /* generate two labels*/
        New_LABEL (CURRENT_SYMTAB, low_pc_label_idx);
        New_LABEL (CURRENT_SYMTAB, high_pc_label_idx);
        low_pc_wn = WN_CreateLabel ( low_pc_label_idx, 0, NULL);
        high_pc_wn = WN_CreateLabel (high_pc_label_idx, 0, NULL);

        WFE_Stmt_Append (low_pc_wn, Get_Srcpos()); /* append the first label */

        low = make_ST_IDX(low_pc_label_idx, CURRENT_SYMTAB);
        high = make_ST_IDX(high_pc_label_idx, CURRENT_SYMTAB);

        block_idx = DST_mk_lexical_block(NULL, low, high, DST_INVALID_IDX);

        level->level_dst = block_idx;
        level->high_pc_label = high_pc_wn; /* save the second label */


        parent_idx = scope_dst_stack[scope_dst_stack_index-1].active_dst;

        if (parent_idx.byte_idx < 0 && parent_idx.block_idx < 0)
           parent_idx = comp_unit_idx; 

        /* attach the DW_TAG_lexical_block to its parent dst */         
        DST_append_child ( parent_idx, block_idx);

        /* update the parent-child relationship, current dst becomes the active dst
           for its children */

        scope_dst_stack[scope_dst_stack_index].parent_dst = parent_idx;
        scope_dst_stack[scope_dst_stack_index].active_dst = block_idx;   

        /* attach variable tags to the block dst */  
        while (kids) {
            DST_append_child (block_idx, kids->var_dst);
            kids = kids->next;
        } 
     }
}

/* find the block node at specific level */

static  DST_LEVEL_LINK * WFE_Find_Level_Link(int level, int count)
{
  DST_LEVEL_LINK *pl;

  pl = scope_dst_stack[level].level;

  if (pl) {
     while (pl) {
        if (pl->count == count) break;
        pl=pl->next;
     }
  }
...
/* handles the scope end statement */

void WFE_Finish_Lexical_Block()
{
   DST_LEVEL_LINK *pl;

   if (scope_dst_stack_index >= NESTED_BLOCK_START_LEVEL  &&  scope_dst_stack[scope_dst_stack_index].level != NULL) {

     pl =  WFE_Find_Level_Link(scope_dst_stack_index, scope_dst_stack[scope_dst_stack_index].count);
     if (pl)
     {
         /* insert the high_pc label */
         WFE_Stmt_Append(pl->high_pc_label, Get_Srcpos());
     }
   }

   if ( scope_dst_stack_index > FUNCTION_SCOPE_LEVEL) {
   /* update the parent-child relationship 
      current block dst should not be the active one anymore 
      go back one level 
   */
       scope_dst_stack[scope_dst_stack_index].parent_dst = scope_dst_stack[scope_dst_stack_index-1].parent_dst;
       scope_dst_stack[scope_dst_stack_index].active_dst = scope_dst_stack[scope_dst_stack_index-1].active_dst;
...
int WFE_Get_Scope_Level_Count(int level)
{
   return scope_dst_stack[level].count;
}

void WFE_Increment_Scope_Level_Count(int level)
{
   FmtAssert(level < MAX_SCOPE_LEVEL,  
             ("WFE_Increment_Scope_Level_Count: DST stack exhausted"));
   scope_dst_stack[level].count++;
}

void WFE_Decrement_Scope_Level_Count(int level)
{
   scope_dst_stack[level].count--;
}

void WFE_Reset_Scope_Level_Count(int level)
{
   scope_dst_stack[level].count=0;
}

void  WFE_Reset_All_Scope_Level_Count()
{
   int level;

   for (level =0; level<MAX_SCOPE_LEVEL; level++)
       scope_dst_stack[level].count=0;
}


void WFE_Initialize_Scope_Stack()
{
  int i; 
  for (i=0; i<MAX_SCOPE_LEVEL; i++) {
      scope_dst_stack[i].parent_dst = DST_INVALID_IDX;
      scope_dst_stack[i].active_dst = DST_INVALID_IDX;
      scope_dst_stack[i].level = NULL;
      scope_dst_stack[i].count = 0;
  }

  scope_dst_stack_index=FUNCTION_SCOPE_LEVEL;

  if (!var_dst_mempool_initialized)
  {
     MEM_POOL_Initialize(&var_dst_mempool,"var_dst_mempool",FALSE);
     var_dst_mempool_initialized = true;
  }
  MEM_POOL_Push(&var_dst_mempool);
}

/* create a node for a block at a specific level */
static  DST_LEVEL_LINK * WFE_Create_Level_Link(int level, int count)
{
  DST_LEVEL_LINK * pl;
  ...
  {
    DST_LEVEL_LINK * temp;
    temp =  scope_dst_stack[level].level;
    pl = (DST_LEVEL_LINK *) MEM_POOL_Alloc (&var_dst_mempool, (sizeof(DST_LEVEL_LINK)));
    pl->kids = NULL;
    pl->count = count; 
    pl->next = NULL;
    pl->processed = false;
    ...
    if (!temp)
      scope_dst_stack[level].level = pl;
    else
    {
...
/* append variable tag to the right block node */
...
static void WFE_append_to_scope (DST_INFO_IDX dst)
{
   ...
   pt = (DST_KIDS_LINK *) MEM_POOL_Alloc( &var_dst_mempool,  (sizeof(DST_KIDS_LINK)));
   pt->next = NULL;
   pt->var_dst = dst;

   pl = WFE_Create_Level_Link(parser_scope_level, scope_dst_stack[parser_scope_level].count);
