...
static vector <pair<tree, tree> > alias_vector;
...
inline void
Force_Inline (ST *func_st, tree fndecl, INT errcode)
{
  // only give warning if user requested noinline
  if (DECL_NOINLINE_ATTRIB(fndecl))
    ErrMsgSrcpos (errcode, Get_Srcpos(), ST_name(func_st));
  Clear_PU_no_inline(ST_pu(func_st));
  Set_PU_must_inline(ST_pu(func_st));
}

static void
Check_If_Must_Inline (ST *func_st, tree fndecl)
{
        TY_IDX func_ty = ST_pu_type(func_st);
        if (!TY_has_prototype (func_ty)) {
          // must have prototype
          Force_Inline (func_st, fndecl, EC_Inline_Prototype);
          return;
        }

        // we inline all the functions for multicore right now
        if (flag_multicore) {
          Force_Inline (func_st, fndecl, EC_Inline_Multicore);
          return;
        }

        // texture and surface parameters must be inlined since need name
        for (tree pdecl = DECL_ARGUMENTS (fndecl); pdecl; pdecl = TREE_CHAIN (pdecl) )
        {
          if (DECL_TEXTURE(pdecl) || DECL_SURFACE(pdecl)) {
            Force_Inline (func_st, fndecl, EC_Inline_Texture);
            return;
          }
        }
        ...
        if (RETURN_INFO_return_via_first_arg(return_info)) {
          ...
          Force_Inline (func_st, fndecl, EC_Inline_Retval_Limit);
          return;
        }
        ...
        if (tl != (TYLIST_IDX) NULL) {
          ...
          for (; TYLIST_ty(tl); tl = TYLIST_next(tl)) {
              ...
              && TY_kind(ty) == KIND_POINTER) 
            {
              ...
              Force_Inline (func_st, fndecl, EC_Inline_Ptr);
              return;
            }
            ...
            while (PLOC_is_nonempty(ploc)) {
              ...
              {
                // doesn't fit in registers
                Force_Inline (func_st, fndecl, EC_Inline_Param_Limit);
                return;
              }
...
#ifdef TARG_NVISA
    if (!ST_in_global_mem(func_st)) { // ignore entry functions
      if (DECL_NOINLINE_ATTRIB (fndecl)) {
        Check_If_Must_Inline (func_st, fndecl);
        if (PU_must_inline(ST_pu(func_st))) {
          Set_ST_export (func_st, EXPORT_LOCAL);
        }
      }
        ...
        if (PU_must_inline(ST_pu(func_st))) {
          ...
          // not a must-inline, but would like to inline for performance
          Set_PU_is_inline_function (Pu_Table[ST_pu (func_st)]);
          if (ST_export(func_st) == EXPORT_PREEMPTIBLE) {
            // declared preemptible, but until we get separate compilation
            // we treat it as inline function.
...
#if 0
    if (Get_Trace(TP_MISC, 0x100)) {
      // dump info about # and size of parameters
      INT size = 0;
      fprintf(stderr, "%s has %d parameters:", ST_name(func_st), num_args);
      TYLIST_IDX tl = TY_parms(ST_pu_type(func_st));
      if (tl != (TYLIST_IDX) NULL) {
        for (; TYLIST_ty(tl); tl = TYLIST_next(tl)) {
          TY_IDX ty = TYLIST_ty(tl);
          fprintf(stderr, " %s", MTYPE_name(TY_mtype(ty))); 
          if (TY_mtype(ty) == MTYPE_M) {
            fprintf(stderr, "%lld", TY_size(ty)); 
            size += TY_size(ty);
          }
          else {
            size += MTYPE_byte_size(TY_mtype(ty));
          }
        }
      }
      fprintf(stderr, ", total %d", size);
      fprintf(stderr, ", return %d", TY_size(TY_ret_type(ST_pu_type(func_st))));
      fprintf(stderr, "\n");
    }
#endif
}
...
typedef __gnu_cxx::hash_map < ST_IDX, const char* > ST_STRING_MAP;
ST_STRING_MAP st_to_string_map;
// Return a symbol of char array type based on the length of init.
static ST *
WFE_Get_Char_Array_Sym (tree init)
{
  Is_True(TREE_CODE(init) == STRING_CST, ("Tree type must be string_cst"));

  const char *literal = TREE_STRING_POINTER(init);
  const INT literal_len = TREE_STRING_LENGTH(init);

  Is_True(literal_len > 0, ("Unexpected 0-length string literal"));
  // get the array type
  TY_IDX array_ty = Get_TY(TREE_TYPE(init));
  ST *tmp_sym = Gen_Read_Only_Symbol(array_ty, "_tmp_array_");
  Set_ST_is_initialized(tmp_sym);
  Set_ST_in_global_mem(tmp_sym);
  // wide characters not handled
  FmtAssert(TY_size(TY_etype(array_ty)) == 1, ("Char length > 1 not handled"));

  // Maintain a map from the symbol to its initialization. We are currently
  // in the middle of an initialization, so don't initialize this sym now.
  // These syms are initialized in Initialize_Pending_Symbols.
  st_to_string_map[ST_st_idx(tmp_sym)] = literal;

  return tmp_sym;
}

// Initialize symbols generated by WFE_Get_Char_Array_Sym.
static void
Initialize_Pending_Symbols(void)
{
  for (ST_STRING_MAP::const_iterator iter = st_to_string_map.begin();
       iter != st_to_string_map.end();
       ++iter) {
    const char* literal = iter->second;
    const INT literal_len = strlen(literal)+1;
    INITO_IDX ino = New_INITO(&St_Table[iter->first]);

    const char *p = literal;
    INITV_IDX blk = New_INITV();
    INITV_IDX inv = New_INITV();
    INITV_Init_Integer (inv, MTYPE_I1, (INT64)*p);
    INITV_Init_Block (blk, inv);
    Set_INITO_val(ino, blk);
    INITV_IDX last_inv = inv;

    for (INT i=1; i<literal_len; ++i) {
      inv = New_INITV();
      INITV_Init_Integer (inv, MTYPE_I1, (INT64)*(p+i));
      Set_INITV_next(last_inv, inv);
      last_inv = inv;
    }
  }
  st_to_string_map.clear();
}
#endif // TARG_NVISA
...
WFE_Add_Aggregate_Init_Address (tree init)
{
  ...
  case STRING_CST:
	...
	// generate code
	//   char *p = temp
	// Initialize_Pending_Symbols() adds the initialization as an
	// array initialization:
	//   char temp[length-of-string] = { char-array terminated with null }
	// The initialization for temp is generated after that of p. Cgemit
	// fixes the order so that temp's initialization is emitted before p.
	//
	// Get a symbol for temp, which is of type array of length string
	// length.
	ST *const_st = WFE_Get_Char_Array_Sym(init);
#else
...
#ifdef TARG_NVISA
        Set_ST_initv_in_other_st (const_st);
#endif // TARG_NVISA
	}
    	break;
...
extern ST *
WFE_Generate_Temp_For_Initialized_Aggregate (tree init, const char * name)
{
