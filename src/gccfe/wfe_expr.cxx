  ...
  if (code == PARM_DECL || code == VAR_DECL) {
    ...
    if (WN_has_side_effects(addr_wn) &&
	 ...
	 assign_code == POSTDECREMENT_EXPR)) {
      ST       *preg_st;
      PREG_NUM  preg;
      TY_IDX    address_ty_idx = Make_Pointer_Type(elem_ty_idx);
      preg_st = MTYPE_To_PREG(Pointer_Mtype);
      preg    = Create_Preg (Pointer_Mtype, NULL);
      wn      = WN_Stid (Pointer_Mtype, preg, preg_st, address_ty_idx, addr_wn);
      WFE_Set_ST_Addr_Saved (addr_wn);
    ...
    if (!WFE_Keep_Zero_Length_Structs &&
      ...
      wn = WN_CreateIstore(OPR_ISTORE, MTYPE_V, desc, component_offset, 
			   Make_Pointer_Type(elem_ty_idx), rhs_wn,
			   addr_wn, field_id);
      WFE_Stmt_Append(wn, Get_Srcpos());
#ifdef TARG_SL
...
#ifdef TARG_NVISA
// as with sync, not a pure call, so make intrinsic_call
static void
emit_builtin_threadfence (INTRINSIC intrinsic, tree exp, INT32 k)
{
  WN *wn;
  tree list = TREE_OPERAND (exp, 1);
  emit_barrier (TRUE, list, k);
  wn = WN_Create_Intrinsic (OPC_VINTRINSIC_CALL, intrinsic, 0, NULL);
  WFE_Stmt_Append (wn, Get_Srcpos());
  emit_barrier (FALSE, list, k);
} /* emit_building_threadfence */
...
static WN*
emit_builtin_clock (tree exp)
{
  TYPE_ID ret_mtype = TY_mtype(Get_TY(TREE_TYPE(exp)));
  ...
  if (WN_rtype(wn) != ret_mtype)
    wn = WN_Cvt (WN_rtype(wn), ret_mtype, wn);
...
static WN*
emit_builtin_synchronous_start (tree exp, INTRINSIC iopc)
{
  TYPE_ID Return_Val_Ty = MTYPE_V;
...
static WN*
emit_builtin_synchronous_end (tree exp, INTRINSIC iopc)
{
  TYPE_ID Return_Val_Ty = MTYPE_V;
  
  OPCODE     opc = OPCODE_make_op (OPR_INTRINSIC_CALL, Return_Val_Ty, MTYPE_V);
  
  WN *wn = WN_Create_Intrinsic (opc, iopc, 0, NULL);
  WFE_Stmt_Append (wn, Get_Srcpos());
  return wn;
}
...
static void
emit_builtin_pmevent (tree exp)
{
  tree list = TREE_OPERAND (exp, 1);
  TY_IDX arg_ty_idx = Get_TY (TREE_TYPE (TREE_VALUE (list)));
  TYPE_ID arg_mtype = TY_mtype (arg_ty_idx);
  WN *wn, *arg_wn = WFE_Expand_Expr (TREE_VALUE (list));
  ...
  if (TREE_CODE (TREE_VALUE (list)) != INTEGER_CST)
    ErrMsgSrcpos(EC_Intcon_Expected, Get_Srcpos(), "__prof_trigger()");
} /* emit_builtin_pmevent */

static WN* 
emit_builtin_pm (const INTRINSIC iopc)
{
  WN *wn = WN_Create_Intrinsic (OPC_U4INTRINSIC_CALL, iopc, 0, NULL);
  WFE_Stmt_Append (wn, Get_Srcpos());

  ST       *preg_st = MTYPE_To_PREG(MTYPE_U4);
  TY_IDX    preg_ty_idx = Be_Type_Tbl(MTYPE_U4);
  PREG_NUM  preg = Create_Preg (MTYPE_U4, NULL);

  wn = WN_Ldid (MTYPE_U4, -1, Return_Val_Preg, preg_ty_idx);
  wn = WN_Stid (MTYPE_U4, preg, preg_st, preg_ty_idx, wn),
  WFE_Stmt_Append (wn, Get_Srcpos());

  wn = WN_Ldid (MTYPE_U4, preg, preg_st, preg_ty_idx);
  return wn;
} /* emit_builtin_pm */

static WN *
emit_builtin_bitcount (tree exp, INTRINSIC iopc, TYPE_ID formal_type)
{
  tree arg = TREE_VALUE (TREE_OPERAND (exp, 1));
  TY_IDX arg_ty_idx = Get_TY (TREE_TYPE (arg));
  WN *arg_wn  = WN_CreateParm (formal_type, WFE_Expand_Expr (arg), arg_ty_idx, WN_PARM_BY_VALUE);
...
static WN*
emit_builtin_texfetch (tree exp, const char *name, TYPE_ID rtype, TYPE_ID ctype)
{
  ...
  // store the coord into a temp to avoid issues with calls and commas
  TY_IDX temp_ty_idx = Get_TY(TREE_TYPE(TREE_VALUE(list)));
  ST *temp = Gen_Temp_Symbol(temp_ty_idx, "coord_arg");
  WN *wnst = WN_Stid(MTYPE_M, 0, temp, temp_ty_idx, coord_arg);
  WFE_Stmt_Append(wnst, Get_Srcpos());
  coord_arg = WN_Ldid(MTYPE_M, 0, temp, temp_ty_idx);
...
// pass in ptx inst name, element type of result and coord
static WN*
emit_builtin_surfread (tree exp, INT cdim, INT ddim, TYPE_ID dtype, const char *name)
{
  tree list = TREE_OPERAND (exp, 1);
  WN *surface_arg = WFE_Expand_Expr (TREE_VALUE (list));
  list       = TREE_CHAIN (list);
  WN *coord_arg1 = WFE_Expand_Expr (TREE_VALUE (list));
  WN *coord_arg2;
  if (cdim == 2) {
    list       = TREE_CHAIN (list);
    coord_arg2 = WFE_Expand_Expr (TREE_VALUE (list));
  }
  list       = TREE_CHAIN (list);
  WN *clamp_arg = WFE_Expand_Expr (TREE_VALUE (list));
...
// pass in ptx inst name, element type of result and coord
static void
emit_builtin_surfwrite (tree exp, INT cdim, INT adim, TYPE_ID atype, const char *name)
{
  tree list = TREE_OPERAND (exp, 1);
  WN *addr_arg = WFE_Expand_Expr (TREE_VALUE(list));
  if (WN_operator (addr_arg) == OPR_COMMA) {
    addr_arg = WN_kid1 (addr_arg);
  }
  list       = TREE_CHAIN (list);
  WN *surface_arg = WFE_Expand_Expr (TREE_VALUE (list));
  list       = TREE_CHAIN (list);
...
static WN*
emit_builtin_bar_red (tree exp, INTRINSIC iopc)
{
...
#ifdef TARG_NVISA
// Given an offset and a type, return offset if it is already aligned to
// the type size. Else return offset after aligning it to the size.
static INT
Get_next_aligned_ofst (INT ofst, TYPE_ID type)
{
  INT size = MTYPE_byte_size(type);

  if (ofst % size == 0)
    return ofst;

  return ofst + size - ofst % size;
}

static ST * valist_args = NULL; // this sym is always of type char*
// The TY of valist_array is finalized after seeing all calls to printf
// in a function, as a result, the TY (specifically the TY size) should
// not be used until finalized.
static ST * valist_array = NULL; // for holding the va_list
static void
Clear_local_sym_from_cache (void)
{
  valist_args = NULL;
  valist_array = NULL;
}

// For each variable argument to a printf call, store the value of the
// argument in a char buffer, aligned to the size of the argument.
// Modify the call to invoke vprintf and pass base of the array as
// the 2nd argument:
//   char buf[#bytes required to hold args with padding];
//   char *base;
//   int ofst = 0;
//   *(<type-of-vararg1>*)(&buf[ofst]) = vararg1;
//   ofst += size of vararg1
//   ofst = ofst aligned-to-size-of-vararg2
//   *(<type-of-vararg2>*)(&buf[ofst]) = vararg2;
//   /* vararg1 and vararg2 could be expression */
//   base = buf;
//   /*** Only emit the following for zero variable arguments. ***/
//   vprintf("<format string>", base);
//
static WN *
WFE_Change_Printf_To_Vprintf (WN * call_wn)
{
  static PU_IDX current_pu_idx = PU_IDX_ZERO;
  // If we are processing a different PU, then clear cache of local symbols.
  if (current_pu_idx == PU_IDX_ZERO)
    current_pu_idx = ST_pu(Get_Current_PU_ST());
  else if (current_pu_idx != ST_pu(Get_Current_PU_ST()))
  {
    current_pu_idx = ST_pu(Get_Current_PU_ST());
    Clear_local_sym_from_cache();
  }
  Is_True(WN_operator(call_wn) == OPR_CALL, ("WN must be call"));
  Is_True(!strcmp(ST_name(WN_st(call_wn)), "printf"), ("Call must be printf"));
  // get the function for vprintf
  ST * new_call_st = Get_ST(built_in_decls[BUILT_IN_VPRINTF]);
  FmtAssert(new_call_st, ("Cannot find symbol for vprintf"));

  WN * new_wn = WN_Create (OPR_CALL, WN_rtype(call_wn), MTYPE_V, 2);
  // call flags
  WN_Set_Call_Default_Flags(new_wn);
  const TYPE_ID elem_type = MTYPE_I1;
  // array element size
  INT esize = MTYPE_byte_size(elem_type);

  // number of array elements, here it is just based on the # of args, will
  // fill in # of required bytes later
  INT array_size = WN_kid_count(call_wn) - 1;
  TY_IDX array_ty = valist_array ? ST_type(valist_array) : TY_IDX_ZERO;
  if (array_size > 0 &&
      valist_array == NULL /* no symbol allocated yet for va_list */)
  {
    // create an array
    ARB_HANDLE arb = New_ARB();
    ARB_Init(arb, 0, array_size, esize);
    Set_ARB_flags(arb, ARB_flags(arb) | ARB_FIRST_DIMEN | ARB_LAST_DIMEN);
    STR_IDX str = Save_Str("__valist_array__");

    // array type
    TY_Init(New_TY(array_ty), array_size * esize, KIND_ARRAY, MTYPE_M, str);
    Set_TY_arb(array_ty, arb);
    Set_TY_etype(array_ty, MTYPE_TO_TY_array[elem_type]);
    // 8-byte aligned
    Set_TY_align(array_ty, MTYPE_byte_size(MTYPE_I8));
    // array symbol
    valist_array = Gen_Temp_Symbol(array_ty, "_valist_array_");
  }

  TY_IDX ty_idx = Make_Pointer_Type(MTYPE_TO_TY_array[elem_type]);
  if (!valist_args)
    valist_args = Gen_Temp_Symbol(ty_idx, "_valist_args_");

  // prepare the new call statement
  WN_st_idx(new_wn) = ST_st_idx(new_call_st);
  WN_kid0(new_wn) = WN_COPY_Tree(WN_kid0(call_wn));

  WN * ldid = WN_Ldid(TY_mtype(ty_idx), 0, valist_args, ty_idx);
  WN_kid1(new_wn) = WN_CreateParm(WN_rtype(ldid), ldid, ty_idx,
                                  WN_PARM_BY_VALUE);

  INT myofst = 0;
  for (INT i=1; i<WN_kid_count(call_wn); i++)
  {
    // obtain the value of the argument
    WN * val = WN_COPY_Tree(WN_kid(WN_kid(call_wn, i), 0));
    // type of argument
    TY_IDX valtype = MTYPE_TO_TY_array[WN_rtype(val)];
    myofst = Get_next_aligned_ofst(myofst, WN_rtype(val));
    // create an expression to store to
    WN * addr = WN_Lda(Pointer_Mtype, myofst, valist_array);

    // array store
    WN * istore = WN_Istore(WN_rtype(val), 0, Make_Pointer_Type(valtype),
                            addr, val);
    WFE_Set_ST_Addr_Saved(val);
    WFE_Stmt_Append(istore, Get_Srcpos());
    myofst += MTYPE_byte_size(WN_rtype(val));
  }

  // Update the array size with the real computed size. Till now array_size
  // was a dummy value, which only implied if size is greater than 0 or not.
  array_size = myofst;

  if (array_size > 0)
  {
    // If this va_list is larger than what array_ty can hold, or if this
    // is the first time we are setting up the ty, then update the array
    // size.
    if (array_size * esize > TY_size(array_ty))
    {
      // update sizes
      Set_TY_size(array_ty, array_size * esize);
      Set_ARB_ubnd_val(TY_arb(array_ty), array_size);
    }

    WN * stid = WN_Stid(TY_mtype(ty_idx), 0, valist_args, ty_idx,
                        WN_Lda(Pointer_Mtype, 0, valist_array));
    WFE_Stmt_Append(stid, Get_Srcpos());
    Set_ST_addr_saved(valist_array);
    Set_ST_addr_passed(valist_array);
  }
  // delete old call
  WN_DELETE_Tree(call_wn);

  WN_Set_Linenum(new_wn, Get_Srcpos());
  return new_wn;
}
#endif // TARG_NVISA
                 ...
                 bool expect_boolean)
{
  ...
  switch (code)
		...
		wn = WN_Intconst (Pointer_Mtype, 0);
	        ...
	        wn = WN_Lda (Pointer_Mtype, ST_ofst(st), st);
                ...
                if (Target_ISA >= TARGET_ISA_compute_20) {
                  // if take address of constant space, must put in global
                  if (ST_in_constant_mem(st)) {
                    DevWarn("change constant to global mem");
	...
	if (desc_ty_idx == 0)
	  desc_ty_idx = hi_ty_idx;
#ifdef TARG_NVISA
	// If the component fld is volatile, make the composite type volatile.
	else if (TY_is_volatile(desc_ty_idx))
	  Set_TY_is_volatile(hi_ty_idx); 
#endif
	...
	TYPE_ID ldid_rtype = rtype;
#ifdef TARG_NVISA
	// For certain expressions of the form <truncation>(op1 op op2),
	// GCC can distribute the truncation past the operation into the
	// operands. This is done in convert_to_integer(), called from
	// convert().
	// If op1 or op2 is a bit-field, nvisa will have problem if the
	// truncation is done on the operand itself, because that will
	// cause gfec to use U4BSLDID (instead of U8BSLDID) for example.
	// But this gets lowered to U8U8LDBITS, causing mismatch between
	// operation size and operand size. Undo the truncation on bit-fields.
	if (is_bit_field && field_id != 0) {
	  TY_IDX fld_ty = Get_Field_Type(hi_ty_idx, field_id);
	  if (fld_ty != hi_ty_idx)
	    ldid_rtype = TY_mtype(fld_ty);
	}
#endif // TARG_NVISA
	wn = WN_CreateLdid (OPR_LDID, ldid_rtype,
...
// mtyp == MTYPE_V => wn == 0
	...
	Is_True(! is_bit_field || field_id <= MAX_FIELD_ID,
	  ...
	  if (WN_operator (wn0) == OPR_LDA          &&
		...
		break;
          ...
          TY_IDX item_ty, ptr_targ_ty;
#ifndef KEY
          item_ty = field_id != 0 ? hi_ty_idx : ty_idx;
#else
          item_ty = field_id != 0 ? hi_ty_idx : iload_ty_idx;
#endif
          ptr_targ_ty = hi_ty_idx;

          if (TREE_THIS_VOLATILE(exp)) {
            Set_TY_is_volatile(item_ty);
            Set_TY_is_volatile(ptr_targ_ty);
          }
          if (TREE_READONLY(exp)) {
            Set_TY_is_const(item_ty);
            Set_TY_is_const(ptr_targ_ty);
          }
    ...
    case TRUTH_ORIF_EXPR:
      {
        ...
        wn1 = WFE_Expand_Expr_With_Sequence_Point (TREE_OPERAND (exp, 1),
						   Boolean_type);
#ifdef TARG_NVISA
        // To have mtypes match, wn0 and wn1 must be Boolean type
        {
          TYPE_ID exp0_mtype = 
            Widen_Mtype(TY_mtype(Get_TY(TREE_TYPE(TREE_OPERAND(exp,0)))));
          TYPE_ID exp1_mtype = 
            Widen_Mtype(TY_mtype(Get_TY(TREE_TYPE(TREE_OPERAND(exp,1)))));
          if (Boolean_type != MTYPE_B && exp0_mtype != Boolean_type)
	    wn0 = WN_Cvt (exp0_mtype, Boolean_type, wn0);
          if (Boolean_type != MTYPE_B && exp1_mtype != Boolean_type)
	    wn1 = WN_Cvt (exp1_mtype, Boolean_type, wn1);
        }
#endif
	...
	if (desc_ty_idx == 0)
	  desc_ty_idx = hi_ty_idx;
#ifdef TARG_NVISA
	// If the component fld is volatile, the iload should be through
	// a volatile type.
	else if (TY_is_volatile(desc_ty_idx)) {
	  // composite type
	  Set_TY_is_volatile(hi_ty_idx);
	  // composite type, we will create a pointer to this
	  Set_TY_is_volatile(elem_ty_idx);
	}
#endif
	...
	for (list = TREE_OPERAND (exp, 1); list; list = TREE_CHAIN (list)) {
	  if (TREE_CODE(TREE_VALUE(list)) == ERROR_MARK)
        ...
        //   ret_mtype = MTYPE_V;
#ifdef TARG_NVISA
        enum built_in_function built_in_code = END_BUILTINS;
#endif
        st = NULL;
        if (code0 == ADDR_EXPR                  &&
...
#ifdef TARG_NVISA
            built_in_code = DECL_FUNCTION_CODE (func);
#endif
...
//		    intrinsic_op = TRUE;
...
#else
	      case BUILT_IN_FSQRT:	// older gcc name
#endif
#endif
                arg_wn = WFE_Expand_Expr (TREE_VALUE (TREE_OPERAND (exp, 1)));
		...
		break;
              ...
              case BUILT_IN_F4ADD_TRUNC:
                ...
                intrinsic_op = TRUE;
                break;
              case BUILT_IN_F4ADD_FLOOR:
                iopc = INTRN_F4ADD_FLOOR;
                intrinsic_op = TRUE;
                break;
              case BUILT_IN_F4ADD_CEIL:
              ...
              case BUILT_IN_F4MUL_TRUNC:
                ...
                intrinsic_op = TRUE;
                break;
              case BUILT_IN_F4MUL_FLOOR:
                iopc = INTRN_F4MUL_FLOOR;
                intrinsic_op = TRUE;
                break;
              case BUILT_IN_F4MUL_CEIL:
              ...
              case BUILT_IN_F8MUL_CEIL:
                ...
                intrinsic_op = TRUE;
                break;
              case BUILT_IN_FDIV_ROUND:
                iopc = INTRN_F4DIV_ROUND;
                intrinsic_op = TRUE;
                break;
              case BUILT_IN_FDIV_TRUNC:
                iopc = INTRN_F4DIV_TRUNC;
                intrinsic_op = TRUE;
                break;
              case BUILT_IN_FDIV_FLOOR:
                iopc = INTRN_F4DIV_FLOOR;
                intrinsic_op = TRUE;
                break;
              case BUILT_IN_FDIV_CEIL:
                iopc = INTRN_F4DIV_CEIL;
                intrinsic_op = TRUE;
                break;
              case BUILT_IN_FRCP_ROUND:
                iopc = INTRN_F4RCP_ROUND;
                intrinsic_op = TRUE;
                break;
              case BUILT_IN_FRCP_TRUNC:
                iopc = INTRN_F4RCP_TRUNC;
                intrinsic_op = TRUE;
                break;
              case BUILT_IN_FRCP_FLOOR:
                iopc = INTRN_F4RCP_FLOOR;
                intrinsic_op = TRUE;
                break;
              case BUILT_IN_FRCP_CEIL:
                iopc = INTRN_F4RCP_CEIL;
                intrinsic_op = TRUE;
                break;
              case BUILT_IN_FSQRT_ROUND:
                iopc = INTRN_F4SQRT_ROUND;
                intrinsic_op = TRUE;
                break;
              case BUILT_IN_FSQRT_TRUNC:
                iopc = INTRN_F4SQRT_TRUNC;
                intrinsic_op = TRUE;
                break;
              case BUILT_IN_FSQRT_FLOOR:
                iopc = INTRN_F4SQRT_FLOOR;
                intrinsic_op = TRUE;
                break;
              case BUILT_IN_FSQRT_CEIL:
                iopc = INTRN_F4SQRT_CEIL;
                intrinsic_op = TRUE;
                break;
              case BUILT_IN_DDIV_ROUND:
                iopc = INTRN_F8DIV_ROUND;
                intrinsic_op = TRUE;
                break;
              case BUILT_IN_DDIV_TRUNC:
                iopc = INTRN_F8DIV_TRUNC;
                intrinsic_op = TRUE;
                break;
              case BUILT_IN_DDIV_FLOOR:
                iopc = INTRN_F8DIV_FLOOR;
                intrinsic_op = TRUE;
                break;
              case BUILT_IN_DDIV_CEIL:
                iopc = INTRN_F8DIV_CEIL;
                intrinsic_op = TRUE;
                break;
              case BUILT_IN_DRCP_ROUND:
                iopc = INTRN_F8RCP_ROUND;
                intrinsic_op = TRUE;
                break;
              case BUILT_IN_DRCP_TRUNC:
                iopc = INTRN_F8RCP_TRUNC;
                intrinsic_op = TRUE;
                break;
              case BUILT_IN_DRCP_FLOOR:
                iopc = INTRN_F8RCP_FLOOR;
                intrinsic_op = TRUE;
                break;
              case BUILT_IN_DRCP_CEIL:
                iopc = INTRN_F8RCP_CEIL;
                intrinsic_op = TRUE;
                break;
              case BUILT_IN_DSQRT_ROUND:
                iopc = INTRN_F8SQRT_ROUND;
                intrinsic_op = TRUE;
                break;
              case BUILT_IN_DSQRT_TRUNC:
                iopc = INTRN_F8SQRT_TRUNC;
                intrinsic_op = TRUE;
                break;
              case BUILT_IN_DSQRT_FLOOR:
                iopc = INTRN_F8SQRT_FLOOR;
                intrinsic_op = TRUE;
                break;
              case BUILT_IN_DSQRT_CEIL:
              ...
              case BUILT_IN_FMAF:
                ...
                intrinsic_op = TRUE;
                break;
              case BUILT_IN_FDIV_APPROX:
                iopc = INTRN_F4DIV_APPROX;
                intrinsic_op = TRUE;
                break;
              case BUILT_IN_FMAF_ROUND:
                iopc = INTRN_F4MA_ROUND;
                intrinsic_op = TRUE;
                break;
              case BUILT_IN_FMAF_TRUNC:
                iopc = INTRN_F4MA_TRUNC;
                intrinsic_op = TRUE;
                break;
              case BUILT_IN_FMAF_FLOOR:
                iopc = INTRN_F4MA_FLOOR;
                intrinsic_op = TRUE;
                break;
              case BUILT_IN_FMAF_CEIL:
                iopc = INTRN_F4MA_CEIL;
                intrinsic_op = TRUE;
                break;
              case BUILT_IN_FMAF_IEEE_ROUND:
                iopc = INTRN_F4MA_IEEE_ROUND;
                intrinsic_op = TRUE;
                break;
              case BUILT_IN_FMAF_IEEE_TRUNC:
                iopc = INTRN_F4MA_IEEE_TRUNC;
                intrinsic_op = TRUE;
                break;
              case BUILT_IN_FMAF_IEEE_FLOOR:
                iopc = INTRN_F4MA_IEEE_FLOOR;
                intrinsic_op = TRUE;
                break;
              case BUILT_IN_FMAF_IEEE_CEIL:
             ...
             case BUILT_IN_UINT_TO_FLOAT_FLOOR:
              ...
              case BUILT_IN_ULONGLONG_TO_DOUBLE_CEIL:
                ...
                intrinsic_op = TRUE;
                break;
              case BUILT_IN_FLOAT_TO_HALF_ROUND:
                iopc = INTRN_F2F4CVT_ROUND;
                intrinsic_op = TRUE;
                break;
              case BUILT_IN_HALF_TO_FLOAT:
                iopc = INTRN_F4F2CVT;
                intrinsic_op = TRUE;
                break;
              case BUILT_IN_RCP64H:
                iopc = INTRN_F8RCP64H;
                intrinsic_op = TRUE;
                break;
#endif // NVISA
...
//                wn = WFE_Expand_Expr (TREE_VALUE (TREE_OPERAND (exp, 1)));
...
#endif
#ifdef TARG_NVISA
              case BUILT_IN_SYNCTHREADS:
                emit_builtin_threadfence (INTRN_SYNCTHREADS, exp, num_args);
                whirl_generated = TRUE;
                break;
              case BUILT_IN_THREADFENCE:
                emit_builtin_threadfence (INTRN_THREADFENCE, exp, num_args);
                whirl_generated = TRUE;
                break;
	      case BUILT_IN_THREADFENCE_BLOCK:
                emit_builtin_threadfence (INTRN_THREADFENCE_BLOCK, exp, num_args);
                whirl_generated = TRUE;
                break;
	      case BUILT_IN_THREADFENCE_SYS:
                emit_builtin_threadfence (INTRN_THREADFENCE_SYS, exp, num_args);
                whirl_generated = TRUE;
                break;
              case BUILT_IN_BRKPT:
                emit_builtin_brkpt ();
              ...
              case BUILT_IN_CLOCK:
                ...
                wn = emit_builtin_clock (exp);
                whirl_generated = TRUE;
                break;
              case BUILT_IN_CLOCK64:
              ...
              case BUILT_IN_ANY:
                ...
                whirl_generated = TRUE;
                break;
              case BUILT_IN_SYNCHRONOUS_START:
                emit_builtin_synchronous_start (exp, INTRN_SYNCHRONOUS_START);
                whirl_generated = TRUE;
                break;
              case BUILT_IN_SYNCHRONOUS_END:
                emit_builtin_synchronous_end (exp, INTRN_SYNCHRONOUS_END);
                whirl_generated = TRUE;
                break;
              case BUILT_IN_BALLOT:
                wn = emit_builtin_vote (exp, INTRN_VOTEBALLOT);
                whirl_generated = TRUE;
                break;              
              case BUILT_IN_SYNCTHREADS_COUNT:
                wn = emit_builtin_bar_red (exp, INTRN_BARREDPOPC);
                whirl_generated = TRUE;
                break; 
              case BUILT_IN_SYNCTHREADS_AND:
                wn = emit_builtin_bar_red (exp, INTRN_BARREDAND);
                whirl_generated = TRUE;
                break; 
              case BUILT_IN_SYNCTHREADS_OR:
                wn = emit_builtin_bar_red (exp, INTRN_BARREDOR);
                whirl_generated = TRUE;
                break; 
                
              case BUILT_IN_PROF_TRIGGER:
              ...
              case BUILT_IN_POPC:
                ...
                intrinsic_op = TRUE;
                break;
              case BUILT_IN_POPCLL:
                iopc = INTRN_I4U8POPC;
                intrinsic_op = TRUE;
                break;
              case BUILT_IN_BYTEPERM:
                iopc = INTRN_U4U4BYTEPERM;
                intrinsic_op = TRUE;
                break;

              case BUILT_IN_PM0:
                wn = emit_builtin_pm(INTRN_PM0);
                whirl_generated = TRUE;
                break;
              case BUILT_IN_PM1:
                wn = emit_builtin_pm(INTRN_PM1);
                whirl_generated = TRUE;
                break;
              case BUILT_IN_PM2:
                wn = emit_builtin_pm(INTRN_PM2);
                whirl_generated = TRUE;
                break;
              case BUILT_IN_PM3:
                wn = emit_builtin_pm(INTRN_PM3);
                whirl_generated = TRUE;
                break;
		...
		if (i > 0) {
			...
			// and finding $ra in fixed place.
                        warning((char*)"non-zero levels not supported for builtin_return_address");
	      ...
	      case BUILT_IN_APPLY:
		{
		  ...
		  // Store SP & Alloca
		  TY_IDX ty_idx = 
		    Make_Pointer_Type (Be_Type_Tbl (MTYPE_V));
		  ST* alloca_st_0 = 
		  ...
		  wn = WN_Lda (Pointer_Mtype, 0, tmpst, 
			       Make_Pointer_Type (ST_type(tmpst)));
...
*/
        ...
        if (Target_ISA >= TARGET_ISA_compute_20 &&
            ...
            built_in_code == BUILT_IN_PRINTF)
          call_wn = WFE_Change_Printf_To_Vprintf(call_wn);
#endif // TARG_NVISA
...
// individual structures
		...
		Aggregate_Alignment > TY_align (ret_ty_idx))
	      Set_TY_align (ret_ty_idx, Aggregate_Alignment);
            ...
            const char *mrname = Label_Name_Separator "Mreturn" Label_Name_Separator;
