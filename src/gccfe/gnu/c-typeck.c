...
#ifdef TARG_NVISA0
      /* One cannot fetch the address of a texture variable in device
         (GPU) code.  */
      if (DECL_P (arg) && lookup_attribute ("__texture__", DECL_ATTRIBUTES (arg)))
        {
	    error ("attempt to take address of texture variable `%s'",
