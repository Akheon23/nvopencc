...
output_operand_lossage VPARAMS ((const char *msgid, ...))
{
  ...
  if (-1 == asprintf (&fmt_string, "%s%s", pfx_str, _(msgid))) {}
  if (-1 == vasprintf (&new_message, fmt_string, ap)) {}
  /* both ifs above are used to please gcc 4.3.2 which does not accept the cast
