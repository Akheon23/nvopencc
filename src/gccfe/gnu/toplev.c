...
/* inline all functions under multicore mode */

int flag_multicore=0;
...
static const lang_independent_options f_options[] =
{
  ...
  {"multicore", &flag_multicore, 1,
   N_("Inline all functions under multicore mode") },
     ...
     char **argv;
{
  ...
  switch (*arg)
	...
	break;
    case 'T':
      if (!strncmp (arg, "TARG:", 5))
        break;
      else if (!strncmp (arg, "TENV:", 5))
        break;
      else
	return 0;
    case 'L':
      if (!strncmp (arg, "LANG:", 5))
        break;
      else
	return 0;
#endif
