...
/* This combination identifies Win64 */
#if (_MIPS_SZLONG == 32 && _MIPS_SZPTR == 64)
...
static tree handle_constant_attribute   PARAMS ((tree *, tree, tree, int,
						 bool *));
static tree handle_surface_attribute    PARAMS ((tree *, tree, tree, int,
                                                 bool *));
...
static tree
handle_surface_attribute (node, name, args, flags, no_add_attrs)
     tree *node;
     tree name;
     tree args;
     int flags ATTRIBUTE_UNUSED;
     ...
     bool *no_add_attrs;
{
  ...
  DECL_BLOCK_LIMIT (*node)  = next ? TREE_INT_CST_LOW (TREE_VALUE (next)) : 0;
