...
extern "C" int pstatic_as_global;
	...
	case INTEGER_TYPE:
		...
		if ( 
                      (type_tree == long_integer_type_node ||
                       type_tree == long_unsigned_type_node))
		    {
	...
	case UNION_TYPE:
		...
		if (TREE_CODE(type_tree) == UNION_TYPE) {
		   ...
		   if (strcmp(name, "char1") == 0)
			   ...
			   Set_TY_can_be_vector(idx);
		   else if (strcmp(name, "longlong3") == 0)
			   Set_TY_can_be_vector(idx);
		   else if (strcmp(name, "ulonglong3") == 0)
			   Set_TY_can_be_vector(idx);
		   else if (strcmp(name, "longlong4") == 0)
			   Set_TY_can_be_vector(idx);
		   else if (strcmp(name, "ulonglong4") == 0)
			   Set_TY_can_be_vector(idx);
		   else if (strcmp(name, "double1") == 0)
			   Set_TY_can_be_vector(idx);
		   else if (strcmp(name, "double2") == 0)
			   Set_TY_can_be_vector(idx);
		   else if (strcmp(name, "double3") == 0)
			   Set_TY_can_be_vector(idx);
		   else if (strcmp(name, "double4") == 0)
			   Set_TY_can_be_vector(idx);
		}
#endif // NVISA
  ...
  if (TREE_CODE (decl_node) == VAR_DECL &&
    ...
    if (*name == '*' ) name++;
      ...
      if (TREE_CODE(decl_node) == PARM_DECL) {
          const unsigned buf_size = 2048; // This is still too small for most C++ templated code
                                          // May need to switch to malloc eventually. 
          unsigned size_required = 0;
          static char buf[buf_size];

          // Adding out of bounds check
          size_required = strlen(ST_name(Get_Current_PU_ST())) +
                          strlen (IDENTIFIER_POINTER (DECL_NAME (decl_node))) +
                          12; // strlen ("__cudapram_%s_%s")
          if (size_required > buf_size)
              FmtAssert(FALSE, ("Mangled name too large for buffer"));
    ...
    case VAR_DECL:
      {
		...
		&& TREE_CODE(TREE_TYPE(decl_node)) == ARRAY_TYPE
                  && DECL_SHARED(decl_node)
...
*/
  ...
  if (TREE_CODE(decl_node) == PARM_DECL
    && ST_in_global_mem(Get_Current_PU_ST()))
  {
    // param in entry function
